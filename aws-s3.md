---
title: AWS S3 — ai.market  *(sub-runbook of [aws.md](./aws.md))*
...
|---|---|---|---|---|
| `aimarket-aimdata-staging` (node staging bucket, eu-north-1) | SHIPPED | created S720 via `s3api create-bucket`; lockdown commands How to operate-01 | put/get/list/delete round-trip (S720) | 2026-05-28 |
| Bucket lockdown baseline (BPA + BucketOwnerEnforced + SSE-S3) | SHIPPED | How to operate-01 command sequence; `aimarket-s3-svc` policy | verified on staging bucket (S720) | 2026-05-28 |
| `aimarket-backups-prod` (versioned + Object Lock COMPLIANCE/35d) | SHIPPED | created S723 (How to operate-05); see How to operate-06 backup-writer | first ai.market PG dump uploaded + size-matched; write/delete probe (S723) | 2026-05-28 |
...
| S3 connector assume-role (`role_arn` + `external_id`) | SHIPPED (staging) | IAM role `aimarket-connector-aimdata-staging` (trust=`ai-market-backend-sts` + ExternalId; read-only on `aimarket-aimdata-staging`); How to operate-04 | trust verified S740; assume+list green S722 | 2026-05-31 |
| Object lifecycle → Glacier Deep Archive (backups) | PLANNED | How to operate-03 | — | — |
...
- expected_failures: `IllegalLocationConstraint` → When it breaks-01; `BucketAlreadyExists` → When it breaks-02; `AccessDenied` → parent When it breaks-01
- next_step_success: for backups, continue E-05 (versioning+lock) + E-03 (lifecycle)
- next_step_failure: Repair-01
...
- expected_failures: `AccessDenied` (parent When it breaks-01); `NoSuchBucket` (parent When it breaks-04)
...
- pre_conditions: bucket exists; `external_id` chosen (random, per connection); IAM-write available (Max/console — parent Agent capabilities)
...
- expected_failures: `AccessDenied` on AssumeRole (trust principal or ExternalId mismatch) → When it breaks-03
...
|---|---|---|---|---|---|
| F-01 | `IllegalLocationConstraint` on create | missing/`!=`region `--create-bucket-configuration LocationConstraint` (only us-east-1 omits it) | inspect command | Repair-01 | CONFIRMED |
| F-02 | `BucketAlreadyExists` | global name taken by another account | name not in `s3 ls --profile aimarket` | Repair-02 | CONFIRMED |
| F-03 | Connector `AccessDenied` on AssumeRole | trust-policy principal or `sts:ExternalId` mismatch | check role trust policy vs node identity + `external_id` | Repair-03 | CONFIRMED |
| F-04 | `AccessDenied` on object op despite role | permission policy lacks the action or bucket ARN | inspect role permission policy | Repair-03 | CONFIRMED |
| F-05 | Bucket policy `put` blocked | `RestrictPublicBuckets`/`BlockPublicPolicy` rejects a public statement | review BPA vs intended policy | Repair-04 | HYPOTHESIZED |
| F-06 | High egress/latency reading bucket | bucket region != node region | `get-bucket-location` vs node region | Repair-05 | HYPOTHESIZED |
...
## Acceptance criteria
**Acceptance criteria scenario set (10, equal-weight 0.1; sum 1.0).**
