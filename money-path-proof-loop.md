...
4. **Council.** Money path, auth and customer-data scope: GLM, DeepSeek and Gemini, unanimous 3/3, builder excluded. Dispatch all three at once with `dispatch_sha` and `base_sha`. Grep the verdict token rather than reading line 1 (files sometimes begin with a title). Fold every finding into a new commit and re-dispatch; a REVISE/REJECT restarts at the same gate, never earlier. Never commit to a branch while a review on it is running — GLM flags branch mutation mid-review — and if two Mars instances are alive after a connection loss, one claims the branch on the peer bus before touching it. CC may be dispatched by explicit name for a separate non-Council second opinion but never counts in the gate (d50cbd80 override: CC in Gemini seat through 2026-10-05; see runbooks/council.md ACTIVE OVERRIDE); Kimi is removed.
