...
4. **Council.** Money path, auth and customer-data scope takes the full unanimous panel, builder excluded. Under the d50cbd80 ACTIVE OVERRIDE in `runbooks/council.md`, dispatch GLM, DeepSeek, and CC in Gemini's seat through 2026-10-05; do not dispatch Gemini or Kimi. Apply its option A review package and option C fold re-review rules. Grep the verdict token rather than reading line 1 (files sometimes begin with a title). Never commit to a branch while a review on it is running — GLM flags branch mutation mid-review — and if two Mars instances are alive after a connection loss, one claims the branch on the peer bus before touching it.
