...
> **Status R17 — 2026-07-03 (S1112)**: FK-rewires + meet-notes-bridge Gate 1 APPROVED unanimous 3/3 (`build:bq-crm-v2-phase-d-fk-rewires-meet-notes-bridge-s1112`; spec `specs/BQ-CRM-V2-PHASE-D-FK-REWIRES-MEET-NOTES-BRIDGE-S1112-GATE1.md` @ `eee16267` on `spec/bq-crm-fk-rewires-s1112`, MP-authored base `5107c02d` + Vulcan R1-mandate fold, R2 DS/GLM/AG all APPROVE). Live-prod ground truth (S1112): exactly 4 non-dropped tables hold FKs into the 14-table drop inventory — `dispute_case` (0 rows), `allai_escalation_decision` (0 rows), `telegram_user_links` (1 row needing party backfill), `meet_notes_interactions` (14 rows, 10 legacy-only) — no others. Design: drop FK constraints+columns on the two zero-row support tables; backfill + constraint-drop + one-release archival UUID retention for telegram/meet-notes; migration-owned `meet_notes_backfill_mapping` table as the committed idempotency key (Chunk 1); fail-closed backfills with exact rowcounts, explicit source `created_at`+`updated_at`, `CRM_ENTITY_IDENTITY_PROVIDER` constant; 5 chunks, each Gate-4 probed. NEXT: Gate-2 chunking spec, then build C1-C5, then pre-drop triage sweep, then the Max-gated drop.

> **Status R16 — 2026-07-03 (S1111)**: WRITE CUTOVER COMPLETE (`build:bq-crm-v2-phase-d-write-cutover-s1105` = completed, gate 4). Chunk 3 (merge/delete/link) merged `5779328f`: party-native `merge_person_records` (rehomes party+legacy tasks/interactions/relationships; §3.5 identity disposition enforced — colliding `(provider,external_id)` on TARGET deletes the source row, on a THIRD party aborts 409 naming the conflicting party; post-merge invariant asserted in code and tests: source party retains ZERO `party_identity` rows; partial-success blockers preserved), party-native `soft_delete_entity` (party tombstone + `crm_party_task`/`crm_party_interaction` cascade + full legacy cascade retained), party-native `link_person_to_user` (`auth_user` `PartyIdentity` upsert first, then legacy `user_id`/`is_platform_user` projection). Chunk 4 merged `a6868e02`: internal X-Internal-API-Key PATCH endpoint dispatches on `party.party_type` (bridge `crm_entity` identity first, direct party id fallback) — the party-only-contact-404 transition gap is CLOSED (Gate-4 live probe: party-only person PATCH 200, party+legacy dual update, unknown id 404). CI note: the CRM Testing Gate 1 no-legacy-instantiation guard is now AST-based per-function (whitelists `_project_*` projection helpers); scoped coverage floor 60% passes at 61.37% — THIN margin: pre-validate every crm_service.py merge candidate with the full CRM gate suite + coverage before pushing main. Remaining before the drop: three external FK rewires + meet_notes bridge (new BQ, next), pre-drop triage sweep ('Probe Test' party `75997df6`, 4 old `@example.com` test users), then the Max-gated destructive legacy-table drop (explicit GO + unanimous 3/3).

> **Status R15 — 2026-07-03 (S1108)**: WRITE cutover underway (`build:bq-crm-v2-phase-d-write-cutover-s1105`, spec `a0716dbc` on `spec/bq-crm-write-cutover-s1105`, Gate-1 unanimous DS/GLM/AG + Gate-2 mandate verification). Chunk 1 (dedup reads) merged `32359b38`: `find_existing_person`/`find_existing_organization`/`find_existing_person_by_name` are UNION party+legacy with canonical party-id collapse; all seven find-or-create sites routed; Gate-4 live probe confirmed no duplicate on re-upsert of a party-native contact. Chunk 2 (loaders + update flows) merged `a9f22fb4`: party-native `get_person`/`get_organization`/`get_entity` + steward `_load_scoped_entity`, `update_person`/`update_organization` mutate party detail with SAME-TRANSACTION party→legacy projection (new one-way helpers; projection failure aborts the write), advisory-lock (`pg_advisory_xact_lock` on lowered conflict key) TOCTOU closure on email/linkedin/name preflights, CRM-scoped-only lazy detail creation for the 48 detail-less parties, backfill `created_at` provenance fix as separate commit `b11c61a0`. Gate-3 caught and folded a real lock-key/preflight normalization mismatch (`9a7d065e`). Gate-4 live: update on a disposable party-native contact produced byte-equal party detail and legacy projection rows. KNOWN TRANSITION GAP: party-only contacts 404 on the internal X-Internal-API-Key PATCH endpoint until Chunk 4 (party_type detection) lands. Remaining: Chunk 3 (merge/delete/link, §3.5 identity-disposition mandate), Chunk 4, FK rewires + meet_notes bridge, Max-gated drop.

> **Status R14 — 2026-07-02 (S1105)**: Steward display reads party-routed + dead module deleted (`build:bq-crm-v2-phase-d-steward-display-reads-dead-module-s1105`, main `4742c2c0`): `get_pipeline_status` contact-name read and `crm_snapshot` recent-previews now route party-first (new scaffold `list_recent_persons`/`list_recent_organizations`), legacy fallbacks retained; `app/services/data_assembly.py` (zero production callers) deleted wholesale. CALLER SWEEP RECLASSIFICATION: `crm_service.get_person`/`get_organization` (12/12 callers mutation-entangled) and steward `_load_scoped_entity` (9 callers) are NOT read-elim targets — they move to the WRITE-cutover track, which is now the dominant remaining Phase D design (getters + `_load_scoped_entity` + find-or-create dedup reads + `endpoints/crm.py` type-detection pair + the 45 `auth_user` detail-less parties question, one Gate-1). COMPANION PROD DATA REPAIR (`bug:party-created-at-backfill-provenance-s1105`, DS+GLM unanimous, mandates folded): 46 backfilled parties (36 from the S1104 run, 10 from a 2026-06-11 batch) carried `created_at`=backfill-run time instead of the legacy creation date, corrupting every `party.created_at` recency ordering; corrected in one guarded transaction (pre-image captured), 0 mismatches remain. LESSON for any future backfill: carry `created_at` from the source row, never stamp now(). Pre-drop triage refreshed: the 'orphan' parties are expected party-natives (real contacts); 8 active test contacts + duplicate UCSF (`67a6f69c`, zero refs) surfaced to Max for purge ruling.
>
> **Status R13 — 2026-07-02 (S1104)**: Legacy→party detail backfill COMPLETED (`build:bq-crm-v2-phase-d-detail-backfill-completion-s1104`). An S1104 read-only prod audit found the Phase-A projection ~90% incomplete (401 live person parties, only 30 `party_person` rows; 338/384 legacy people with no `crm_entity` identity mapping; 36 legacy people with no party row). One idempotent INSERT-ONLY completion pass (`backfill_legacy_crm_detail_completion` in `app/domains/crm/core/service.py`, dry_run/execute, collision-skip, reports only on insert) was built by MP, unanimously approved by DS+GLM after one fold round, dry-run-verified against prod (exact match to audit), then executed: 36 parties / 354 identities / 375 detail rows created (run `crm-v2-detail-completion-806773f2`). Gate-4: all defect classes zero; `party_person`=389, `party_organization`=32; full-stream union walk 389/389 people + 32/32 orgs, 0 dupes, 0 vanishes. Merged `330fd8fe`, deployed. Data-completeness prerequisite for the getter cutover, WRITE track, FK rewires, and the Max-gated table drop is SATISFIED. Pre-drop residuals (pre-existing): 45 `auth_user` detail-less parties (design question at WRITE-track Gate 1), 3 no-legacy orphan parties incl. 'Test Person' (manual triage), duplicate UCSF org (`70a2bc98` party-native vs `67a6f69c` legacy-backfilled). See §7 Phase D.
...
| `koskadeux-mcp/tools/crm.py` | Gateway MCP tools over REST | **Primary natural-language agent surface** — `crm_request` tool family via `/api/v1/crm/*`. `app/mcp/crm_remote.py` (formerly `/mcp/crm`) deleted S1099 under BQ-CRM-MCP-ENDPOINT-REMOVAL-S1098; its ~7 legacy `select(CRM*)` reads died with it. Historical: `crm_agent_request.py` retired at `7c51e21`. |
