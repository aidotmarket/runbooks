...
| `objects[].approx_distinct_count[]` | Deterministic approximate cardinality estimate and algorithm/error flag, subject to the approved minimum-occupancy behavior | No distinct value crosses, but in one-row, two-row, and other low-population sources the estimate can disclose row-level equality or distinctness properties and therefore requires the approved occupancy rule. |
...
For `null_rate`, `approx_distinct_count`, `length_histograms`, and `numeric_range_buckets`, one-row, two-row, and other low-population or low-cardinality sources can disclose row-level properties even without revealing exact raw values. Before S3 review, the implementation candidate must fix a minimum aggregate occupancy and one deterministic behavior for each affected field: emit a fixed `suppressed_low_occupancy` sentinel, use predeclared wider buckets or coarser cardinality classes uniformly for the entire selected depth class, or remove the field through a new Gate 1 and unanimous S3 decision. The threshold and behavior are signed policy, never chosen per source by the seller or model, and production remains blocked until S3 approves them.
...
| `CAPTURED` | Stripe confirms the one capture. | Reveal the immutable complete result to the seller. No refund is due for a completed-and-captured scan. Owner chooses full publication or decline. |
...
3. A reconstruction review attempts exact raw-value and row-property recovery against representative eolymp fingerprints plus one-row, two-row, low-cardinality, and adversarial-column-name fixtures. Every affected aggregate - meaning every per-object or per-column aggregate, explicitly including `approx_distinct_count` cardinality estimates - below the S3-approved minimum occupancy is suppressed with the fixed sentinel, uniformly widened or coarsened for its entire depth class, or removed through a new Gate 1/S3 decision. Row-property recovery attempts must include equality/distinctness recovery through `approx_distinct_count`. Any recovery outside that approved policy blocks launch.
...
16. A completed-and-captured result is charged before seller reveal. Publish exposes every fact, contradiction, coverage limitation, narrative state, and disclaimer atomically; decline exposes none and leaves no active badge.
...
3. Security review passes raw-value and row-property reconstruction, D6 exfiltration/obfuscation, hostile-input, signature/nonce, commitment-key custody and dictionary non-enumerability, grounding of every model-authored public field, customer-side-sanitization, and corpus-access tests.
