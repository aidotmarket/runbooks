...
Status: **GATE 1 R3 candidate — R2 findings folded; awaiting unanimous GLM, DeepSeek and Gemini review.** This is a design only; it authorises no implementation, deployment, credential mutation or customer-data access.
...
- `binding-model`, `proof-model`, `disclosure-preimage`, `request-model`, `request-bytes`, `platform-envelope-model`, `platform-envelope-preimage`, and `local-candidate`. Any other named model whose legal inputs are restricted by a parent context receives an equivalently model-isolated operation.
...
The verifier also enforces **schema-coverage completeness in both repositories' cross-repo jobs**. Each repository derives fields from its own imported Pydantic `model_fields`, rather than from a hand-written field list: backend `DisclosureBinding`, `PreviewDisclosureRequest` and `PlatformEnvelope` (`app/schemas/listing_preview.py:29,157,314`) plus `PreviewSignedProofContract` and `PreviewSignedCommitmentContract` (`app/schemas/dataset_commitment.py:331,377`); AIM Data `DisclosureBinding`, `PreviewDisclosureRequest` and `PlatformEnvelope` (`app/models/preview_disclosure_schemas.py:25,142,291`) plus `DatasetPreviewProofContract` and `DatasetCommitmentContract` (`app/models/dataset_commitment_schemas.py:212,262`). For every named model, coverage is evaluated over the operations and parent contexts in which that model itself accepts the input; a parent model's rejection does not erase a legal value of the nested model. The checker fails unless every field is present in at least one accept vector executed through that model, every optional field is also absent in at least one accept vector executed through that model, and every allowed value of every `Literal` or enum field is present in at least one accept vector. "Present" and "absent" are measured on the parsed input vector before validation or serialization, and nested fields are attributed to the nested model class actually instantiated. Coverage is attributed by parsed model and nested field path, never by text search. This is a corpus-completeness guard, not a schema-equality substitute for the byte gate.
...
| Package identity | Accepted `proof-model` vectors exercise both `package_profile="aim-preview-package-v1"` and `package_profile="aim-preview-package-v2"` through both repositories' proof models. A full request carrying the accepted v1 proof still rejects as `unsupported_proof`, independently proving the parent request restriction. Two proofs differing in any of `preview_package_url`, `package_media_type`, `package_profile`, `package_byte_ceiling`, `scan_policy`, `scan_policy_version`, `scanned_at`, `scan_verdict` or `signer_reference` reject with package mismatch. Backend includes all nine (`backend app/schemas/listing_preview.py:202-219`); AIM Data currently includes only the first four (`aim-data/app/models/preview_disclosure_schemas.py:187-199`). |
...
3. Checks out backend history with `fetch-depth: 0` (an explicit fetch of the anchor commit is an equivalent implementation), requires `contract_anchor_sha` to be an ancestor of the tested backend commit, and requires no backend contract path to differ from anchor `A` to the tested backend commit. That backend `A..HEAD` diff includes only `app/schemas/listing_preview.py`, `app/schemas/dataset_commitment.py`, `app/utils/preview_signing.py`, `app/utils/dataset_commitment.py` and `tests/fixtures/preview/cross_repo_contract/v1/`; the lock files and `.github/workflows/preview-contract-cross-repo.yml` are explicitly excluded. AIM Data paths are not part of this backend diff: `app/models/preview_disclosure_schemas.py`, `app/models/dataset_commitment_schemas.py`, `app/services/preview_signing_service.py` and `app/services/dataset_merkle_service.py` are loaded and executed only from the exact pinned `aim_data_sha`. Thus the follow-up commit may change only the reverse pin/workflow evidence, not the wire after AIM reviewed it.
...
10. Each repository's coverage checker derives the named model fields from that repository, measures presence and omission on parsed input, and evaluates each model over contexts in which that model accepts the input. It fails any field with no accept vector, any optional field with no omission vector, and any `Literal`/enum value with no accept vector. Both repositories accept v1 and v2 through their proof models while a full request carrying the v1 proof rejects as `unsupported_proof`; removing the accepted v1 `proof-model` vector fails coverage in both repositories' jobs.
