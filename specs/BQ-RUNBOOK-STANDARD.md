...
**Status:** Gate 1 APPROVED (R9 — closes MP R8 2 remaining micro-defects; MP APPROVE_WITH_NITS + AG APPROVE)
**Priority:** P0
**Repo:** aidotmarket/runbooks
**Parent of:** per-system runbook BQs (CRM, Celery, AIM Node, Koskadeux, Infisical, Railway, etc.)
**Authored:** S486 R9 (Vulcan) — final Gate 1 revision
**Final review outcome:** MP R8 task 974e9843 (APPROVE_WITH_NITS, 2 micro-defects) + AG R8 task db17cc1f (APPROVE, 4 CLOSED + 1 PARTIAL-non-blocking). R9 closes the 2 MP micro-defects. Gate 1 trajectory: 8 MP rounds (HIGH → HIGH → MEDIUM → MEDIUM → MEDIUM → MEDIUM → APPROVE_WITH_NITS → APPROVE_WITH_NITS) + 2 AG cross-votes (CONDITIONAL → APPROVE).
...
- Directly editing DRAFT to ACTIVE may add a conformant document to the
  integrity-only catalog, but it cannot select a live pin or grant semantic or
  action authority. Exact-pin validation reconstructs the generated catalog,
  manifest, schemas, and source blobs from one commit with replacement objects
  and ambient Git controls disabled. The backend activation service alone may
  monotonically select a reviewed full commit. Catalog fields
  `semantic_verification`, `authority_admission`, and
  `action_authority_eligible` remain false; trusted claim-bound evidence and an
  independent review receipt are separate requirements for satisfying a
  runbook obligation.
...
2. **G4 attempt identification.** Each G4 run has a unique `g4_attempt_id` (UUID v4) issued by **Koskadeux**, not Vulcan. Attempt state lives in two Living State entities — a per-attempt registry and a per-frozen-commit manifest — so that multiple retries under one `frozen_commit_sha` are cleanly representable without overwriting prior attempt state.
   ...
   **At attempt start:**
   ...
   a. Vulcan emits a Living State event `event_type=g4-attempt-opened` with `entity_key=build:bq-runbook-standard`, `payload={frozen_commit_sha, opened_by: 'vulcan', retry_case: <A|B|C|D|null>, prior_attempt_id: <id|null>, request_nonce: <uuid_v4>}`. The `request_nonce` is a fresh UUID v4 generated by Vulcan for THIS open request — it is Vulcan's correlation token, distinct from the `g4_attempt_id` that Koskadeux will issue. (On first attempt for a `frozen_commit_sha`, `retry_case` and `prior_attempt_id` are `null`.)

   b. Koskadeux responds by:
   - **Writing a per-attempt registry entity** at `state:bq-runbook-standard:g4:aim-node:{frozen_commit_sha}:{g4_attempt_id}:attempt-registry` with `{g4_attempt_id, frozen_commit_sha, prior_attempt_id, retry_case, opened_at, opened_by, request_nonce, status: 'active'}`. The `request_nonce` from the originating event is preserved here as the correlation token. Create-only (`expected_version=0`); collision on this key is architecturally impossible because Koskadeux just generated the UUID.
   - **Patching the per-frozen-commit manifest** at `state:bq-runbook-standard:g4:aim-node:{frozen_commit_sha}:attempt-manifest` to append `{g4_attempt_id, request_nonce}` (a tuple, not just the id) to its `attempts` list. Manifest is updated via `patch` with optimistic locking (`expected_version` must match); concurrent opens under the same `frozen_commit_sha` serialize naturally — second opener's patch fails, Koskadeux retries with fresh read of manifest.

   c. Vulcan polls the per-frozen-commit manifest at `…:{frozen_commit_sha}:attempt-manifest` and finds the entry whose `request_nonce` matches the nonce Vulcan generated in step 2(a). The matched entry's `g4_attempt_id` is Vulcan's own attempt id. Vulcan then reads the per-attempt registry at `…:{frozen_commit_sha}:{g4_attempt_id}:attempt-registry` for authoritative attempt state. **Correlation-token rationale.** Matching on `request_nonce` rather than "newest entry" is race-safe under concurrent opens: if another attempt is appended to the manifest between Vulcan's event emission and manifest read, Vulcan still finds its own entry because the nonce is unique per request. Poll timeout: if `request_nonce` is not found after 10 seconds (exponential backoff from 100ms), Vulcan treats the open as failed and re-emits the event with a fresh nonce.
...
8. **Retry protocol.** G4 retries fall into four mechanically representable cases. Each case requires Koskadeux to issue a new `g4_attempt_id` via step 2 (the `g4-attempt-opened` event carries a `retry_case` field and `prior_attempt_id` to link attempts).
   ...
   - **Case D — XAI SUSPECT_OVERFITTING (new in R6).** XAI's correspondence challenger pass returns `SUSPECT_OVERFITTING`, indicating the MP + AG eval set appears hand-tailored to the runbook. Case D forces a fresh eval set without impugning the runbook. Mechanically:
     ...
     - **Runbook-content reviewer appointment.** Max appoints a reviewer agent independent of the prior failing eval-set authoring and scoring sessions.

       **Primary eligible reviewer: XAI.** Per its documented architecture/sign-off role, XAI is the default Case D runbook-content reviewer. XAI is also the only agent guaranteed to be eligible under the standard protocol — because step 4 makes MP a mandatory co-author of every eval-set (including the one XAI just flagged as `SUSPECT_OVERFITTING`), MP cannot review the content that triggered its own co-authored eval-set without a conflict of interest.

       **Conditional MP eligibility.** MP is eligible as Case D reviewer ONLY in the following corner case: XAI issued `SUSPECT_OVERFITTING` on an eval-set authored by a DIFFERENT MP + AG pair than the one Max is asking to perform this review — for example, in a `no_defect` chain where multiple Case D retries have cycled through different MP+AG authoring pairs (Max manually authors or delegates to a fresh agent pair per step 8 Case D `no_defect` branch), and MP now being queried as reviewer did not participate in the particular eval-set XAI flagged. The conflict-of-interest check is: did this specific MP session co-author the eval-set under review? If no, MP is eligible; if yes, ineligible. Max makes the final appointment call.

       **Ineligible:** Vulcan (conflict of interest as standard + runbook author), AG (conflict of interest as scorer and as mandatory eval-set co-author).
...
## Appendix G: R8 → R9 Change Log (final Gate 1 revision)

R8 received MP APPROVE_WITH_NITS (2 remaining micro-defects) + AG APPROVE. R9 closes the 2 MP micro-defects.

| MP R8 STILL_OPEN | R9 fix | Location |
|---|---|---|
| Case D reviewer MP-eligibility paradox: step 4 makes MP a mandatory eval-set co-author, so "MP eligible if MP didn't co-author" is unreachable in practice | Rewrote reviewer rules: XAI is the primary and always-eligible reviewer; MP conditional-only (eligible in `no_defect` retry chains where the specific MP session being queried did not co-author the eval-set under review); Max makes final call | §7 G4 step 8 Case D reviewer rules |
| Step 2(c) race condition: "newest g4_attempt_id" from manifest is not race-safe under concurrent opens | Added `request_nonce` (UUID v4) correlation token: Vulcan generates in event payload; Koskadeux preserves in registry and manifest tuple entries; Vulcan polls manifest matching on nonce (not "newest"); 10s timeout with backoff for re-emit | §7 G4 step 2(a), 2(b), 2(c) |

## Gate 1 Final State

**Status:** APPROVED
**Approval basis:** MP R8 APPROVE_WITH_NITS + AG R8 APPROVE, with R9 closing the 2 MP-flagged micro-defects. R9 is textual precision on two already-designed mechanisms (reviewer eligibility and correlation-token race fix); neither changes the protocol shape both agents approved.
**Final commit:** (set post-R9 commit)
