...
- **Rollback.** Turning the AS off (`CONNECTOR_OAUTH_ENABLED=false`) stops new authorizations and refreshes only; access tokens already issued (≤ 1 h) keep verifying locally against the cached JWKS. Live access is cut by the connector kill switch (`CONNECTOR_ENABLED=false`, or the DB global / per-profile / per-tool switches) and by grant revocation, which core checks live on every call. Action-path rolls back by its flags with its tables retained. Tables are never dropped as part of rollback.
