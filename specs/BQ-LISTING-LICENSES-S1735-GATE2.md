...
**Amended by:** [BQ-LISTING-LICENSES-S1735-GATE2-AMENDMENT-1.md](BQ-LISTING-LICENSES-S1735-GATE2-AMENDMENT-1.md) (S1738; legacy AIM Data publish withdrawal, Seller Workspace Gate 4 proof and buyer record visibility).

**Status:** Gate 2 **APPROVED 3/3** on r3 `51581db9` (2026-09-22: GLM APPROVE with 2 LOW 1 NIT, DeepSeek APPROVE with 2 NIT, Gemini APPROVE, no findings — `council/glm/response-20260922-002833-769989.md`, `council/deepseek/response-20260922-002845-747844.md`, `council/gemini/response-20260922-002856-987586.md`; the nits are folded in this status-only follow-up, no design change; Gate 3 mandates are the reviewers' §6/M-G3 lists in those files and the chunk ACs here). Build dispatch may begin with chunk A. (r2 `e243cecf`: DeepSeek APPROVE, Gemini APPROVE_WITH_MANDATES, GLM REVISE — `council/*/response-20260922-0018*`; folded below. r1 `67c26cf9`: GLM REVISE — `council/glm/response-20260922-000305-307373.md`; DeepSeek APPROVE_WITH_MANDATES — `council/deepseek/response-20260921-235022-303378.md`; Gemini APPROVE_WITH_MANDATES — `council/gemini/response-20260922-000320-469178.md`. Every finding folded in this revision by Mars (wording/scope fold, no design change; MP authored r1). Fold log at the end of §1.) No build dispatch and no production enablement until this exact document commit is approved unanimously at Gate 2 and the BQ `gate2` state matches it.
...
| Custom PDF v1 (raw bytes = ASCII `%PDF-1.4 test`, `source_sha256` `d663640088750cf16276d623c2588d7233f2b84b45f4b2e20832f47b16aa5618`), training permitted | `{"ai_training":true,"source_sha256":"d663640088750cf16276d623c2588d7233f2b84b45f4b2e20832f47b16aa5618"}` | 118 | `80a42b3f3b022e19ce60ad7b2cf59b524877b92320ec49f8af859833764a55f4` |
...
**Legal-gate truth table (binding for every chunk).** `LISTING_LICENSES_ENABLED=false`: checkout, `create_order`, agent and wallet purchases behave exactly as at the pinned backend — the buyer platform-terms gate runs under the existing `TERMS_GATE_MODE` (shadow by default), `CURRENT_TERMS_VERSION` stays 1.0, no licence acceptance is required or recorded for new orders, no licence snapshot is required for new orders (existing signed records and their lifecycle gates remain enforced), public samples are served as today, x402 follows its own `X402_ENABLED` flag, and the removed legacy `POST /generate-download-token` stays removed (the one non-reversible change, chunk C). `LISTING_LICENSES_ENABLED=true`: `CURRENT_TERMS_VERSION` becomes 1.1; the buyer platform-terms gate is enforced at order creation regardless of `TERMS_GATE_MODE`; the licence acceptance is required at the constructor and checked at every issuance door; `require_listing_license_snapshot` gates every public sample surface; inherited listings refuse purchase and samples until their seller has accepted terms 1.1; x402 is refused. Tests: `tests/test_listing_license_flag_matrix.py` asserts both columns for checkout, sample serving, terms version and x402.
...
**Files:** backend `requirements.txt` (`langdetect==1.0.9`; `pypdf` already present) and publish/listing schemas/services named in §9 plus `app/services/custom_license_service.py`, `app/api/v1/endpoints/licenses.py`, gateway v2 publish schema; frontend Seller Workspace components and listing editor/API types; AIM Data `frontend/src/pages/DatasetDetail.tsx`, `frontend/src/lib/api.ts`, `app/routers/marketplace_publish.py`, publish models/client.

**AC/tests:** exactly two cards; toggle defaults true visibly and on omitted API input; covenant/identity required; custom hygiene matrix, including the §9 language matrix (English, Spanish, 50-character document), fails closed — canonical test path `tests/test_listing_license_custom_hygiene.py`; bytes immutable; all three publish routes produce identical hashes and seller records; approval hash invalidates on change. Tests: backend `tests/test_custom_license_upload.py`, `tests/test_listing_license_publish.py`, `tests/test_listing_license_seller_acceptance.py`; frontend `components/seller-workspace/*License*.test.tsx`; AIM Data `tests/test_marketplace_publish_license.py` and `frontend/src/pages/DatasetDetail.test.tsx`.
...
**Risk/rollback:** bulk default mistaken for authority. Nullable acceptance and explicit 409 prevent it. Turning the flag off makes new purchases revert to legacy behaviour; existing signed records and their lifecycle gates remain enforced; public contract pages stay available.

### 12.7 Chunk G — operations and real test-pool E2E

**Files:** runbooks `listing-licenses.md`, `account-teardown.md` (H.1 legal-record inventory), `INDEX.md`, `ERRORS.md`; E2E specs/fixtures only in the repository that owns the existing test harness. No production code is added here except a narrowly reviewed defect found by the E2E.
...
The §5 legal-gate truth table binds every chunk; each chunk's flag-off proof asserts its column.
...
- tables and immutability rules for `license_documents`, `seller_license_acceptances`, `license_acceptances`, `license_acceptance_events`, listing/version snapshots and organisation identity;
