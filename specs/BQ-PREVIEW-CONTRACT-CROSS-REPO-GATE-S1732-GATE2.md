...
`backend-transition` accepts only: initial installation; no SHA change; or the single reverse-pin follow-up where `manifest_sha256` is unchanged, `aim_data_sha` advances to the fetched AIM commit whose own lock names `A` and that digest, and `contract_anchor_sha` either remains `A` or changes once from an initial placeholder to `A`. The placeholder must be a valid pre-`A` backend ancestor, never a floating ref; no later change to `contract_anchor_sha` is allowed except a new reviewed paired wire anchor. The reverse-pin follow-up is accepted only when the exact `aim_data_sha` is reachable from fetched AIM Data `origin/main`: with full AIM history, run `git -C peer/aim-data-main merge-base --is-ancestor "$AIM_DATA_SHA" origin/main` after verifying the checked-out pin and its lock/digest. An unmerged AIM commit fails with `aim_data_sha_not_merged_to_main` before either runner. The named disposable diagnostic mode in §9 may inspect an unmerged AIM commit and reach later diagnostic stages, but its result cannot satisfy the required `Preview contract cross-repo` check on an implementation PR or `main`. Require the branch's own post-`A` changes to contain only the two permitted paths below. Any non-anchor pin bump fails. The push of the AIM merge classifies against its `before` SHA as no-change or a permitted AIM transition; the push of the backend merge classifies the reverse-pin sequence against its `before` SHA, with the same anchored evidence. Test merged and unmerged AIM pins on both PR and push events.

`anchor-diff` checks the tested backend `HEAD` contains `A`, then identifies the backend branch tip: fetch `refs/pull/<number>/head` on a PR and use that exact ref; on the backend merge-commit push, use `HEAD^2` and require a two-parent merge with `HEAD^1` on the previous `main` history. Fetch `origin/main` with full history on a PR. Compute the branch base as `git merge-base <branch-tip> origin/main` on PR or `git merge-base HEAD^2 HEAD^1` on push, and require that base to be an ancestor of `A`. The branch tip must contain `A`; the branch's post-`A` tree diff, not the synthetic PR merge ref or merged `main` tree diff, is confined to the two named paths. The implementation branch must not merge `main` into itself after `A`; rebase/update by replacing `A` is also forbidden. In shell terms, after resolving and verifying those event-specific refs:

```bash
git merge-base --is-ancestor "$CONTRACT_ANCHOR_SHA" HEAD
git merge-base --is-ancestor "$BRANCH_BASE" "$CONTRACT_ANCHOR_SHA"
git merge-base --is-ancestor "$CONTRACT_ANCHOR_SHA" "$BRANCH_TIP"
git diff --exit-code "$CONTRACT_ANCHOR_SHA" "$BRANCH_TIP" -- . ':(exclude)preview-contract-aim-data.lock.json' ':(exclude).github/workflows/preview-contract-cross-repo.yml'
```

Unrelated commits added to backend `main` while the PR is held open do not enter the `A..BRANCH_TIP` diff. A changed, added, renamed, or deleted branch path anywhere else fails, including `app/services/listing_preview_disclosure.py`. AIM paths are never compared in a backend diff; they execute from the pinned AIM checkout. Once AIM pins `A`, the backend branch must not amend, rebase, or delete `A`. Test an unrelated backend-`main` commit that still passes and a disposable post-anchor change to `app/services/listing_preview_disclosure.py` that fails.
    ...
    steps:
      ...
      - name: Verify pins, anchor, and manifest
        run: |
          ...
          git -C peer/aim-data-main fetch --no-tags origin main
          if ! git -C peer/aim-data-main merge-base --is-ancestor "${{ steps.lock.outputs.aim_data_sha }}" origin/main; then
...
jq --arg repo "$REPO" '
  ((.required_status_checks.checks // []) +
   ((.required_status_checks.contexts // []) | map({context:.,app_id:-1})))
