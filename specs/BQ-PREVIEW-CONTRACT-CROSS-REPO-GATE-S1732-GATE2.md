...
The protected backend contract paths for ordinary PRs and unchanged-lock pushes are the two lock/workflow paths above; `app/services/listing_preview_disclosure.py`; `app/schemas/listing_preview.py`; `scripts/preview_contract_gate.py`; `scripts/build_preview_contract_corpus.py`; `tests/preview_contract_runner.py`; `tests/test_preview_contract_cross_repo.py`; `tests/test_listing_preview_database.py`; `tests/test_listing_preview_contracts.py`; and everything below `tests/fixtures/preview/cross_repo_contract/v1/`. Count additions, deletions, and renames by old and new path. A later change to any of these paths needs a new reviewed paired anchor; unrelated backend-main movement remains green. AIM paths execute only from the pinned AIM checkout. Once AIM pins `A`, do not amend, rebase, or delete it. Test the held-open PR with unrelated main movement, ordinary direct and merge pushes, an ordinary PR, manual re-runs, a new paired anchor, and a post-anchor service-path mutation that fails.
...
for SNAPSHOT in before after; do
  jq -S --arg repo "$REPO" --slurpfile before /tmp/preview-protection-before.json '[.required_status_checks.checks[]? | select(.context != "Preview contract cross-repo" and (.context != "producer-contract" or $repo != "aidotmarket/aim-data")) | .context as $name | select(any($before[0].required_status_checks.checks[]?; .context == $name)) | if has("app_id") then {context,app_id} else {context} end] | sort_by(.context)' "/tmp/preview-protection-$SNAPSHOT.json" > "/tmp/preview-bindings-$SNAPSHOT.json"
done
