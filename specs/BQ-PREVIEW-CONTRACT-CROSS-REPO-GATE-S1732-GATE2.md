...
Before committing anchor `A` or letting AIM pin it, generate the corpus in the temporary proposal directory and place the complete raw byte/digest diff against the prior committed corpus (the empty corpus on first installation) in the backend implementation PR. For every new or changed accepting vector, show its input diff, `.bin` bytes in a reviewable hex diff, byte length, and SHA-256; record a reviewer's approval of each row before copying the proposal into the committed corpus. A digest-only summary or review after either code merge does not satisfy this gate.
...
The developer-only corpus writer uses that same synthetic seller seed for every signed proof, commitment, request, and envelope `seller_signature`; it uses `SHA-256(b"preview-contract-cross-repo-v1 synthetic platform signing seed")` for envelope `signature`. These are Ed25519 32-byte private seeds, never corpus fields. The generated seller public key must match each fixture's signer reference: its 64-lowercase-hex fingerprint suffix is `SHA-256` of the raw 32-byte Ed25519 seller public key, encoded as lowercase hexadecimal without a prefix. The platform public key is base64url-encoded raw 32-byte Ed25519 material in the envelope's `signer_keys` entry whose `key_id` equals the envelope `key_id`, with its SHA-256 fingerprint in that entry. All signatures are base64url-encoded raw 64-byte Ed25519 signatures. The Node verifier derives these two public keys independently from the fixed seeds using Node crypto, checks the referenced key IDs, raw-key lengths, and fingerprints, and verifies signatures over independently serialized domain-prefixed preimages. For direct `proof-model` rows, `proof-v1-policy-v1` uses the five-field signing context in `commitment-v1-previous-absent`, and `proof-v2-policy-v2` uses `commitment-v2-previous-present`; the proof object and signature must equal the corresponding nested proof. Node verifies proof signatures with `proof_bytes(context, proof)` semantics and commitment signatures with `commitment_bytes(commitment)` semantics, including nested proofs in `request-approve-v2`. It verifies `seller_signature` on every signed request and envelope against `disclosure_bytes(binding)`, and envelope `signature` against `platform_envelope_bytes(envelope)` using the matching embedded platform key. Binding/model/preimage/local-candidate rows without a signature field carry no standalone signature assertion; their bytes and digest assertions still apply. The verifier obtains all signature fields from the named `inputs/<id>.json` objects and the platform public key from `signer_keys`; no key/signature sidecars or manifest fields are permitted. Corrupting any proof, commitment, request, or envelope signature must fail Node, and a focused test must fail if a declared signature check is omitted.
...
Each runner first resolves and changes to `--repo-root`, prepends only that root to `sys.path`, and rejects an imported `app` module whose resolved path is outside that root. This prevents the caller checkout from satisfying the peer runner's imports. It then emits canonical result JSON with `repo_sha`, `manifest_sha256`, sorted `results`, and `coverage_observations`. An accept result has `id`, `status:"accept"`, `byte_length`, `sha256`, and base64url `bytes`; a reject result has `id`, `status:"reject"`, ordered Pydantic `errors` containing exact `loc`, `type`, and normalized custom `code`. `coverage_observations` is a sorted array with one entry per accepted vector/model instance, including nested instances, each with exactly `id`, `model` (the §3.1 token or fully qualified nested model name), `input_path` (JSON Pointer from the input root), `direct` (boolean: that model is the operation's direct target), and `fields`. `fields` is a sorted array with one entry per `model_fields` member, exactly `name`, `present` (boolean), and `value` (the parsed JSON value when present, omitted when absent). Nested model instances have their own entries; no reject result supplies observations. The runner obtains these observations from its parsed input before validation and emits them only after the model accepts. `coverage --result` reads this section, resolves the named models from its own repository, checks exact field sets/types and direct/nested attribution, and applies §3.4 without rereading the corpus or trusting a supplied list of allowed values. Normalize a custom code only from the exact validator token in `ctx.error`/`msg`, never by substring search. Unexpected exceptions fail the runner. Neither runner writes the corpus. Lock and manifest validation before dependency installation use only the Python standard library.
...
1. Register exactly `DisclosureBinding`, the proof contract, the commitment contract, `PreviewDisclosureRequest`, and `PlatformEnvelope`. Proof/commitment sources are AIM `app/models/dataset_commitment_schemas.py:212-279` and backend `app/schemas/dataset_commitment.py:331-394`; binding/request/envelope sources are in §2. Include recursively nested `ProofSibling` and `SignerKeyEvidence`.
   Normalize `shared.WithdrawalRequest`, `shared.RefreshRequest`, and `shared.SupersessionRequest` coverage observations to the registered `PreviewDisclosureRequest` field walk and inherited field inventory; any other unregistered request token fails.
...
For either transition command, a `pull_request` event uses `git merge-base HEAD <event pull_request.base.sha>` as its diff base; the exact base object must be present in the full self-checkout. A `push` to `main` uses the event's `before` SHA as the diff base (verify it is reachable with the full checkout), including a merge commit. The all-zero `before` SHA is allowed only for initial repository creation and uses the empty tree. A GitHub manual re-run uses its original PR or push payload and the same immutable event SHAs, never current `origin/main`; `workflow_dispatch` is not admitted. Reject other event types, unavailable bases, and shallow histories. Classify the lock diff from that base through the tested `HEAD`; unchanged locks are accepted. No transition command performs a post-checkout unauthenticated fetch. Unit tests cover PR, push, manual re-run, and initial installation.

`aim-transition` accepts only: initial installation when the file is absent at the diff base; no SHA change; or a new `backend_sha` equal to the pinned backend checkout `HEAD` with a new `manifest_sha256` equal to its verified corpus digest. The anchor declaration is the AIM lock's `backend_sha` together with the pinned backend commit's own corpus transition: require that commit's first-parent corpus `manifest.sha256` is absent (initial installation) or differs, and that its manifest/file-set checks pass. Thus an unchanged-manifest, non-anchor SHA bump cannot be declared merely by editing a lock or PR description. The backend reverse lock's `contract_anchor_sha`, read from the tested backend checkout during `backend-transition`, must later equal this same SHA; it cannot be self-referential inside anchor `A`. The `anchor-diff` branch-tip confinement separately proves no later contract change. A same-manifest pin-only change fails. Test the structural declaration and a same-digest non-anchor bump on both PR and push events.

`backend-transition` accepts only: initial installation; no SHA change; or the single reverse-pin follow-up where `manifest_sha256` is unchanged, `aim_data_sha` advances to the pinned AIM commit whose own lock names `A` and that digest, and `contract_anchor_sha` either remains `A` or changes once from an initial placeholder to `A`. At initial anchor `A`, the backend lock must set `aim_data_sha=6529e1ed4d5983a87a9b1e2faadda32e18d4cf76` and placeholder `contract_anchor_sha=d5d2e3915fb5067e04b03dfe97716da380002b6b`; the reverse-pin follow-up changes only those SHA values to merged `B` and `A` respectively, leaving `manifest_sha256` unchanged. Lock fixtures require these exact initial and follow-up values. The placeholder must be a valid pre-`A` backend ancestor, never a floating ref; no later change to `contract_anchor_sha` is allowed except a new reviewed paired wire anchor. The reverse-pin follow-up is accepted only when the exact `aim_data_sha` is reachable from the full AIM Data `main` checkout: run `git -C peer/aim-data-main merge-base --is-ancestor "$AIM_DATA_SHA" HEAD` after verifying the pinned checkout's lock/digest. An unmerged AIM commit fails with `aim_data_sha_not_merged_to_main` before either runner. The named disposable diagnostic mode in §9 may inspect an unmerged AIM commit and reach later diagnostic stages, but its result cannot satisfy the required `Preview contract cross-repo` check on an implementation PR or `main`. Require the branch's own post-`A` changes to contain only the two permitted paths below. Any non-anchor pin bump fails. The push of the AIM merge classifies against its `before` SHA as no-change or a permitted AIM transition; the push of the backend merge classifies the reverse-pin sequence against its `before` SHA, with the same anchored evidence. Test merged and unmerged AIM pins on both PR and push events.

`anchor-diff` takes the tested backend checkout, the event JSON, and the exact anchor `A`; it derives the lock transition classification itself from that event and the lock diff, including at the workflow call site in §5.2. Its base and pass/fail rule are event-specific:

| Event | Exact base and branch tip | Pass/fail rule |
|---|---|---|
| Held-open implementation PR or a later PR declaring a new reviewed paired anchor | Base is the event's `pull_request.base.sha`; tip is the event's `pull_request.head.sha`, verified as `HEAD^2` of the full self-checkout's PR merge commit. Compute `BRANCH_BASE=git merge-base BASE_SHA BRANCH_TIP`. | Require `BRANCH_BASE` ancestor of `A`, `A` ancestor of `BRANCH_TIP` and tested `HEAD`, and no merge of main into the branch or replacement of `A` after declaration. `git diff A BRANCH_TIP` may change only `preview-contract-aim-data.lock.json` and `.github/workflows/preview-contract-cross-repo.yml`. A new anchor must also pass the paired corpus/manifest transition in `aim-transition` and the reviewed reverse-pin transition; a lock-only claim fails. |
| Ordinary PR after the anchor is on protected main | Base is the event's `pull_request.base.sha`; tip is the same verified `pull_request.head.sha`. `BRANCH_BASE=git merge-base BASE_SHA BRANCH_TIP`. | Require `A` ancestor of base, tip, and tested `HEAD`; lock unchanged against `BRANCH_BASE`; and no protected contract-path change in `git diff BRANCH_BASE BRANCH_TIP`. Other paths pass. |
| Initial backend merge push or a later reviewed paired-anchor merge push | Base is the push event's `before` SHA; tip is `HEAD^2`. Require a two-parent merge, `HEAD^1=before`, and `BRANCH_BASE=git merge-base HEAD^1 HEAD^2`. | Apply the held-open anchor rule to `BRANCH_BASE`, `A`, and `HEAD^2`; verify the lock transition against `before..HEAD` and the paired AIM pin/manifest evidence. Any missing parent, base, or declaration fails. |
| Later push to protected `main` with an unchanged lock | Base is the push event's exact `before` SHA and tip is the event's `after` SHA, equal to tested `HEAD`; no second parent is required. | Require `before` ancestor of `HEAD`, `A` ancestor of both, an unchanged lock against `before`, and no protected contract-path change in `git diff before HEAD`. An unrelated direct commit or ordinary merge passes; any contract-path drift fails. |
| Manual re-run | Use the original PR or push event payload, base and tip SHAs above; verify the exact objects are present. | Apply that event's same rule. Never substitute the current moving `main` or branch ref; unavailable historical objects fail closed. |

Declare one `BACKEND_CONTRACT_PATHS` inventory in backend `scripts/preview_contract_gate.py` and use it for every ordinary-PR and unchanged-lock-push `anchor-diff` check. Its exact entries are `preview-contract-aim-data.lock.json`; `.github/workflows/preview-contract-cross-repo.yml`; `app/services/listing_preview_disclosure.py`; `app/schemas/listing_preview.py`; `app/schemas/dataset_commitment.py`; `app/utils/preview_signing.py`; `app/utils/dataset_commitment.py`; `scripts/preview_contract_gate.py`; `scripts/build_preview_contract_corpus.py`; `tests/preview_contract_runner.py`; `tests/test_preview_contract_cross_repo.py`; `tests/test_listing_preview_database.py`; `tests/test_listing_preview_contracts.py`; and the directory prefix `tests/fixtures/preview/cross_repo_contract/v1/`. Gate 1's five backend contract paths are entries in this same inventory; do not maintain a separate checker list. Count additions, deletions, and renames by old and new path. A later change to any of these paths needs a new reviewed paired anchor; unrelated backend-main movement remains green. AIM paths execute only from the pinned AIM checkout. Once AIM pins `A`, do not amend, rebase, or delete it. Test the held-open PR with unrelated main movement, ordinary direct and merge pushes, an ordinary PR, manual re-runs, a new paired anchor, each of the three added Gate 1 paths failing on an unchanged-lock ordinary PR and main push, an unrelated path passing, and a post-anchor service-path mutation that fails.
...
The exact fetches are the `actions/checkout@v4` steps above: each other-repository pinned SHA uses its target-only deploy key; AIM Data `main` for backend reachability uses that same AIM Data key in its separate checkout; and the backend PR branch tip is the second parent of the full self-checkout's PR merge commit, asserted equal to the event's exact head SHA in the `Verify pins, anchor, and manifest` step. All use `persist-credentials: false`; subsequent shell steps use only local objects and perform no `git fetch`. The full self-checkout supplies PR base and push `before` history. No token is written to `.git/config` for a later step. Neither workflow contains `pull_request_target`, a bless/update flag, an artifact containing SSH configuration, or a conditional secret skip. A fork PR without the secret fails at the key preflight.
...
2. Open the paired implementation PRs; complete the §2.2 raw corpus proposal review and record approval before committing backend anchor `A`. Then commit `A` and set AIM's lock to `A` and its manifest digest.
3. Observe a successful AIM Data `Preview contract cross-repo` PR job. On the backend implementation PR, an unmerged AIM reverse pin must fail `backend-transition`; only a separate disposable diagnostic run may examine it, and that run never reports the required check green.
4. **Max-gated admin step:** read existing protection in both repositories. After the new AIM check has reported green, atomically replace only the obsolete required context from the deleted AIM workflow (`producer-contract`, if present) with `Preview contract cross-repo`. For backend, add the observed new required context only after its implementation PR job reports green following AIM merge `B`; until then retain its existing protection and keep the backend PR unmergeable. Apply the remaining protection/merge-method settings below, and read them back before the respective merge.
5. After AIM protection read-back and an actual failed AIM mutation job, prove a disposable failing AIM PR is unmergeable by an administrator.
6. Merge AIM Data as `B`; add the backend's one reverse-pin follow-up naming `B`; rerun its required job until green and complete backend protection with read-back.
7. After backend protection read-back and an actual failed backend mutation job, prove a disposable failing backend PR is unmergeable by an administrator. Then merge backend by merge commit; confirm both `main` jobs pass and `A` is an ancestor.

### 6.3 Exact protection settings and read-back

For each of `aidotmarket/aim-data` and `aidotmarket/ai-market-backend`, Max preserves all unrelated stronger existing protection and applies these exact minimums: required check `Preview contract cross-repo`, strict/up-to-date branch required, administrators enforced, zero pull-request bypass actors, force pushes disabled, deletions disabled, and repository merge methods `allow_merge_commit=true`, `allow_squash_merge=false`, `allow_rebase_merge=false`. First read both `contexts` and `checks` from protection; if the deleted AIM workflow's `producer-contract` appears in either, remove precisely that entry in the same required-status-checks API update that adds the observed new check. Do not remove any other context or check. The backend has no deleted parity workflow; retain any backend `producer-contract` producer if one exists. Preserve each unrelated existing `checks[]` object exactly, including its `app_id` when present and the absence of that member when absent. Convert a legacy `contexts[]` name to `{context}` only; never attach `app_id:-1` to an existing context or widen an App-bound check to any App. Before PATCH, record each unrelated context and its `checks[].app_id` value or absence from the API response; after PATCH, read both arrays back and require the same provider binding for every retained context. `app_id:-1` is used only for the newly observed `Preview contract cross-repo` check. Refuse each repository's transition if its new check has not reported green on its implementation PR (backend only after merged AIM `B`). Read protection back immediately after the update and require the new check present and the obsolete AIM context absent before that repository's merge.
...
jq --arg repo "$REPO" '
  ...
  | map(select(.context != (if $repo == "aidotmarket/aim-data" then "producer-contract" else "__never__" end)))
  | reduce .[] as $item ([]; if any(.[]; .context == $item.context) then . else . + [$item] end)
  | if any(.context == "Preview contract cross-repo") then . else . + [{context:"Preview contract cross-repo",app_id:-1}] end
  | {strict:true,checks:.}
' /tmp/preview-protection-before.json > /tmp/preview-required-checks.json
test "$(jq -r 'keys | sort | join(",")' /tmp/preview-required-checks.json)" = checks,strict
test "$(jq '.checks | length' /tmp/preview-required-checks.json)" -gt 0
gh api --method PATCH "repos/$REPO/branches/main/protection/required_status_checks" --input /tmp/preview-required-checks.json
...
gh api "repos/$REPO/branches/main/protection" > /tmp/preview-protection-after.json
for SNAPSHOT in before after; do
  jq -S --arg repo "$REPO" --slurpfile before /tmp/preview-protection-before.json '
    def bindings:
      ((.required_status_checks.checks // []) +
       ((.required_status_checks.contexts // []) | map({context:.})))
      | reduce .[] as $item ([]; if any(.[]; .context == $item.context) then . else . + [$item] end)
      | map(if has("app_id") then {context,app_id} else {context} end);
    ($before[0] | bindings | map(select(.context != "Preview contract cross-repo" and (.context != "producer-contract" or $repo != "aidotmarket/aim-data")))) as $retained
    | [bindings[] | .context as $name | select(any($retained[]; .context == $name))]
    | sort_by(.context)
  ' "/tmp/preview-protection-$SNAPSHOT.json" > "/tmp/preview-bindings-$SNAPSHOT.json"
done
diff -u /tmp/preview-bindings-before.json /tmp/preview-bindings-after.json
...
Evidence lives in both implementation PR descriptions and the “Changing the verified-preview wire contract” history table in `aim-data.md`: exact `A`, `B`, merge SHAs, manifest digest, the pre-`A` §2.2 raw byte/digest diff with each changed accept row's length, SHA-256 and reviewer approval, workflow job URLs, mutation SHAs/job URLs, deploy-key IDs (never values), protection read-backs, and rollback result. Each chunk's builder report must identify the exact candidate commit SHA and its parent, changed-file list, final spec/corpus digest where applicable, and the command, exit status, and untruncated result or log URL for `python3 scripts/index.py && python3 scripts/check.py` and `git diff --check` for this runbooks fold, plus that chunk's named focused tests, lock/manifest checks, coverage, comparison, and Node check where applicable. Record Gate 1 blob identity and YAML parsing for this spec fold. A prior-candidate log or line count does not verify a later candidate; failures and skips are explicit. The candidate SHA must equal the reviewed PR head before review dispatch.
...
| AC2 | Pre-`A` raw input/byte/digest diff and approval per changed accept row; `preview_contract_gate.py verify-manifest`; exact corpus tree/file-set report |
...
| AC9 | Disposable M1 and M2 anchors. M1 adds an optional omit-None binding field **and** v2 vector/bytes; M2 adds the field with no vector. For each mutation, record the exact backend mutation anchor `A_m`, AIM pin-only commit `B_m`, backend diagnostic reverse-pin commit `C_m`, and four distinct failure records specified below. Unit simulation is supplemental only; delete the branches after recording evidence. |
| AC10 | `coverage` reports; use the disposable paired diagnostic corpus/anchor below to remove `proof-v1-policy-v1`; record both diagnostic runs passing integrity/transition and runner stages, then failing coverage on the missing direct proof v1 value; the required backend check remains red for an unmerged AIM diagnostic pin |
...
For each AC9 mutation, the AIM implementation PR's required `Preview contract cross-repo` job at `B_m` must fail on M1 acceptance/bytes or M2 coverage against pinned `A_m`. The backend implementation PR's required job at `A_m`, before the diagnostic reverse pin, must fail `backend-transition` for the stale AIM pin. After the reverse pin at `C_m` names unmerged `B_m`, that same required backend job must fail `aim_data_sha_not_merged_to_main` before either runner. Separately run a disposable **non-required** backend diagnostic command/job at `C_m` with `backend-transition --diagnostic-unmerged-aim`; it must pass lock/manifest/transition and reach M1 acceptance/bytes failure or M2 coverage failure. Record each of these four job URLs or command-log URLs with its exact tested SHA and failure stage; the diagnostic result never reports the required check green and is forbidden on `main`.
