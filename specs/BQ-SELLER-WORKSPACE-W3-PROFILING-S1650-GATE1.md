...
| `POST /profile-jobs/estimates` | Accept one verified runtime, its acknowledged standing-cost receipt, immutable selector ID, and quota profile. Persist and return a short-lived immutable marginal-cost receipt binding seller, connection, runtime, standing-receipt/source-key-set hashes, selector hash, region, fixed task allocation, quotas, retry assumption, bounded Fargate/Lambda/S3-request/S3-storage-byte-duration/S3-in-region-transfer/KMS/Secrets-Manager/EventBridge/task-and-audit-log/interface-endpoint units, regional price-table version, estimate-model version, currency, bounded low/high estimate, disclosed timestamp, and expiry. No job or task is created. |
...
Before invoking the verifier, ai.market generates a 128-bit random nonce and atomically inserts its SHA-256 hash under a unique `(seller_id, connection_id, runtime_version, nonce_hash)` constraint in the durable control database; a duplicate reservation fails before invocation, and a lost response is retried only with a fresh nonce while the original reservation remains until expiry. The runtime-verification call carries the closed canonical envelope `seller_workspace_profile_runtime_verify.v1` with exactly: `schema_version`, `operation=verify_runtime`, `seller_id`, `connection_id`, `expected_runtime_version`, `expected_stack_id_hash`, `expected_template_digest`, `expected_broker_alias_arn_hash`, `expected_verifier_alias_arn_hash`, `expected_cluster_arn_hash`, `expected_control_key_arn_hash`, `expected_cluster_control_key_binding_hash`, `expected_source_key_set_hash`, `expected_task_definition_arn_hash`, `expected_image_digest`, `expected_network_hash`, `nonce`, `issued_at`, and `expires_at`. It has no job, selector, quota, parser, or result fields. The verifier resolves the hash-bound resources only from the exact stack membership and encrypted owner-bound source-key registration, recomputes the cluster-to-key, source-key-set/policy, and network hashes from live configuration, rejects unknown fields, expiry, clock skew over 60 seconds, nonce/binding mismatch, a resource outside the exact tagged stack/registered set, or any expected/observed mismatch, and returns only the receipt defined in section 6.3, bound to the nonce hash. The seller verifier is stateless and validates nonce shape, binding, issuance, expiry, and clock skew; replay/single-use enforcement belongs only to the durable ai.market reservation boundary.
...
- the standing model assumes no free-tier/discount allocation and versions the exact control-key month, field-token secret month, interface-endpoint count/AZ-hours/data rates, runtime-verifier Lambda request/allocation-duration, source-key count plus `DescribeKey`/`GetKeyPolicy` request units, EventBridge AWS-service-event/target-delivery rates (including an explicit versioned zero where applicable), and task/audit log retention/storage inputs, regional price table, currency, and template lifetime used for its range. The marginal model similarly versions Fargate's one-minute minimum/per-second rules, fixed duration/allocation, attempt count, broker Lambda request/allocation-duration, Secrets Manager retrievals, EventBridge event/delivery units, action-specific S3/KMS operation counts, transient control-object byte-duration, explicitly versioned in-region S3 transfer, task/audit CloudWatch ingestion bytes, and interface-endpoint processed bytes. The runtime-verification receipt and job attestation record the closed bounded units so every displayed model component and range is exactly recomputable without changing the semantic hash. Registered source keys pre-exist W3, so their storage key-month is not an incremental W3 unit; their verification/decrypt calls are. These are conservative modeled disclosures, not a claim to reproduce taxes, negotiated discounts, credits, or the final AWS invoice;
...
Fargate destruction removes task memory and ephemeral storage. The one-day S3 lifecycle is a recovery backstop, not proof of immediate cleanup. Control-bucket versioning, replication, Object Lock, inventory exports, access logging with object names, and backup are disabled by the W3 stack so deletion does not create retained copies. CloudWatch task logs contain only allowlisted lifecycle fields and expire after seven days. CloudTrail and VPC Flow Logs are not W3 product-runtime resources, availability preconditions, or D2 receipt units. Gate 4's synthetic seller-account test harness temporarily creates exact scoped trail/data selectors and ENI/VPC flow logs with bounded destinations/retention and a separately approved test budget, verifies the evidence below, and deletes those harness resources; any seller's pre-existing account audit remains outside W3 and must not contain request/result bodies.
...
- `seller_workspace_profile_runtime_verification_nonces`: seller/connection/runtime ownership, nonce hash only, issued/expiry timestamps, optional receipt hash, and terminal outcome. A unique `(seller_id, connection_id, runtime_version, nonce_hash)` constraint provides the one-use reservation; rows expire under the existing control-data cleanup job after the receipt window;
...
- `seller_workspace_profile_cost_estimates`: seller, connection, runtime and selector ownership, acknowledged standing-receipt/source-key-set hashes, selector hash, region, task allocation, quota profile, retry assumption, bounded Lambda/S3-request/S3-storage-byte-duration/S3-transfer/KMS/Secrets-Manager/EventBridge/task-and-audit-log/interface-endpoint units, regional price-table and estimate-model versions, currency, bounded low/high marginal estimate, immutable receipt hash, disclosed/expiry timestamps, and optional consumed job ID. The receipt is append-only and single-use;
...
- verifier receipt and IAM evidence proving the operation-specific verifier read every required exact-stack configuration, including exact cluster-to-CMK binding, registered source-key set/policies, and task/endpoint security-group egress, returned only closed hashes/mismatch codes, and could not launch/stop a task, pass a role, mutate infrastructure, read a source/control object, decrypt data, or retrieve a secret value;
...
- endpoint-policy, route, security-group, and synthetic Gate 4 harness VPC Flow Log evidence that the private task pulled the exact image through the regional `prod-${AWS::Region}-starport-layer-bucket`, reached only the verified source/control resources, VPC resolver, S3 managed prefix list, and exact interface-endpoint security group, and could not reach another S3 bucket/key, unrelated intra-VPC ENI/service, public S3 endpoint, NAT, internet gateway, ai.market, or allAI destination;
- source-read byte counts and frozen ETag/version/size/source-encryption proof, closed per-attempt compute/Lambda/S3/action-specific KMS/Secrets Manager/EventBridge/task-and-audit-Logs/interface-endpoint usage counters, exact standing/marginal estimate-range recomputation, request/result checksum/size/media-type/KMS/tag/schema validation, stable semantic hash plus separately recomputed attestation/result-integrity hashes, exact request/result cleanup timestamps, and bounded exact-attempt absence-list results after deletion;
...
- runtime verification covers duplicate nonce-hash reservation under concurrent ai.market transactions, fresh-nonce recovery after a lost verifier response, stateless seller-side nonce shape/binding/expiry checks across cold starts, exact stack-resource membership, exact cluster `Configuration.ManagedStorageConfiguration` control-key binding, zero-to-eight registered source keys and exact task-policy/key-policy restrictions, task/endpoint security-group rules, VPC resolver/S3 prefix-list destinations, every required configuration read, closed receipt hashes/mismatch codes, and explicit denial of every verifier mutation, task-launch, role-pass, source/control-object read, decrypt/data-key, and secret-value action;
...
- source-key registration/verification covers zero and eight keys, alias/wildcard/duplicate/ninth/cross-account/cross-region/asymmetric/disabled keys, wrong or broadened key policy, stale set hash, template/runtime/selector/attempt binding, S3-managed source success, current/versioned exact registered-key success, and missing/unregistered/different returned SSE-KMS key rejection before body parsing;
- redaction scans seed recognizable synthetic source-key ARNs through registration, authorization/template delivery, verifier mismatch/error, broker transport, and task failure paths, then prove those raw ARNs are absent from HTTP errors, structured logs, traces, Celery payloads/results, audit, evidence, unencrypted database JSON, result validation errors, and allAI; only owner-authorized encrypted persistence/process memory and owner-visible template instructions may contain them.
