...
Round-10 review of candidate `0f0cac4f1fec03db18a3804a3b33c4d6ab97f979`, file SHA-256 `7847469ef94f35b995ce44b7b9f30d46e64bea6db02b0c5b6e801fdb8caae407`, returned CC `APPROVE`, GLM `REQUEST_CHANGES`, and DeepSeek `REQUEST_CHANGES`, recorded in event `c70465f4-2f94-4339-8f29-67fb29c62293`. This corrective revision folds every round-10 blocker, nit, and the CC receipt-field advisory. No round-10 verdict carries forward to this digest.

Round-11 review of candidate `8462fae1a4af6304283ac309f7b6a339f7ca3ae4`, file SHA-256 `f5e43e7005405fd8fe599391bc449218bc88de22cf0ed934df348e910200c09e`, returned CC `APPROVE`, GLM `REQUEST_CHANGES`, and DeepSeek `APPROVE_WITH_NITS`, recorded in event `589bbf81-100c-425f-b0af-0123c3412c7e`. This corrective revision folds the Route 53 Profiles blocker and every round-11 nit. No round-11 verdict carries forward to this digest.

Round-12 review of candidate `4da58ce4160bec4eba18a7ad690591c7f919c5f3`, file SHA-256 `28632e5f4b19e7a85bb486892ae2d954ab44c39c3780b3477cc956f818f86e1f`, returned CC `REQUEST_CHANGES`, GLM `APPROVE`, and DeepSeek `REQUEST_CHANGES`, recorded in event `6a7bb78c-4c4d-410a-8c04-fdeda4316f3b`. This corrective revision folds the direct-rule-classifier blocker and every round-12 nit. No round-12 verdict carries forward to this digest.

Round-13 review of candidate `e52126370969ec5372355b81b7d7c213bd1919d1`, file SHA-256 `82cb11f2c98a2c00a89824a9657e14fe71242290ecc070282f63ffe58340a79b`, returned CC, GLM, and DeepSeek `APPROVE_WITH_NITS`, recorded in event `bb4a2ec8-214b-45ac-a56e-feea365373f6`. This final wording revision folds every round-13 nit. No round-13 verdict carries forward to this digest.
...
The verifier accepts only the operation-specific envelope in section 7 and reads only configuration belonging to the exact tagged stack/runtime. It has `cloudformation:DescribeStacks`, `cloudformation:GetTemplate`, and `cloudformation:ListStackResources` for the exact stack; Lambda configuration/alias/policy reads for the broker and verifier; IAM role and policy-document reads for the five exact roles (connection-control, verifier, broker, task, and task-execution) and their attached policies; `ecs:DescribeClusters` for the exact cluster and `ecs:DescribeTaskDefinition` for the pinned revision; ECR image-description reads for the approved repository/digest; `kms:DescribeKey` and `kms:GetKeyPolicy` on the exact control key and up to eight runtime-registered source keys; control-bucket public-access, policy, encryption, lifecycle, versioning, replication, and Object Lock configuration reads; EventBridge rule/target and CloudWatch log-group configuration reads for the exact stack resources; the EC2 describe calls required for the stack's exact VPC, subnet, route-table, security-group, endpoint, endpoint-security-group, VPC-resolver, and S3 managed-prefix-list IDs; read-only Route 53 Resolver calls `GetFirewallConfig`, `GetFirewallRuleGroup`, `ListFirewallRules`, `GetFirewallDomainList`, `ListFirewallDomains`, `GetFirewallRuleGroupAssociation`, and bounded `ListFirewallRuleGroupAssociations` for the exact stack-found firewall resources and exact VPC; bounded `ListResolverRuleAssociations` with the exact `VPCId`, `GetResolverRuleAssociation`, and `GetResolverRule` for every returned association; bounded `ListResolverEndpoints` with service-side `Direction=OUTBOUND` and exact `HostVPCId` filters, followed by `GetResolverEndpoint` and `ListResolverEndpointIpAddresses` for every returned endpoint; and bounded `route53profiles:ListProfileAssociations` with `ResourceId` equal to the exact W3 VPC ID. Every direct association and rule must be `COMPLETE`, have no `ResolverEndpointId`, have `ShareStatus=NOT_SHARED`, and identify an AWS-owned autodefined rule: `OwnerId="Route 53 Resolver"` and ARN `arn:${partition}:route53resolver:${region}::autodefined-rule/rslvr-autodefined-rr-*` with the exact runtime partition and region. The verifier does not use `RuleType` as an ownership or safety predicate; AWS-owned autodefined rules are accepted solely by the closed service-owned identity above, regardless of the type AWS returns. It rejects every seller-created or RAM-shared direct rule regardless of type, every exact-VPC outbound endpoint, and any nonempty Profile-association result regardless of returned owner, status, or VPC identifier; the exact filtered Profile list must be empty after full pagination. Directly associated private hosted zones can alter seller-local answers but cannot create a forwarding path; the exact S3 endpoint-policy resource allowlist, task egress, and TLS still restrict the resolved destination to the exact allowed AWS resources, so this remains within the documented compromised-seller self-falsification boundary rather than an ai.market custody or exfiltration path. It requires the cluster's `Configuration.ManagedStorageConfiguration.FargateEphemeralStorageKmsKeyId` to equal the exact control-key ARN; every registered source key to be an enabled symmetric customer-managed key in the seller account and runtime region whose key-policy application statements permit only the exact task role to decrypt through regional S3 with the expected source-object encryption context and the exact verifier role to perform only `DescribeKey`/`GetKeyPolicy`; the live source-key-set hash to equal the authorization/runtime binding; and the DNS Firewall association, canonical allow-domain list, one-entry `*` block list, priorities, `ALLOW`/`BLOCK NODATA` actions, `INSPECT_REDIRECTION_DOMAIN`, wildcard-allow absence, and `FirewallFailOpen=DISABLED` to equal section 5. It rejects a missing/different binding, alias, wildcard allow, unapproved redirection target, extra firewall association, any direct rule association outside the exact AWS-owned autodefined identity above, any exact-VPC outbound Resolver endpoint, any exact-VPC Route 53 Profile association, a cross-account/region key, or broader task-role use. AWS describe/list APIs that do not support resource ARNs use `Resource: *`, but the closed verifier code accepts only the connection role and IDs bound to the exact connection or found in the exact `ListStackResources` result, supplies the exact-VPC server-side filter on each association/endpoint list above, revalidates every returned direct-rule VPC and endpoint/subnet ID client-side, bounds and exhausts pagination, and hashes the fixed canonical safe projection `{non_autodefined_rule_associations:0, endpoint_bearing_rule_associations:0, outbound_endpoints:0, profile_associations:0, pagination_complete:true}` into the resolver-control hash. Allowed AWS autodefined rule enumeration, names, domains, and VPC-CIDR-dependent reverse rules do not enter that stable projection. It emits only the source-key-set, DNS-firewall, and resolver-control hashes rather than raw source-key ARNs, policies, domain names, Profile details, credentials, or secret values.

The verifier has explicit denies for every mutation, `iam:PassRole`, `ecs:RunTask|StopTask|ExecuteCommand`, Route 53 Resolver `Create*|Update*|Delete*|Associate*|Disassociate*|ImportFirewallDomains|Put*|TagResource|UntagResource`, Route 53 Profiles `Create*|Update*|Delete*|Associate*|Disassociate*|TagResource|UntagResource`, STS role chaining, `s3:GetObject*`, `s3:ListBucket` on the source bucket, `kms:Decrypt|GenerateDataKey`, `secretsmanager:GetSecretValue`, and any resource not tagged/bound to the one runtime. It returns only a closed `seller_workspace_profile_runtime_verification.v1` receipt containing the expected/observed runtime version, stack/template/broker/task-definition/image/role/policy/network/control-resource/source-key-set/DNS-firewall/resolver-control hashes, `nonce_hash`, verification timestamp, safe mismatch codes, and receipt hash; the receipt hash covers every preceding receipt field including `nonce_hash`. Raw policies, DNS domain lists, Profile details, ARNs not already stored for the owning connection, subnet/routes, bucket names, and source-prefix identities do not cross to ai.market.
...
| `POST /connections/{connection_id}/profile-runtime/estimates` | Before exposing the versioned stack template/upgrade instructions, create an immutable standing-cost receipt binding seller, connection, source-key-set hash/count, runtime/template digest, region, Availability Zone and interface-endpoint count, endpoint hourly/data-processing rates, control-key/Secrets Manager monthly storage and request-price assumptions, explicit versioned zero foundational DNS Firewall rule-group/association rate, exact custom-domain count and domain-name-month rate, explicit prohibition and zero association-hours for Advanced rules, explicit zero W3 Profile-VPC association units, verifier Lambda and KMS/Route-53-Resolver/Route-53-Profiles configuration-read units, EventBridge and audit-log assumptions, price-table/model/currency versions, bounded monthly/hourly range, disclosure time, and 15-minute expiry. A nonzero Profile association on the exact W3 VPC makes the runtime ineligible rather than adding Profile pricing to the W3 range; unrelated pre-existing Profile costs and registered source-key storage are not W3-created units, while W3-attributable verification and decrypt requests remain included. No AWS resource is created. |
...
| `POST /connections/{connection_id}/profile-runtime/verify` | Require the bound runtime-cost acknowledgement, then invoke only the immutable runtime-verifier alias with the closed verification envelope below. Verify the exact stack/template, cluster-to-CMK binding, broker/verifier aliases, role/policy split, task-definition and image digests, control-resource settings, private subnets/routes/security-group egress/endpoint rules, exact DNS Firewall walled-garden association/rules/domain-list/redirection/fail-mode hash, direct Resolver state, absence of any exact-VPC Route 53 Profile association, and absence of source-read or mutation authority. Persist only the closed hashed verification receipt. It never creates or changes AWS resources. |
...
| `POST /profile-jobs/estimates` | Accept one verified runtime, its acknowledged standing-cost receipt, immutable selector ID, and quota profile. Persist and return a short-lived immutable marginal-cost receipt binding seller, connection, runtime, standing-receipt/source-key-set/DNS-firewall hashes, selector hash, region, fixed task allocation, quotas, retry assumption, bounded Fargate/Lambda/S3-request/S3-storage-byte-duration/S3-in-region-transfer/EC2-cross-AZ-transfer/KMS/Secrets-Manager/EventBridge/Route-53-Resolver-DNS-Firewall-query/task-and-audit-log/interface-endpoint units, regional price-table version, estimate-model version, currency, bounded low/high estimate, disclosed timestamp, and expiry. DNS Firewall query units include queries caused by following approved CNAME/DNAME chains. No job or task is created. |
...
- the standing model assumes no free-tier/discount allocation and versions the exact control-key month, field-token secret month, interface-endpoint count/AZ-hours/data rates, explicit zero foundational DNS Firewall rule-group/association rate, exact custom-domain count and domain-name-month rate, explicit zero Advanced-rule association-hours, explicit zero W3 Profile-VPC association units, runtime-verifier Lambda request/allocation-duration, source-key count plus `DescribeKey`/`GetKeyPolicy` and Route 53 Resolver/Route 53 Profiles configuration-read request units, EventBridge AWS-service-event/target-delivery rates (including an explicit versioned zero where applicable), and task/audit log retention/storage inputs, regional price table, currency, and template lifetime used for its range. The marginal model similarly versions Fargate's one-minute minimum/per-second rules, fixed duration/allocation, attempt count, broker Lambda request/allocation-duration, Secrets Manager retrievals, EventBridge event/delivery units, DNS Firewall inspected-query units including redirection-follow queries, action-specific S3/KMS operation counts, transient control-object byte-duration, explicitly versioned in-region S3 transfer, task/audit CloudWatch ingestion bytes, fixed image manifest/layer bytes, interface-endpoint processed bytes, and possible EC2 cross-AZ transfer bytes. The runtime-verification receipt and job attestation record their separate closed bounded units so every displayed model component and range is exactly recomputable without changing the semantic hash. Registered source keys and unrelated seller Profile associations pre-exist W3, so their storage/association charges are not incremental W3 units; verification/decrypt calls are, and any Profile association on the exact W3 VPC fails verification instead of being costed as an eligible runtime. These are conservative modeled disclosures, not a claim to reproduce taxes, negotiated discounts, credits, or the final AWS invoice;
...
- `seller_workspace_profile_runtime_cost_estimates`: seller/connection ownership, source-key-set hash/count, runtime/template digests, region/AZ/endpoint count, control-key/field-token-secret months, explicit exclusion of pre-existing source-key months and unrelated Profile charges, endpoint-hour/data, explicit zero foundational DNS Firewall rule-group/association rate, exact custom-domain count/domain-name-month rate, explicit zero Advanced-rule association-hours and W3 Profile-VPC association units, verifier-Lambda, KMS describe/policy-read, bounded Route 53 Resolver and Route 53 Profiles configuration-read units, EventBridge, and task/audit-log retention/storage units, price-table/model/currency versions, bounded low/high standing range, immutable receipt hash, disclosed/expiry timestamps, and optional authorization ID. The closed canonical receipt projection contains every named unit and assumption; the receipt is append-only and single-use;
- `seller_workspace_profile_runtimes`: seller/connection ownership, AWS region, status, optimistic version, encrypted or redacted broker/runtime references, verified task/image/network/role hashes, exact cluster-to-CMK, source-key-set/policy, DNS-firewall, and resolver-control hashes, standing-cost receipt/acknowledgement actor/time/event/hash, token-key version, verified/disabled timestamps;
...
- `seller_workspace_profile_cost_estimates`: seller, connection, runtime and selector ownership, acknowledged standing-receipt/source-key-set/DNS-firewall hashes, selector hash, region, task allocation, quota profile, retry assumption, exact DNS allow-domain count/redirection-chain query ceiling, bounded Lambda/S3-request/S3-storage-byte-duration/S3-transfer/EC2-cross-AZ-transfer/KMS/Secrets-Manager/EventBridge/DNS-Firewall-query/task-and-audit-log/interface-endpoint units, fixed image manifest/layer bytes, regional price-table and estimate-model versions, currency, bounded low/high marginal estimate, immutable receipt hash, disclosed/expiry timestamps, and optional consumed job ID. The closed canonical receipt projection contains every named unit and assumption; the receipt is append-only and single-use;
...
- verifier receipt and IAM evidence proving the operation-specific verifier read every required exact-stack configuration, including exact cluster-to-CMK binding, registered source-key set/policies, task/endpoint security-group egress, exact DNS Firewall VPC association/rules/domain-list/redirection/fail-mode hash, exact direct Resolver rule/endpoint state, and absence of any Route 53 Profile association on the exact W3 VPC; returned only closed hashes/mismatch codes; and could not launch/stop a task, pass a role, mutate infrastructure, read a source/control object, decrypt data, or retrieve a secret value;
...
- runtime verification covers duplicate nonce-hash reservation under concurrent ai.market transactions, fresh-nonce recovery after a lost verifier response, stateless seller-side nonce shape/issuance/expiry checks across cold starts, nonce-hash receipt coverage, atomic first-response `pending -> consumed` binding, concurrent/replayed receipt rejection, rejected/expired outcomes, exact stack-resource membership, exact cluster `Configuration.ManagedStorageConfiguration` control-key binding, zero-to-eight registered source keys and exact task-policy/key-policy/broker-environment restrictions, mutation/removal/non-canonical broker environment rejection, task/endpoint security-group rules, VPC resolver/S3 prefix-list destinations, exact DNS Firewall rule group/allow-and-block-domain-lists/VPC association/priorities/actions/redirection and `FirewallFailOpen=DISABLED`, plus absence of wildcard allows/extra associations/Advanced rules. Bounded paginated `ListResolverRuleAssociations(VPCId=exact)` reads followed by exact gets must accept every returned AWS-owned autodefined rule only when its association/rule is `COMPLETE`, has no endpoint, is `NOT_SHARED`, and has the exact service owner/ARN namespace; seller-created `SYSTEM`, endpoint-bearing, foreign-owner, shared, malformed-namespace, and nonterminal cases fail. Bounded `ListResolverEndpoints(Direction=OUTBOUND, HostVPCId=exact)` followed by exact endpoint/IP-address gets must reject every returned endpoint, and fully paginated `ListProfileAssociations(ResourceId=exact VPC ID)` must return an empty set; any returned object fails without relying on whether that SDK/service version renders its `ResourceId` as an ID or ARN. Each service-side filter is revalidated where a nonempty safe result is possible; omission, truncation, permission denial, mismatched direct-rule/endpoint IDs, or an unexhausted token fails verification. Clean AWS autodefined state and every prohibited direct/shared/Profile case pin the fixed safe resolver-control projection and hash without VPC-specific rule inventory, and redaction tests prove no Profile/rule/endpoint details cross to ai.market. Tests also cover every other required configuration read, closed receipt hashes/mismatch codes, and explicit denial of every verifier mutation, task-launch, role-pass, source/control-object read, decrypt/data-key, secret-value, named Route 53 Resolver mutation pattern, or named Route 53 Profiles mutation pattern;
...
- the verifier validates every pinned task-definition field, immutable broker source-key environment, exact cluster-to-CMK configuration, exact DNS-firewall hash, and exact resolver-control hash covering zero non-autodefined/endpoint-bearing direct rules, zero exact-VPC outbound endpoints, zero Profile associations, and complete pagination; broker-side `RunTask` permits that exact revision only, and `DescribeTasks(include=["TAGS"])` validates the persisted exact task ARN, revision, image, returned Fargate ephemeral-storage KMS key, network attachments, and task tags before accepting a result; the task role and task network have no ECS API path;
...
- Route 53 Resolver rule associations and VPC forwarding scope: <https://docs.aws.amazon.com/Route53/latest/APIReference/API_route53resolver_ListResolverRuleAssociations.html>
- Route 53 Resolver rules and outbound-endpoint identity: <https://docs.aws.amazon.com/Route53/latest/APIReference/API_route53resolver_GetResolverRule.html>
- Route 53 Resolver AWS autodefined, custom, system, and recursive rule semantics: <https://docs.aws.amazon.com/Route53/latest/DeveloperGuide/resolver-overview-forward-vpc-to-network-using-rules.html>
- Route 53 Resolver dedicated autodefined-rule inventory and type semantics: <https://docs.aws.amazon.com/Route53/latest/DeveloperGuide/resolver-overview-forward-vpc-to-network-autodefined-rules.html>
- Route 53 Resolver official CLI examples for autodefined association, rule ID, ARN, type, and owner fields: <https://docs.aws.amazon.com/cli/latest/userguide/cli_route53resolver_code_examples.html>
- Route 53 Resolver endpoint host-VPC identity: <https://docs.aws.amazon.com/Route53/latest/APIReference/API_route53resolver_ListResolverEndpoints.html>
- Route 53 Resolver endpoint subnet identity: <https://docs.aws.amazon.com/Route53/latest/APIReference/API_route53resolver_ListResolverEndpointIpAddresses.html>
- Route 53 Profiles effective VPC DNS resources and settings: <https://docs.aws.amazon.com/Route53/latest/DeveloperGuide/profiles.html>
- Route 53 Profiles exact-VPC association listing and pagination: <https://docs.aws.amazon.com/Route53/latest/APIReference/API_route53profiles_ListProfileAssociations.html>
- Route 53 Profiles IAM action/resource semantics: <https://docs.aws.amazon.com/service-authorization/latest/reference/list_route53profiles.html>
- Route 53 Profiles pricing: <https://aws.amazon.com/route53/pricing/>
