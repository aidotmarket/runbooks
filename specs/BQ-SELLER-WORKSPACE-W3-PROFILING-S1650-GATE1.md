...
| `POST /profile-jobs/estimates` | Accept one verified runtime, its acknowledged standing-cost receipt, immutable selector ID, and quota profile. Persist and return a short-lived immutable marginal-cost receipt binding seller, connection, runtime, standing-receipt/source-key-set hashes, selector hash, region, fixed task allocation, quotas, retry assumption, bounded Fargate/Lambda/S3-request/S3-storage-byte-duration/S3-in-region-transfer/EC2-cross-AZ-transfer/KMS/Secrets-Manager/EventBridge/task-and-audit-log/interface-endpoint units, regional price-table version, estimate-model version, currency, bounded low/high estimate, disclosed timestamp, and expiry. No job or task is created. |
...
- `RunTask`/`TagResource`/`DescribeTasks`/`StopTask` evidence that tag-on-create succeeds only through `ecs:CreateAction=RunTask` with the exact allowed keys/fixed values/safe dynamic shapes; standalone tagging and missing/extra/wrong tags fail; only the fixed request-key/checksum bootstrap pair differs from the task definition; the verifier proves the pinned task-definition, immutable broker source-key environment, exact cluster task-namespace/resource-tag conditions, and absence of `ListTasks`; the broker enforces the exact revision and validates the persisted exact task ARN/hash/image/Fargate-CMK/network/tags before result acceptance or stop; and attempted cross-cluster/task, command, selector-valued environment, role, image, resource, or network overrides fail before ECS action;
