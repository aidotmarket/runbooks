...
- private subnets, separate no-ingress task and Lambda security groups, and only the endpoints needed to verify, launch, and run the task. The broker and verifier Lambdas attach to those private subnets and the Lambda security group; that group's egress permits TCP 443 only to the exact interface-endpoint security group and regional S3 managed prefix list, plus DNS only to the VPC resolver, while IAM still prevents either Lambda from source-object reads. Task-security-group egress uses the same destination classes and has no other rule. The interface-endpoint security group admits TCP 443 only from the task and Lambda security groups. The S3 gateway endpoint policy is an explicit allowlist for: the verified source bucket and objects under its exact connection prefix; the control bucket and only `requests/<connection-id>/.../request.json` plus `results/<connection-id>/.../evidence.json`; and read-only `s3:GetObject` on the mandatory regional ECR layer objects `arn:${AWS::Partition}:s3:::prod-${AWS::Region}-starport-layer-bucket/*`. Source/control bucket resources are additionally restricted to the verified seller account. ECR API, ECR Docker, CloudWatch Logs, Secrets Manager, and required KMS use private interface endpoints;
...
The runtime-verification call carries the closed canonical envelope `seller_workspace_profile_runtime_verify.v1` with exactly: `schema_version`, `operation=verify_runtime`, `seller_id`, `connection_id`, `expected_runtime_version`, `expected_stack_id_hash`, `expected_template_digest`, `expected_broker_alias_arn_hash`, `expected_verifier_alias_arn_hash`, `expected_cluster_arn_hash`, `expected_control_key_arn_hash`, `expected_cluster_control_key_binding_hash`, `expected_source_key_set_hash`, `expected_task_definition_arn_hash`, `expected_image_digest`, `expected_network_hash`, `nonce`, `issued_at`, and `expires_at`. It has no job, selector, quota, parser, or result fields. The verifier resolves the hash-bound resources only from the exact stack membership and encrypted owner-bound source-key registration, recomputes the cluster-to-key, source-key-set/policy, and network hashes from live configuration, rejects unknown fields, expiry, clock skew over 60 seconds, nonce replay, a resource outside the exact tagged stack/registered set, or any expected/observed mismatch, and returns only the receipt defined in section 6.3.
...
```json
{
  ...
  "runtime_attestation": {
    ...
    "usage": {
      ...
      "vpc_flow_log_ingested_bytes": 0,
      "cloudtrail_management_events": 0,
      "cloudtrail_data_events": 0,
...
- the standing model assumes no free-tier/discount allocation and versions the exact control-key month, field-token secret month, interface-endpoint count/AZ-hours/data rates, runtime-verifier Lambda request/allocation-duration, EventBridge AWS-service-event/target-delivery rates (including an explicit versioned zero where applicable), required CloudTrail management/data-event and VPC Flow Log ingestion/storage assumptions, task/audit log retention/storage inputs, regional price table, currency, and template lifetime used for its range. The marginal model similarly versions Fargate's one-minute minimum/per-second rules, fixed duration/allocation, attempt count, broker Lambda request/allocation-duration, Secrets Manager retrievals, EventBridge event/delivery, CloudTrail event, and VPC Flow Log byte units, action-specific S3/KMS operation counts, task/audit CloudWatch ingestion bytes, and interface-endpoint processed bytes. The runtime-verification receipt and job attestation record the closed bounded units so every displayed model component and range is exactly recomputable without changing the semantic hash. These are conservative modeled disclosures, not a claim to reproduce taxes, negotiated discounts, credits, or the final AWS invoice;
...
- `seller_workspace_profile_runtime_cost_estimates`: seller/connection ownership, source-key-set/runtime/template digests, region/AZ/endpoint count, control-key/field-token-secret months, endpoint-hour/data, verifier-Lambda, EventBridge, CloudTrail/VPC Flow Log, and task/audit-log retention/storage units, price-table/model/currency versions, bounded low/high standing range, immutable receipt hash, disclosed/expiry timestamps, and optional authorization ID. The receipt is append-only and single-use;
...
- `seller_workspace_profile_cost_estimates`: seller, connection, runtime and selector ownership, acknowledged standing-receipt/source-key-set hashes, selector hash, region, task allocation, quota profile, retry assumption, bounded Lambda/Secrets Manager/EventBridge/CloudTrail/VPC-Flow-Log/audit units, regional price-table and estimate-model versions, currency, bounded low/high marginal estimate, immutable receipt hash, disclosed/expiry timestamps, and optional consumed job ID. The receipt is append-only and single-use;
