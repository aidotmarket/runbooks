...
The request object, not a container command or selector-valued environment variable, is the immutable task input. `RunTask` may supply exactly two broker-derived bootstrap environment values: `W3_REQUEST_KEY` (the canonical key constructed from the validated connection/job/attempt) and `W3_REQUEST_SHA256` (the checksum of the create-only request object). The broker never copies an ai.market-provided override map and rejects every other container, command, environment, role, image, resource, or network override. The fixed entrypoint retrieves only that key and verifies checksum, media type, KMS key, request-object tags, schema, input hash, selector scope, and expiry before reading source data. The task does not call ECS or inspect ECS task tags. Before accepting a result, the broker describes the persisted exact task ARN and verifies the task-definition revision, image digest, network attachments, and broker-set task tags against the immutable request. The request contains the validated envelope and selector; it is not an audit-only artifact. A request/runtime mismatch stops before source read; a later ECS identity/tag mismatch prevents result acceptance, requests stop of that exact task, and terminally fails the attempt.
...
- `ecs:RunTask` for the exact task-definition family/revision on the exact cluster, with Fargate launch type, required task tags, and only the two broker-derived request bootstrap values in section 5. The broker derives the ECS `clientToken` as the 64-character lowercase hex SHA-256 over the canonical version, seller, connection, job, attempt, and input hash; the same start envelope and request parameters therefore replay the same AWS request, a changed input conflicts, and ECS's 24-hour token lifetime exceeds the 20-minute job deadline;
...
- `s3:GetObject` plus `s3:GetObjectTagging` only for `results/<connection-id>/<job-id>/<attempt>/evidence.json`, and `s3:DeleteObject` only for those exact request and result key shapes after terminal acknowledgement. The broker validates result size and checksum from the bounded `GetObject` response and never calls `GetObjectAttributes`;
...
- `s3:GetObject` on that exact source prefix and, only when versioned selectors are used, `s3:GetObjectVersion`; the task never calls `HeadObject` or `GetObjectAttributes`, and the dedicated attribute actions remain absent;
...
| --- | --- |
| `POST /connections/{connection_id}/profile-runtime/verify` | Invoke only the immutable runtime-verifier alias with the closed verification envelope below. Verify the exact stack/template, broker/verifier aliases, role/policy split, task-definition and image digests, control-resource settings, private subnets/routes/security group/endpoints, and absence of source-read or mutation authority. Persist only the closed hashed verification receipt. It never creates or changes AWS resources. |
...
| `POST /profile-jobs/estimates` | Accept one verified runtime, immutable selector ID, and quota profile. Persist and return a short-lived immutable cost-estimate receipt binding seller, connection, runtime, selector hash, region, fixed task allocation, quotas, retry assumption, regional price-table version, estimate-model version, currency, bounded low/high estimate, disclosed timestamp, and expiry. No job or task is created. |
...
The runtime-verification call carries the closed canonical envelope `seller_workspace_profile_runtime_verify.v1` with exactly: `schema_version`, `operation=verify_runtime`, `seller_id`, `connection_id`, `expected_runtime_version`, `expected_stack_id_hash`, `expected_template_digest`, `expected_broker_alias_arn_hash`, `expected_verifier_alias_arn_hash`, `expected_task_definition_arn_hash`, `expected_image_digest`, `nonce`, `issued_at`, and `expires_at`. It has no job, selector, quota, parser, or result fields. The verifier rejects unknown fields, expiry, clock skew over 60 seconds, nonce replay, a resource outside the exact tagged stack, or any expected/observed mismatch and returns only the receipt defined in section 6.3.

Every broker start/poll/cancel call instead carries `seller_workspace_profile_broker_request.v1` with exactly: `schema_version`, `operation=start|poll|cancel`, `seller_id`, `connection_id`, `runtime_version`, `job_id`, `attempt`, `task_arn`, `input_hash`, `selector`, `quota_profile`, `parser_version`, `image_digest`, `evidence_schema_version`, `cost_estimate_receipt_hash`, `cost_acknowledgement_event_id`, `price_table_version`, `estimate_model_version`, `currency`, `issued_at`, and `expires_at`. `task_arn` is JSON null for start and the decrypted exact ARN for poll/cancel; the broker requires it to belong to the exact cluster and to hash to the persisted attempt binding before calling ECS. Fields not applicable to `poll|cancel` remain present and must match the persisted immutable start request. The seller, connection, runtime, job, attempt, selector, limits, estimate/acknowledgement, and digests are immutable. The broker rejects unknown fields, expiry, clock skew over 60 seconds, a conflicting replay, an input-hash mismatch, a selector outside the verified prefix, stale/mismatched cost authority, a poll/cancel task mismatch, and any runtime identity drift.
...
The ai.market profile-control worker uses the existing Redis/Celery substrate only for metadata orchestration. It has a new dedicated `seller_workspace_profile_control` queue and a separately deployed worker with concurrency 4, prefetch 1, late acknowledgement, and no source SDK client. The Fargate task, not the Celery child, is the parser. Task state is reconciled only by broker poll responses over the exact encrypted attempt-to-task binding; seller-account task events are audit evidence, not control input. After `RunTask` acceptance, the broker returns the exact ARN and ai.market atomically stores its encrypted value plus hash before moving `starting -> running`. If the worker or transport fails before that commit, redelivery sends the identical start envelope; the broker reconstructs the identical deterministic ECS `clientToken`, AWS returns the one idempotent task, and the binding is committed without a duplicate launch. A stale Celery delivery may replay that start or poll the bound task idempotently but may not create a new attempt.
...
The task uploads the closed semantic object/hash plus bounded task-observed usage. After the exact task reaches `STOPPED`, the broker recomputes the semantic hash, reconciles task observations with ECS state and the immutable attempt records, and constructs the final runtime attestation from provider timestamps, fixed allocation, verified runtime identity, closed application-operation counters, log-ingestion bytes, and the current immutable price-table record, then computes the attestation and result-integrity hashes. The cost-usage counters aggregate at most two attempts and are non-negative, bounded by the quotas and request graph, and contain no identifier or source-derived value. It never edits semantic evidence. The broker persists the two subobjects immutably only after every check succeeds. W4 may reference only `semantic_hash`; timestamps, CPU/memory observations, billable duration, region, allocation, request/log usage, and price-table provenance never alter it. Golden fixtures pin canonical `semantic_evidence` bytes and `semantic_hash`; runtime tests pin the attestation schema and recomputation, not volatile values.
...
- before start, `POST /profile-jobs/estimates` creates the immutable receipt defined in section 7. The seller sees selected object count, maximum bytes read, task size, maximum runtime, possible retry count, currency, and bounded AWS-cost range; `POST /profile-jobs` requires explicit acknowledgement of that exact current receipt;
- the estimate model versions both the regional prices and the fixed maximum Fargate duration/allocation, attempt count, S3 operation counts, KMS operation counts, and CloudWatch log bytes used for the disclosed low/high range. The runtime attestation records exact bounded billable compute/allocation units and closed S3/KMS/Logs usage counters across all attempts, plus price-table version, estimate-model version, and currency, so every modeled component and the disclosed range are exactly recomputable without changing the semantic hash;
...
A stale/missing price table makes profiling unavailable; it never silently estimates zero. An estimate expires after 15 minutes and is single-use for one job creation. A selector, runtime, region, quota, task allocation, retry assumption, price-table version, estimate-model version, currency, or bounded-range change requires a new estimate and acknowledgement. No listing fee, ai.market infrastructure surcharge, or billing integration is introduced by W3.
...
- `seller_workspace_profile_cost_estimates`: seller, connection, runtime and selector ownership, selector hash, region, task allocation, quota profile, retry assumption, regional price-table and estimate-model versions, currency, bounded low/high estimate, immutable receipt hash, disclosed/expiry timestamps, and optional consumed job ID. The receipt is append-only and single-use;
...
Database checks enforce state enums, positive versions/attempts/limits, terminal timestamps, phase deadlines not exceeding the absolute deadline, payload size, hash shapes, unique seller/idempotency operation, unique job/attempt and client-token hash, at most one encrypted task ARN per attempt, one job per consumed estimate receipt, one evidence row per successful attempt, and same-seller composite foreign keys. Audit update/delete triggers remain append-only. Selector, estimate, acknowledgement, attempt identity, and successful evidence payloads are immutable; a new source, estimate binding, or parser version creates a new selector/estimate/job/evidence row.
...
- verifier receipt and IAM evidence proving the operation-specific verifier read every required exact-stack configuration, returned only closed hashes/mismatch codes, and could not launch/stop a task, pass a role, mutate infrastructure, read a source/control object, decrypt data, or retrieve a secret value;
...
- `RunTask`/`DescribeTasks` evidence that only the fixed request-key/checksum bootstrap pair differed from the task definition, both matched the create-only request, the verifier proved the pinned task-definition fields, the broker enforced that exact revision and verified the bound exact task ARN/image/tags before result acceptance, and attempted command, selector-valued environment, role, image, resource, or network overrides failed before launch;
...
- immutable seller-visible estimate receipt, exact acknowledgement actor/time/event and launch binding, plus recomputable actual task usage.
...
- the verifier validates every pinned task-definition field; broker-side `RunTask` permits that exact revision only, and `DescribeTasks` validates the persisted exact task ARN, revision, image, network attachments, and task tags before accepting a result; the task role and task network have no ECS API path;
- identical start replay before and after a simulated worker/transport crash uses the identical deterministic ECS client token, recovers and encrypts the same exact task ARN, creates no duplicate, and permits exact poll/stop/attestation/cleanup; a different input, attempt, or ARN fails closed, and neither plaintext ARN nor token appears in ai.market/application logs, audit, evidence, UI, or allAI;
- runtime verification rejects a control-key policy missing the exact Fargate `GenerateDataKeyWithoutPlaintext`, constrained `CreateGrant`/`Decrypt`, cluster account/name encryption context, or operator `DescribeKey` statements, and rejects broader grant operations or another cluster/account;
...
- property-based transitions prove phase deadlines never exceed the absolute deadline, no nonterminal state survives it, and commit-versus-cancel/expiry races during validation have one transactional winner;
...
- fixed CPU/memory/storage and no override, price-table staleness, exact estimate/range recomputation from `price_table_version`, estimate-model version, currency, all-attempt Fargate billable units, bounded S3/KMS operation counts, CloudWatch log-ingestion bytes, retry maximum, usage reconciliation, and tag completeness; two fixtures with different ancillary request/log counts must reconcile without any undefined input;
- estimate/acknowledgement tests cover exact selector/runtime/region/quota/allocation/retry/version/currency binding, 15-minute expiry, single use, actor/time/event persistence, replay, stale table, selector change, and launch denial without a current exact receipt;
