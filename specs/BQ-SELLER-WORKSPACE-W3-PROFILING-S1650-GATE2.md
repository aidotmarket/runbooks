...
The sole permitted W2 safety-test edit is `tests/test_s1647_seller_workspace_b2b.py::test_workspace_verifier_uses_exact_prefix_policy_and_900_second_session`. It must assert session duration `900`, purpose `workspace-verify`, and the exact single-statement list-only policy from section 5: `Version=2012-10-17`; `Sid=ListExactVerificationPrefix`; `Effect=Allow`; `Action=s3:ListBucket`; the exact fixture bucket ARN; and `StringLike s3:prefix=[normalized_prefix, normalized_prefix + "*"]`. It must assert the exact `ListObjectsV2(Bucket=..., Prefix=normalized_prefix, MaxKeys=1)` call and a returned key inside that prefix, assert that no statement/action grants `s3:GetObject`, and fail if any `head_object` call occurs. Every unrelated b2b test and byte remains unchanged; b1, b2a, and the W2 PostgreSQL test remain byte-for-byte unchanged. All four W2 safety files must pass in the focused command below.
