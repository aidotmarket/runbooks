...
- `tests/test_s1647_seller_workspace_b1.py`, `tests/test_s1647_seller_workspace_b2a.py`, `tests/test_s1647_seller_workspace_b2b.py`, and `tests/test_s1647_seller_workspace_postgres.py` are the W2 safety suite. The b1, b2a, and PostgreSQL files remain byte-for-byte unchanged; b2b changes only `test_workspace_verifier_uses_exact_prefix_policy_and_900_second_session` to bind the mandatory list-only verifier behavior and only the AWS profile-stage expectation in `test_capabilities_claim_only_completed_aws_connect_and_keep_r2_w3_w5_unavailable` to bind the implemented-but-dependencies-unready behavior. Every other byte and assertion remains unchanged.
...
| `app/models/seller_workspace.py` | Add the W3 enums and ORM models in section 4. Use a migration-compatible projection and write strategy so legacy W2 reads and writes against the actual pre-W3 W2/`s1647` fixture neither select nor insert the absent `SellerWorkspaceAuditEvent.profile_job_id` column; bare `deferred()` is insufficient if an `INSERT` still emits that column. W3 code after the W3 migration must explicitly load and use it where required. Do not change `DeliveryAuthority`, `DeliveryAuthorityKind`, `legacy_serial_id`, or existing W2 constraints. |
...
| `tests/test_s1647_seller_workspace_b2b.py` | Change only `test_workspace_verifier_uses_exact_prefix_policy_and_900_second_session` to assert the exact list-only session policy and `ListObjectsV2` call and to assert absence of any `s3:GetObject` statement or `head_object` call; and change only the AWS profile-stage expectation in `test_capabilities_claim_only_completed_aws_connect_and_keep_r2_w3_w5_unavailable` to `enabled=false`, `status=unavailable`, `reason=dependencies_unready` when all flags are true but reviewed W3 dependencies are absent. Every other byte and assertion in both tests and every unrelated test and byte in this file remain unchanged. |
...
- `infra/seller_workspace_profile/verifier/handler.py` for closed verifier-envelope validation, complete read-only stack identity and network, endpoint, route, DNS, and Resolver inventories, exact five-key resolver hash, and hash-only receipt;
...
- `seller_workspace_profile_runtime/Dockerfile` for the exact digest-pinned multi-stage Chainguard Python 3.14.7 builder and final runtime, hash-locked production dependencies, pinned-builder runtime tests, non-root/read-only/no-shell execution, and fixed entrypoint;
...
The narrow post-residue implementation correction boundary contains exactly two existing entries in the unchanged 55-path manifest: `seller_workspace_profile_runtime/Dockerfile` for the already-approved round-10 `runtime-tests` package-root layout correction, and `tests/test_s1650_profile_iac.py` for exactly one additional implementation-line correction. In `tests/test_s1650_profile_iac.py`, replace only the literal `vulnerability-report.json` assertion with `seller-workspace-profile-runtime-grype.json`; keep its existing `sbom.spdx.json` assertion and every other line and assertion unchanged. This authorization adds no repository path and does not change the 55-path manifest. The workflow already uses the approved exact artifact filename and remains unchanged.

`requirements.txt`, `requirements-dev.txt`, W2 safety tests `test_s1647_seller_workspace_b1.py`, `test_s1647_seller_workspace_b2a.py`, and `test_s1647_seller_workspace_postgres.py`, `railway.worker.json`, and every delivery/serial/`legacy_serial` file remain byte-for-byte unchanged and therefore are not manifest change paths. W2 safety test `test_s1647_seller_workspace_b2b.py` is a manifest change path solely for the named verifier-policy test and the named capability test's AWS profile-stage expectation above; every other byte and assertion in those tests and every unrelated b2b test remain byte-for-byte unchanged. The dedicated `requirements-cfn-lint.lock` is tooling-only and is never installed into the backend/web/control or runtime environments. No W3 backend/web/control module, schema, route, task, service, or test may import, invoke, dynamically load, or rely on PyArrow or the parser runtime, and no new backend dependency is added for W3. The unchanged backend `requirements.txt` at original base `229689c821022b2fe8f8901f7ca32eca6f685fcf` already contains `datasets>=2.16.0` and `pandas>=2.0.0`; the mandated backend `.venv` therefore contains the pre-existing transitive `pyarrow==23.0.1` installation required by `datasets`. That installation is baseline-only and is neither used nor claimed by W3. The separately locked runtime remains the only W3 PyArrow/parser execution surface.
...
3. Two private `/26` task subnets in distinct available AZs, one from each VPC CIDR, with isolated route tables. There is no default route, NAT gateway, internet gateway, egress-only gateway, VPN, Direct Connect, or IPv6 association. Exact CloudFormation resource-set closure prohibits every connectivity resource outside items 1-13, explicitly including any VPC peering connection and any Transit Gateway resource or attachment; this closed-stack prohibition is proved from the template resource set and does not authorize peering or Transit Gateway verifier APIs.
...
`SellerWorkspaceAuditEvent` gains nullable `profile_job_id` plus `(profile_job_id, seller_id) -> (ProfileJob.id, ProfileJob.seller_id)` and an index. Its ORM mapping must use a migration-compatible projection and write strategy: legacy W2 reads and writes operating against the actual pre-W3 W2/`s1647` fixture must neither select nor insert the absent W3 column. Bare `deferred()` is insufficient if an `INSERT` still emits `profile_job_id`. Every W3 repository/query after the W3 migration that requires the binding must explicitly load and use it. No W2 column is rewritten.
...
The W2 correction in `BotoAWSConnectionVerifier._verify_sync` removes the entire `ReadExactVerificationPrefix` statement and `head_object`. Its session policy has `Version=2012-10-17` and exactly one list-only statement: `Sid=ListExactVerificationPrefix`; `Effect=Allow`; `Action=s3:ListBucket`; the exact verified bucket ARN; and `StringLike s3:prefix=[bounded_prefix, bounded_prefix + "*"]`. Proof is a `ListObjectsV2(MaxKeys=1)` response with a returned key inside the normalized non-root prefix. Version discovery separately requests only `s3:ListBucketVersions`. The W3 AWS adapter never owns an S3 object client method.
...
The runtime Dockerfile is a multi-stage build with exactly these two image references: builder `cgr.dev/chainguard/python@sha256:3472bbd8c7a7fe9254dcccdbd8eefb8edf603d0882aa4ed527f8497957b10f5f` and final runtime `cgr.dev/chainguard/python@sha256:27c8d0e215a516e6d366549d1673baf6c42784277afcc8d904715a73982b9ba4`. Both digests identify Python 3.14.7 multi-architecture indexes containing `linux/amd64` and `linux/arm64`; neither reference may be replaced by a tag, platform-manifest digest, floating alias, build argument, or alternate base. The builder installs the existing byte-for-byte unchanged `seller_workspace_profile_runtime/requirements.lock` with `python -m pip install --require-hashes --no-cache-dir --target /opt/w3/deps -r /opt/w3/requirements.lock`. A `runtime-tests` stage derived from that exact builder installs the existing byte-for-byte unchanged `requirements-test.lock` with `--require-hashes` into `/opt/w3/test-deps`, copies the production `profile_task` tree to `/opt/w3/seller_workspace_profile_runtime/profile_task` and all `seller_workspace_profile_runtime/tests` to `/opt/w3/tests`, sets `PYTHONPATH=/opt/w3:/opt/w3/deps:/opt/w3/test-deps`, and runs `python -m pytest -q tests`. The final stage copies only `/opt/w3/deps` from the builder and `profile_task` into `/opt/w3/profile_task`, sets `PYTHONPATH=/opt/w3:/opt/w3/deps`, and fixes `ENTRYPOINT ["python", "-m", "profile_task.main"]`. It runs as UID/GID `65532:65532`, contains no shell, and retains the task's non-root, read-only-root-filesystem, no-capability, no-new-privileges, no-host-namespace, and no-device execution boundary. ECS supplies no command or entrypoint override.

Vulcan independently proved this one-line Dockerfile layout correction against auto-preserved backend commit `89c3ca9e076734a2f2d878c5d4ad9238017bdf17` with the mandatory no-cache `linux/amd64` `runtime-tests` build: 26 passed, exit 0. This is supporting buildability evidence only; it does not authorize any additional change or implementation action.

The runtime production lock remains unchanged and pins `boto3==1.42.91`, `botocore==1.42.91`, `pydantic==2.13.2`, and `pyarrow==23.0.1` with platform wheel hashes; CSV and JSON use only the Python standard library. No pandas, DuckDB, fsspec, URL filesystem, plugin, or parser extension is installed. Verified pre-authoring probes recorded with Events `a7fa4fea-d481-4a9c-b40d-482e70f0366f` and `4ed4f638-7d46-4857-882b-343a4a7713a2` establish that the unchanged production lock installs on both `linux/amd64` and `linux/arm64`; `boto3 1.42.91`, `pyarrow 23.0.1`, and `pydantic 2.13.2` import with those exact versions; all 26 runtime tests pass under Python 3.14.7; and fully built `linux/amd64` and `linux/arm64` probe images each scan with zero HIGH and zero CRITICAL findings. Those probes are buildability evidence only, not Gate 2 approval or release evidence. The immutable production image and Gate 3 artifact remain `linux/amd64` because the ECS task definition's `CpuArchitecture` is exactly `X86_64`; the `linux/arm64` probe does not broaden the release platform.
...
`20260902_001_s1650_seller_workspace_w3.py` creates the nine tables and one nullable audit FK/index in dependency order. It sets `lock_timeout='5s'`, creates no W3 rows, updates no W2 row, and does not inspect or touch listing/order/delivery/serial data. Upgrade is tested on empty and realistic W2-populated PostgreSQL. The unchanged `tests/test_s1647_seller_workspace_postgres.py` must pass against its actual pre-W3 W2/`s1647` fixture with legacy W2 reads and writes neither selecting nor inserting `profile_job_id`; bare `deferred()` does not satisfy this requirement if an `INSERT` still emits the absent column. W3 PostgreSQL tests after the W3 migration must explicitly load and use the new column and prove its index and same-seller composite foreign key.
...
`capability_payload` reports profile `enabled=false`, `status=unavailable`, `reason=not_implemented` until the full reviewed implementation is present and `AWS_PROFILE_IMPLEMENTED=True`. Once that implementation marker is true, the profile stage is implemented but remains `enabled=false`, `status=unavailable`, `reason=dependencies_unready` whenever all flags are explicitly true but any reviewed W3 dependency is absent or unready; it reports profile available only if master/connect/profile flags are explicitly true, the W2 and W3 implementation markers are true, principal/KMS configuration is valid, the dedicated worker heartbeat is fresh, price table is current, and the exact connection/runtime is verified. Every AWS publish/delivery stage and every R2 stage remains `enabled=false`, `status=unavailable`, `reason=not_implemented`, including when all flags are true. A table, route, stack, image, environment typo, or partial deployment never creates capability truth. This Gate 2 work does not enable any flag.
...
- `tests/test_s1650_profile_postgres.py` — real PostgreSQL migration, constraints, composite ownership, explicit post-migration loading/use of `SellerWorkspaceAuditEvent.profile_job_id`, the new column/index/FK, exact append-only audit operations/safe snapshots, empty/nonempty downgrade;
...
The only round-11 change authorized inside the existing `tests/test_s1650_profile_iac.py` manifest path is the one-string assertion correction from `vulnerability-report.json` to `seller-workspace-profile-runtime-grype.json`. Its existing `sbom.spdx.json` assertion and all other tests, assertions, and bytes remain unchanged.

Exactly two W2 safety-test edits are permitted in `tests/test_s1647_seller_workspace_b2b.py`. First, `test_workspace_verifier_uses_exact_prefix_policy_and_900_second_session` must assert session duration `900`, purpose `workspace-verify`, and the exact single-statement list-only policy from section 5: `Version=2012-10-17`; `Sid=ListExactVerificationPrefix`; `Effect=Allow`; `Action=s3:ListBucket`; the exact fixture bucket ARN; and `StringLike s3:prefix=[normalized_prefix, normalized_prefix + "*"]`. It must assert the exact `ListObjectsV2(Bucket=..., Prefix=normalized_prefix, MaxKeys=1)` call and a returned key inside that prefix, assert that no statement/action grants `s3:GetObject`, and fail if any `head_object` call occurs. Second, `test_capabilities_claim_only_completed_aws_connect_and_keep_r2_w3_w5_unavailable` may change only its AWS profile-stage expectation so that, when all flags are true but reviewed W3 dependencies are absent, profile remains `enabled=false`, `status=unavailable`, `reason=dependencies_unready`; every other byte and assertion in that test is frozen, including `enabled=false`, `status=unavailable`, `reason=not_implemented` for every AWS publish/delivery and R2 stage. Every unrelated b2b test and byte remains unchanged; b1, b2a, and the W2 PostgreSQL test remain byte-for-byte unchanged. All four W2 safety files must pass in the focused command below.

Hostile credential/token canaries may be assembled at test runtime from non-triggering source fragments, but the repository secret scan must pass without any bypass, rule disablement, exclusion, baseline or allowlist expansion, severity reduction, or equivalent weakening. Safe symbolic source names are required instead of blocked secret-pattern identifiers whenever they provide the same test coverage.
...
Backend/control focused commands run under Python 3.11 exactly as before, from the mandated Python 3.11 backend `.venv` with its unchanged baseline dependencies. The runtime-image Python change does not apply to these commands:
...
Runtime parser tests run only in the Dockerfile's `runtime-tests` stage derived from the exact digest-pinned Python 3.14.7 builder, never in the repository's backend environment or a host Python environment. Building that target installs the unchanged production lock into `/opt/w3/deps`, installs the unchanged test lock into `/opt/w3/test-deps`, and executes all files under `seller_workspace_profile_runtime/tests` with the exact command `python -m pytest -q tests`. The final candidate is then built once for `linux/amd64`, loaded only for local verification, and bound to the lowercase `sha256:` manifest digest emitted by Buildx metadata; a tag is only a temporary local build handle and is never pushed or accepted as artifact identity:

```text
S1650_RUNTIME_IMAGE="seller-workspace-profile-runtime:${GITHUB_SHA}"
S1650_RUNTIME_METADATA="${RUNNER_TEMP}/seller-workspace-profile-runtime-build.json"
docker buildx build --no-cache --platform linux/amd64 --target runtime-tests --progress plain -f seller_workspace_profile_runtime/Dockerfile seller_workspace_profile_runtime
docker buildx build --no-cache --platform linux/amd64 --load --metadata-file "$S1650_RUNTIME_METADATA" --tag "$S1650_RUNTIME_IMAGE" -f seller_workspace_profile_runtime/Dockerfile seller_workspace_profile_runtime
S1650_RUNTIME_DIGEST="$(jq -er '."containerimage.digest" | select(test("^sha256:[0-9a-f]{64}$"))' "$S1650_RUNTIME_METADATA")"
S1650_RUNTIME_IMAGE_ID="$(docker image inspect --format '{{.Id}}' "$S1650_RUNTIME_IMAGE")"
```

The five frozen runtime test modules import `seller_workspace_profile_runtime.profile_task`; with the production tree copied to `/opt/w3/seller_workspace_profile_runtime/profile_task`, the exact pinned runtime pytest command runs with `/opt/w3` as its working directory and only `/opt/w3`, `/opt/w3/deps`, and `/opt/w3/test-deps` on `PYTHONPATH`, so those imports resolve without adding an `__init__.py`, import-helper/configuration file, or any extra repository path to the 55-path manifest. It must exit cleanly only after all 26 tests pass and every multiprocessing child has terminated; a lingering child or parent-process hang fails Gate 3.

Gate 3 runs the following proofs against `S1650_RUNTIME_IMAGE_ID`, not against the temporary tag or either base image. The dependency command must import each production dependency from the final image and print exactly the locked versions. The unchanged bootstrap behavior with no request environment is safe exit code `50`; it must remain `50` under network-none, read-only-root, all-capabilities-dropped, no-new-privileges, and UID/GID `65532:65532`. The image must also have no shell. The only accepted scanner is the multi-architecture immutable image `anchore/grype@sha256:8a93fc48da96bd6ec5981279d099b69de11541dc68fdf222fb9161f8ff284af7`: Grype `0.118.0`, GitCommit `756eb9a24f7beeafb6871a24e943e8a3ae210695`, supported DB schema `6`. The workflow invokes that exact digest through Docker with only the Docker socket and evidence output directory mounted, scans the exact `S1650_RUNTIME_IMAGE_ID`, retains the exact `--fail-on high` threshold, writes JSON into the mounted evidence directory, and separately asserts that the report contains zero `High` and zero `Critical` matches:

```text
docker run --rm --entrypoint python "$S1650_RUNTIME_IMAGE_ID" -c 'import boto3, botocore, pyarrow, pydantic; assert (boto3.__version__, botocore.__version__, pyarrow.__version__, pydantic.__version__) == ("1.42.91", "1.42.91", "23.0.1", "2.13.2"); print(boto3.__version__, botocore.__version__, pyarrow.__version__, pydantic.__version__)'
set +e
docker run --rm --network none --read-only --cap-drop ALL --security-opt no-new-privileges:true --user 65532:65532 "$S1650_RUNTIME_IMAGE_ID"
...
docker run --rm \
  ...
  "$S1650_GRYPE_IMAGE" \
  "$S1650_RUNTIME_IMAGE_ID" --fail-on high --output json --file /evidence/seller-workspace-profile-runtime-grype.json
