...
| `app/models/seller_workspace.py` | Add the W3 enums and ORM models in section 4. Map `SellerWorkspaceAuditEvent.profile_job_id` as deferred or use an equivalently safe explicit-projection strategy so legacy W2 repository queries against the pre-W3 `s1648` schema never eagerly select the absent column; W3 code must explicitly load/use it where required. Do not change `DeliveryAuthority`, `DeliveryAuthorityKind`, `legacy_serial_id`, or existing W2 constraints. |
...
`SellerWorkspaceAuditEvent` gains nullable `profile_job_id` plus `(profile_job_id, seller_id) -> (ProfileJob.id, ProfileJob.seller_id)` and an index. Its ORM mapping must be deferred or use an equivalently safe explicit-projection/query strategy: W2 repositories operating against the pre-W3 `s1648` schema must not eagerly select the absent W3 column, while every W3 repository/query that requires the binding must explicitly load and use it. No W2 column is rewritten.
...
`20260902_001_s1650_seller_workspace_w3.py` creates the nine tables and one nullable audit FK/index in dependency order. It sets `lock_timeout='5s'`, creates no W3 rows, updates no W2 row, and does not inspect or touch listing/order/delivery/serial data. Upgrade is tested on empty and realistic W2-populated PostgreSQL. The unchanged W2 PostgreSQL test must pass against its pre-W3 `s1648` fixture without eager selection of `profile_job_id`; W3 PostgreSQL tests after the W3 migration must explicitly load/use the new column and prove its index and same-seller composite foreign key.
