...
grype "$S1650_RUNTIME_IMAGE_ID" --fail-on high --output json > "${RUNNER_TEMP}/seller-workspace-profile-runtime-grype.json"
jq -e '[.matches[] | select(.vulnerability.severity == "High" or .vulnerability.severity == "Critical")] | length == 0' "${RUNNER_TEMP}/seller-workspace-profile-runtime-grype.json"
```

The image gate permits no ignore list, vulnerability baseline, Grype database pin, `--only-fixed` option, scan-scope reduction, severity reduction, threshold weakening, or equivalent waiver. A missing/stale scan database, pull failure, malformed report, absent architecture or manifest digest, dependency/version mismatch, unexpected bootstrap exit, shell presence, or nonzero HIGH/CRITICAL count fails closed. The Grype JSON report and immutable production manifest digest are emitted as Gate 3 evidence; neither the verified pre-authoring probes nor an `arm64` build substitutes for this fresh `linux/amd64` candidate proof.
