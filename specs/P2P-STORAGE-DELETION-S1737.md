...
| AIM relay: `app/api/v1/aim_relay.py`, `app/services/aim_relay_service.py`, its router include (`router.py` 59, 139), and every `relay_mode` / `AIMRelayService` / `AIMConnectionMode.RELAY` branch in `aim_service.py` (at least ~103-133, ~535-611, ~681-697, ~765-766, ~823-849) | Relay unused. `connection_mode` is always DIRECT. Also remove the four `AIMTraceEventType.RELAY_*` members, their metadata schemas in `aim_observability_service.py`, and the relay-only `buyer_node_id` request field (`app/schemas/aim_session.py`). `RELAY` leaves both Python enums; the Postgres enum labels stay (removing a label is not worth a type rewrite). |
...
One revision; Alembic runs it in one transaction before uvicorn starts (`scripts/run_alembic_startup.sh`). In this order:

1. `SET LOCAL lock_timeout = '5s'`.
2. `LOCK TABLE ... IN ACCESS EXCLUSIVE MODE` on every table read or altered: the nine tables below, `aim_nodes`, `aim_sessions`, `orders`, `transfer_sessions`, `fulfillment_download_tokens`.
3. Refuse (raise, naming the table and predicate) if any of: a row in `order_staging`, `transfer_session_members`, `transfer_chunk_receipts`, `transfer_part_receipts`, `order_delivery_members`, `buyer_download_meter`, `listing_sample_assets`, `seller_sample_assets`, `listing_asset_tombstones`; `aim_nodes` with `relay_mode` or `relay_registered_at IS NOT NULL`; `aim_sessions` with `connection_mode = 'relay'`; a non-null value in any shared column listed above, or `transfer_sessions.completion_state <> 'none'`.
4. Drop the `listing_asset_delete_tombstone` trigger, then `retain_listing_asset_key()`.
5. Drop tables children first: `transfer_part_receipts`, `transfer_chunk_receipts`, `transfer_session_members`, `order_delivery_members`, `buyer_download_meter`, `order_staging`, `listing_sample_assets`, `seller_sample_assets`, `listing_asset_tombstones`. No `CASCADE`.
6. Drop on live tables: `transfer_sessions` index `uq_transfer_manifest_receiving_order`, constraint `ck_transfer_completion_state`, columns `manifest_hash`, `staging_generation`, `completion_state`, `completion_lease_until`; `fulfillment_download_tokens` FK `fk_fulfillment_grant_version`, columns `manifest_hash`, `purchased_version_id`; `orders.delivery_manifest_hash`; `aim_nodes.relay_mode`, `aim_nodes.relay_registered_at`.
...
Acceptance: `git grep` over `app` and `scripts` returns no application reference to `listing_asset_store`, `ListingAssetStore`, `ListingDeliveryStore`, `manifest_fulfillment`, `ManifestFulfillment`, `delivery_transfer_plan`, `PartBuffer`, `ManifestAdmission`, `delivery_runtime`, `delivery_store_executors`, `DeliveryStoreExecutors`, `delivery_session_state`, `delivery_buyer`, `delivery_completion`, `delivery_reconciliation`, `delivery_staging`, `multi_file_versions`, `multi_file_delivery`, `ListingSampleAsset`, `SellerSampleAsset`, `ListingAssetTombstone`, `public_sample_service`, `vz_samples`, `AIMRelayService`, `aim_relay`, `relay_mode`, `relay_registered_at`, `AIMConnectionMode.RELAY`, `RELAY_`, `relay_connected`, `relay_disconnected`, `relay_frame_forwarded`, `relay_error`, `delivery_manifest_hash`, `staging_generation`, `completion_state`, `completion_lease_until`, or any removed config name. `manifest_hash` survives only on `listing_versions` and the publishing contract. The report lists any other hit with its reason.

## After deploy

A runbooks PR that must merge in the same release rewrites the procedure in `runbooks/data-delivery-p2p.md` so it never names a dropped object: step 3 loses the `relay_nodes`, `relay_sessions` and `staging_rows` lines; the rest stays until the bucket is gone. The current step 3 is run only before merge; after the migration, only the rewritten procedure is run.
