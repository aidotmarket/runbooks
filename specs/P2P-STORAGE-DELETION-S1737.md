...
2. `LOCK TABLE ... IN ACCESS EXCLUSIVE MODE`, in this fixed order: `transfer_part_receipts`, `transfer_chunk_receipts`, `transfer_session_members`, `order_delivery_members`, `buyer_download_meter`, `order_staging`, `listing_sample_assets`, `seller_sample_assets`, `listing_asset_tombstones`, `fulfillment_download_tokens`, `transfer_sessions`, `orders`, `aim_sessions`, `aim_nodes`.
...
6. Drop on live tables, each table's index, constraint or FK before its columns: `transfer_sessions` index `uq_transfer_manifest_receiving_order`, constraint `ck_transfer_completion_state`, then columns `manifest_hash`, `staging_generation`, `completion_state`, `completion_lease_until`; `fulfillment_download_tokens` FK `fk_fulfillment_grant_version`, then columns `manifest_hash`, `purchased_version_id`; `orders.delivery_manifest_hash`; `aim_nodes.relay_mode`, `aim_nodes.relay_registered_at`.
...
Acceptance, generated rather than hand-listed: the builder derives the forbidden set from (a) every module, class, function, route path, Pydantic field and string constant defined in a deleted file or deleted block, and (b) every `Settings` field left with no reference outside deleted code (at #451 head this includes the `TRANSFER_*` runtime settings such as `TRANSFER_UPLOAD_MAX_AGE_S` and `TRANSFER_FINALIZE_WAIT_S`), plus the named columns, `buyer_node_id`, the relay enum members and the lowercase relay event keys. `git grep` over `app` and `scripts` then returns no hit for any of them, except `manifest_hash` on `listing_versions` and the publishing contract. The report includes the generated list and the script that produced it; no test or tooling is added to the repo for this.

## After deploy

A runbooks PR that must merge in the same release rewrites `runbooks/delivery-ack-monitor.md` down to the surviving legacy monitor (no manifest route, no `MANIFEST_DELIVERY_REQUIRED` remedy) and the procedure in `runbooks/data-delivery-p2p.md` so it never names a dropped object: step 3 loses the `relay_nodes`, `relay_sessions` and `staging_rows` lines; the rest stays until the bucket is gone. The current step 3 is run only before merge; after the migration, only the rewritten procedure is run.
