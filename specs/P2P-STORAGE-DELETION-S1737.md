...
Release 1 is the code deletion with **no schema change**. Release 2 is one migration that drops only the nine tables below, which no release-1 or earlier code path touches while the flags are off. No live table is locked or altered, so the migration cannot contend with a live request.

Release 2 merges only after release 1 is the only running image. Check, for each of `ai-market-backend`, `ai-market-celery-worker`, `ai-market-celery-beat`:

```bash
railway deployment list -s <service> -e production --limit 5 --json | python3 -c "import json,sys;d=[x for x in json.load(sys.stdin) if x['status'] not in ('REMOVED','FAILED','CRASHED','SKIPPED')];print([(x['status'],x['meta']['commitHash'][:10]) for x in d])"
```

Pass: exactly one entry, `SUCCESS`, at the release-1 merge SHA.

## Migration (release 2)

One revision, one transaction, run by `scripts/run_alembic_startup.sh` before uvicorn.

1. `SET LOCAL lock_timeout = '5s'`.
2. Refuse (raise, naming the table) if any of the nine tables has a row.
...
Downgrade raises: every dropped table was empty. Tests: the empty state upgrades; a row in any table refuses.

Production state read by Vulcan 2026-09-22 (procedure step 3 in `runbooks/data-delivery-p2p.md`): all nine tables 0 rows; `aim_nodes` 2 rows, 0 relay; `aim_sessions` 0 rows; every shared column null. Re-run immediately before each release.
...
Acceptance, generated rather than hand-listed: the builder's script parses deleted files and blocks at the base SHA and lists each removed module (dotted path), top-level class and function (qualified by module), route (method and path) and `Settings` field; plus these qualified entities: `Order.delivery_manifest_hash`, `TransferSession.manifest_hash`, `.staging_generation`, `.completion_state`, `.completion_lease_until`, `.order_delivery_manifest_hash`, `FulfillmentDownloadToken.manifest_hash`, `.purchased_version_id`, `AIMNode.relay_mode`, `.relay_registered_at`, `AIMConnectionMode.RELAY`, `AIMTraceEventType.RELAY_*`, the four lowercase relay event keys, and the `buyer_node_id` field of the AIM session schema. Pass/fail at the candidate SHA: no import of a removed module, no reference to a removed qualified name, no removed route in `app.routes`, no removed field on `Settings` or on the named models (checked by importing them), and word-boundary `git grep` for names unique to the removed code (`delivery_manifest_hash`, `staging_generation`, `completion_lease_until`, `order_delivery_manifest_hash`, `relay_registered_at`, `buyer_node_id`, the relay event keys) returns nothing in `app` or `scripts`. The one allowlisted survivor is `listing_license_snapshot_gate`, moved from `public_sample_service.py` to `public.py`. The report includes the script and its output; nothing is added to the repo.

## After deploy

Release 1 (code). Gate: a runbooks PR that rewrites `runbooks/delivery-ack-monitor.md` down to the surviving legacy monitor (no manifest route, no `MANIFEST_DELIVERY_REQUIRED` remedy) merges with it. Then `/health` green and the full procedure in `runbooks/data-delivery-p2p.md`.

Release 2 (tables). Gate: a runbooks PR that replaces `staging_rows` in procedure step 3 with nothing merges with it. Then `/health` green, `alembic_version` at the new head, the rewritten procedure.
