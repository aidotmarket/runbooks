...
**Artifact size:** This Round 4 specification is exactly 466 lines.
...
| `frontend` | `ai-market-frontend` built from `FRONTEND_RC_SHA` with `NEXT_PUBLIC_API_URL=http://host.docker.internal:18000` | `127.0.0.1:13000 -> 3000` | Loopback only; private Compose network |
...
| `browser-runner` | `mcr.microsoft.com/playwright:v1.61.0-noble@sha256:35488358a60d3e7eacc4f02e0985e3b46240424d6d7b229732332016e6127508` | no host port | Private Compose bridge; journey and fixture mounted read-only; host-published services reached only through `host.docker.internal` |
...
The backend uses only the new PostgreSQL and Redis services. No Railway URL, production database dump, shared Docker volume, external Docker network, or production cache is allowed. Container host aliases for `api.ai.market` and Railway database hosts resolve to loopback as a second guard. Browser navigation is limited to `http://host.docker.internal:13000`, `http://host.docker.internal:18081`, and Stripe-hosted test pages. The frontend must bake `NEXT_PUBLIC_API_URL=http://host.docker.internal:18000` because its browser-side Axios client uses that value as `baseURL` (`ai-market-frontend:api/client.ts:5-10`). The signed VZ publish response nevertheless contains a known, non-configurable `marketplace_url` in the hard-coded `https://ai.market/listings/<id>` shape, currently interpolated from `listing.slug` (`ai-market-backend:app/routers/vz_publish.py:180-188`); the runner records that string as evidence, must never navigate to it, and instead builds the local test-frontend listing URL from the returned `listing_id` for AC5.

Host networking is not the selected topology. On 2026-09-03, Titan-1's Docker Desktop 4.87.0 advertised the `host` driver, but a disposable `--network host` container could not reach a host listener bound to `127.0.0.1`; the same container reached it through `host.docker.internal`. The `browser-runner` therefore stays on the private Compose bridge, has no published port, and uses Docker Desktop's `host.docker.internal` gateway for the three loopback-published services. `tests/compose-contract.sh` rejects `network_mode: host`, requires the private network and read-only mounts, and checks the exact browser-visible origins below.
...
FRONTEND_URL=http://host.docker.internal:13000
CORS_ORIGINS_EXTRA=http://host.docker.internal:13000
...
AIM_DATA_PAYMENT_SETUP_URL=http://host.docker.internal:13000/dashboard/data-verification/payment-method
VECTORAIZ_PAYMENT_SETUP_URL=http://host.docker.internal:13000/dashboard/data-verification/payment-method
```
...
9. `FRONTEND_URL` is exactly `http://host.docker.internal:13000` and is present in the effective CORS allowlist under the new S1656-only test exception.
...
The new Settings assertions are scoped only to the `ENVIRONMENT == "test"` branch of the existing validator. They do not run as unconditional shared assertions and cannot refuse a production boot with `ENVIRONMENT` in `prod`/`production`, canonical `FRONTEND_URL=https://ai.market`, or live-name variables set; the current validator already separates test-localhost and production handling (`ai-market-backend:app/core/config.py:839-900`). The exact fail-closed tests cover each of the four live-name variables, including `STRIPE_WEBHOOK_SECRET_PREVIOUS`, and prove production-shaped Settings remain accepted by this new test-only branch. They also prove the new HTTP allowance accepts only the exact `host.docker.internal:13000` S1656 origin and refuses every other non-local host or port. Account retrieval belongs in the environment repository's `bin/preflight`, after the pure checks and before migrations, seed, or health-ready. The application retains its current runtime account checks. No new bypass or safety setting is authorized.
...
The operator signs into AIM Data as seller-01 through `http://host.docker.internal:18081`. AIM Data performs the real `/api/v1/vz/register` flow, prepares the synthetic dataset, and sends the real signed `/api/v1/vz/publish`. The active seller gate must pass from seeded facts, not from the provisioning create-only exception. The returned listing is stored in the test Postgres. The journey takes the returned `listing_id`, builds `http://host.docker.internal:13000/listings/<listing_id>`, and renders that local route while its browser-side calls use `http://host.docker.internal:18000`. It records the response's known hard-coded production `marketplace_url` as a non-configurable string and must not follow or navigate to it.
...
4. The server returns `setup_required`; only then does AIM Data show the exact `http://host.docker.internal:13000/dashboard/data-verification/payment-method` handoff to the local test frontend.
...
**AC1 — Clean-clone boot.** From a clean clone of `aidotmarket/money-path-test-environment` on Titan-1, with only Docker and an authorized Infisical machine identity already present, `./bin/up` is the only command. It validates exact product SHAs and the committed package/lock/config/browser manifests, runs `./bin/preflight --check-browser-runner`, builds all three product images, starts the digest-pinned Playwright 1.61.0 / Chromium 149.0.7827.55 revision 1228 runner on the private bridge with its journey mounted read-only, starts PostgreSQL 16 and Redis, migrates, seeds, starts the webhook tunnel, and waits for healthy backend, frontend, AIM Data, database, cache, and browser-runner readiness. Preflight from inside the pinned runner proves `host.docker.internal` resolves and the real frontend at port 13000, backend at 18000, and AIM Data at 18081 are reachable; it refuses host-network mode or any mismatched `FRONTEND_URL`, `CORS_ORIGINS_EXTRA`, `NEXT_PUBLIC_API_URL`, or handoff origin. No host Node, global Playwright, global browser, manual file edit, or second terminal is required.
...
**AC5 — Register, publish, and render.** Starting from the real AIM Data UI at `http://host.docker.internal:18081`, seller-01 registers its install and publishes exactly one synthetic listing through the signed VZ route. The returned `listing_id` exists in test Postgres; the runner builds `http://host.docker.internal:13000/listings/<listing_id>` and renders it in normal Chrome with API calls to `http://host.docker.internal:18000`. Navigation logs prove the hard-coded production `marketplace_url` returned by VZ publish was recorded as a known non-configurable string but never followed. Production listing/search reads show no S1656 marker.

**AC6 — Just-in-time setup boundary.** Browser and component evidence proves no card control in Settings, no card prompt before quote acceptance and both acknowledgements, and no handoff before explicit paid start. That start returns `setup_required` and then, and only then, exposes the exact `http://host.docker.internal:13000/dashboard/data-verification/payment-method` handoff; the browser completes it against the real frontend/API origins without interception or mocks.
...
**Boundary:** The self-contained exact Playwright/Chromium runner manifests, read-only journey/fixture mounts, private bridge execution through the exact `host.docker.internal:13000`, `:18000`, and `:18081` origins, exactly three synthetic accounts from the committed pool names, guarded reset/reseed, test Connect Express truth for seller-01 only, one-command GA role execution, real register/publish with the production `marketplace_url` recorded but never navigated, local listing render built from `listing_id`, Stripe-hosted setup, signed webhook/dedupe, readiness, scan, manual capture, evidence redaction, and production no-effect checks. `versions.env`, `compose.yaml`, `bin/preflight`, `bin/up`, and `tests/compose-contract.sh` may change only to complete the already-reserved runner image, service, topology/reachability check, launch, and structural assertions; the package lock may contain only the exact section 3.3 dependency graph. No fixture mutation, ad-hoc SQL, `network_mode: host`, external host runner, intercepted route, or mock may satisfy an external acceptance criterion.
