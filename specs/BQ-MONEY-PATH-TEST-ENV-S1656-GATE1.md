...
4. **§8 AC8 return-first.** Implemented as a held webhook INGRESS: the evidence helper stops the path-restricted proxy for the replacement setup so Stripe's first attempt fails at the edge, the seller's return reconciles first, the ingress is released and Stripe redelivers its own signed event (`POST /v1/events/{id}/retry`), which the backend records as duplicate. No event is forged or replayed locally.
