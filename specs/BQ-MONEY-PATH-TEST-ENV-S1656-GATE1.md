...
11. **§H evidence.** `summary.json` records the paid outcome as persisted fields (epoch state, authorization and capture amounts in cents, PaymentIntent status and capture method) read from the test database and Stripe test mode, not only as pass/fail booleans; object identifiers remain opaque hashes.
