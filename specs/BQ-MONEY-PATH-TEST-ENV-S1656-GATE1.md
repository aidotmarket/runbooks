...
**Artifact size:** This Round 3 specification is exactly 460 lines.
...
| `browser-runner` | `mcr.microsoft.com/playwright:v1.61.0-noble@sha256:35488358a60d3e7eacc4f02e0985e3b46240424d6d7b229732332016e6127508` | no host port | Private Compose network; committed runner manifests, browser specs, and synthetic fixture mounted read-only |
...
The hostname is a bounded DNS record in the production `ai.market` zone, even though it routes only test traffic. Creation after Gate 1 is the sole allowed production-zone mutation. `./bin/down` uses only the separate `CLOUDFLARE_S1656_TEARDOWN_API_TOKEN` management credential defined in section 5.1 to disable and remove the exact Cloudflare Tunnel published-application route and delete the exact `s1656-money-path-webhook.ai.market` DNS record. It then fails unless the Cloudflare control-plane lookup made with that credential shows neither route nor record and an authoritative DNS query shows the hostname no longer resolves. The connector-only `CLOUDFLARE_TUNNEL_TOKEN` is never used for management or proof. The redacted teardown evidence records the checked hostname, route/record absence, query time, and command exit status without exposing either token or any provider object identifier.
...
**AC5 — Register, publish, and render.** Starting from the AIM Data UI, seller-01 registers its install and publishes exactly one synthetic listing through the signed VZ route. The returned `listing_id` exists in test Postgres; the runner builds the local test-frontend listing URL from that ID and renders it in normal Chrome. Navigation logs prove the hard-coded production `marketplace_url` returned by VZ publish was recorded as a known non-configurable string but never followed. Production listing/search reads show no S1656 marker.
...
**AC11 — One-command reset and teardown.** `./bin/reset` removes only S1656 containers, networks, and volumes, then recreates, migrates, and reseeds the empty environment with exactly the three section 6 users. `./bin/down` removes only containers, networks, and disposable volumes labeled for the single S1656 Compose project, stops the tunnel, and uses only `CLOUDFLARE_S1656_TEARDOWN_API_TOKEN` to disable/remove its published-application route and delete the exact S1656 DNS record; `CLOUDFLARE_TUNNEL_TOKEN` remains connector-only. Each command checks the compose project and the exact account, zone, tunnel, route, and DNS record identifiers before deletion and refuses a missing or over-scoped credential, unresolved identifier, or non-S1656 target. Teardown is not successful until the management credential's Cloudflare control-plane queries show both the route and DNS record absent and an authoritative DNS query shows the hostname no longer resolves.
...
**Boundary:** Add the section 5.2 test-environment-scoped fail-closed validation and the section 2.3 conjunctive test-environment pay-in routing. Tests must cover each live-name variable including `STRIPE_WEBHOOK_SECRET_PREVIOUS`; `ENVIRONMENT=prod`, `ENVIRONMENT=production`, `FRONTEND_URL=https://ai.market`, and separately each non-empty `RAILWAY_ENVIRONMENT` and `PRODUCTION` deployment marker; live mode; non-synthetic and wrong-domain users; malformed/unknown/superseded attempts; mismatched bindings; connected-account events; and every non-pay-in synthetic event. Each production/deployment-marker negative test must otherwise satisfy every allow condition, must prove the marker rejects before those conditions are evaluated, and must leave the event suppressed/no-op. Preserve every production and non-pay-in suppression result. No endpoint, schema, payment, Connect, migration, or feature-default change.
...
**Boundary:** Exact source/image pinning, ports, private networks, Infisical injection, proxy/tunnel, safe lifecycle commands, health checks, and structural tests. The connector uses only `CLOUDFLARE_TUNNEL_TOKEN`; host-side `bin/down` uses only `CLOUDFLARE_S1656_TEARDOWN_API_TOKEN`, validates the literal account/zone identifiers and recorded S1656 tunnel/record identifiers before mutation, removes only that route and record, and proves both absent through the control plane plus authoritative DNS. A missing or over-scoped management credential, unresolved identifier, or unrelated target fails closed; contract tests prove no unrelated account, zone, tunnel, route, or record request reaches the transport. `versions.env`, `compose.yaml`, and `bin/preflight` reserve the exact section 3.3 runner image/digest and fail-closed check contract, but no seed or browser implementation lands until Chunk D.
