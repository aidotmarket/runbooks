...
- **Deletions (Amendment B, S1750).** Because a generation may span several messages, the server never infers a deletion from absence. A file that was in the gateway's previous inventory and is no longer found is sent in the next generation with `present: false`. That record is appended to the audit log once; it may be retransmitted until acknowledged (§7.2), and the server applies a repeated record without effect.
...
- **Resume and acknowledgement (Amendment B, S1750).** The server accepts audit entries strictly in order (`seq` = last stored + 1, `prev_hash` = last stored entry hash, §6.4) and refuses a repeated `seq`, so the gateway must know where the server stopped. `resume` and `ack` are unsigned transport control frames (§7.3). They are not audit entries and are never stored.
  ...
  - **Ack.** The server sends `{"ack": <seq>}` only after the transaction storing that entry has committed. The gateway treats an entry (for example a queued receipt) as delivered only when its `seq` is acknowledged, by `ack` or by a later `resume`. An ack never makes the gateway prune, rewrite or stop keeping a local entry; replay is always driven by `resume` against the full local log.
  - **Content refusals do not stall the chain.** Only a bad signature, a wrong `seq` or a wrong `prev_hash` refuses storage (the connection closes with `4403` and no ack). An entry that is correctly signed and chained but refused on content (an invalid body, a `description` without a confirmed request, or a type other than `receipt`, `revocation_ack`, `prepare_ack` and `error` from a revoked gateway) is stored with its body replaced by `{"withheld": "<refusal code>"}` and its real entry hash, is not applied, and is acknowledged. The withheld body is never kept.
  - **Divergence is visible.** The backend logs every `4403` refusal and `4409` close with the gateway id and alerts support at most once per gateway per hour.
...
- **Control frames (server → gateway, unsigned, not audit entries; Amendment B):** `{"nonce": <base64url, 32 bytes>}`, `{"resume": {"seq": <integer ≥ 0>, "entry_hash": <64 lowercase hex, or "" when seq is 0>}}` and `{"ack": <integer ≥ 1>}`. The gateway ignores a control frame of any other shape. Close codes: `4403` (refused by the server) and `4409` (`audit_divergence`, sent by the gateway).
...
- **Chunk A (backend):**
  ...
  - the flag-off 404s;
  - resume and ack (Amendment B): `resume` is the first frame after `hello`, with `{0, ""}` for a new gateway and the last stored `seq` and entry hash otherwise; `ack` is sent only after commit, and a forced failure before commit sends none; a repeated or skipped `seq` and a wrong `prev_hash` close with `4403` and no ack; a correctly chained entry refused on content is stored withheld, not applied and acknowledged, and the next entry is accepted; `4403` and `4409` each alert at most once per hour; the Python entry hash equals the vector;
  - deletions: a generation split across several messages deletes nothing by absence; `present: false` marks the file deleted and raises `file_missing`; the same record retransmitted changes nothing;
