# BQ-AIM-DATA-GATEWAY-S1741: Gate 2, implementation specification (R5)
...
- **Resolution (MP R2 1, MP R3 4, GLM R3 2).** The legacy resolve path refuses orders whose status is not `disputed` (`order_service.py:2049-2052`), so it is not used. A gateway problem is resolved only through `resolve_gateway_problem(problem_id, outcome, resolved_by)`, reached from one admin route, `POST /admin/gateway-problems/{problem_id}/resolve` with `{"outcome": "released | refunded | partially_refunded", "refund_cents"?, "note"?}`. The route requires the existing admin role check (as `disputes.py:327-350` does) and records `resolved_by`. In one database transaction under the order money lock it:
  ...
  - on `refunded` or `partially_refunded` (MP R4 1): records the decision and the requested amount (`refund_cents`, the order total for a full refund) on the problem and sets it `refund_pending`. The backend has no refund-creation path today: refunds are issued in Stripe and come back as `charge.refunded` events, which `refund_processing_service.admit_order_refund` admits and `apply_order_refund_effects` applies. Support therefore issues the refund in Stripe as it does now, putting the problem id in the refund's metadata. When the effects of a refund for this order are applied, that handler sets the problem `resolved` with the refunded amount and resolves the `DisputeCase` in the same transaction. The refund reasons and `gateway_problem_open` keep payout refused meanwhile. If no refund effect arrives within 72 hours, the problem raises an alert to support; an admin may then move it back to `open` with `{"outcome": "reopen"}` (for example, after a failed refund). Any other resolve call on a `refund_pending` problem is refused with `409 refund_pending`. Creating provider refunds from the backend is out of scope here.
...
7. R5: are the §18 folds wording-level and consistent with what you approved at R4, and is the refund path (§2.3) accurate to the pinned backend?
...
| MP R2 4 (fragmentation), MP R2 SIMPLER | §6.5 1,024-interval limit; §3.3 summary-only `prepare_ack`; §3.4 |
