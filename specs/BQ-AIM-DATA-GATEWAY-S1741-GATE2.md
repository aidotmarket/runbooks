...
| `file_missing` | (with `file_id`) the gateway reported the file `present: false` (§6.2); absence from one inventory message is never enough |
...
- **Deletions (Amendment B, S1750).** Because a generation may span several messages, the server never infers a deletion from absence. A file that was in the gateway's previous inventory and is no longer found is sent in the next generation with `present: false`. That record is appended to the audit log once and is resent only by the replay rule in §7.2, which never resends a stored entry; an exact duplicate entry is refused as out of order (§7.2). A later entry that repeats `present: false` for an already deleted file changes nothing.
...
- **Scope.** Every gateway → ai.market message is written here before it is sent. `hello` is the exception: it is the connection handshake (§7.2), not an audit entry. The first audit entry is `seq` 1 with `prev_hash` `""`.
- **Canonical encoding and entry hash (Amendment B, S1750).** The canonical encoding is JSON with keys sorted by code point at every level, no insignificant whitespace, UTF-8 without `\u` escaping of other non-ASCII text, and `&`, `<`, `>`, U+2028 and U+2029 escaped as `\u0026`, `\u003c`, `\u003e`, `\u2028` and `\u2029` (what Go's `encoding/json` produces). Every number in an audit entry is an integer between −2⁶³ and 2⁶³−1, written in plain decimal with no sign for zero or positive values, no leading zeros, no decimal point and no exponent; object keys are unique and strings are valid UTF-8. The server refuses any entry that breaks these rules before it checks the signature, as a malformed entry (§7.2); it checks the received text itself, because a parsed value cannot show `-0`, `1.0`, `1e2` or a duplicate key. `sig` signs the canonical encoding of the five other fields. An entry's hash is the lowercase hex SHA-256 of the canonical encoding of the whole six-field entry, `sig` included. The next entry's `prev_hash` is that value, and so is the `entry_hash` in `resume` (§7.2). A golden vector (§10) pins both for Go and Python.
...
- **Connection.** On each connection the server sends a 32-byte random nonce, written as 64 lowercase hex characters. The gateway answers with a signed `hello {gid, nonce, version, ts}`, and the server verifies it against the registered public key.
- **Gateway → server messages.** Every one is signed by the gateway key, with a monotonic `seq`. These are the audit-log entries.
- **Resume and acknowledgement (Amendment B, S1750).** The server accepts audit entries strictly in order (`seq` = last stored + 1, `prev_hash` = last stored entry hash, §6.4) and refuses a repeated `seq`, so the gateway must know where the server stopped. `resume` and `ack` are unsigned transport control frames (§7.3). They are not audit entries and are never stored.
  - **Boundary.** The server records the first verified `hello` at `seq` 0 with an empty entry hash. Audit entries start at `seq` 1.
  - **Resume.** After it verifies `hello`, the server sends `{"resume": {"seq": <last stored audit seq, 0 if none>, "entry_hash": "<its entry hash, empty if none>"}}` before anything else. The gateway resends its stored entries from `seq + 1`, in order, and sends no new entry until that replay is sent; new entries follow it in order. If the server's `seq` is ahead of the gateway's log, or `entry_hash` does not equal the gateway's own hash for that `seq`, the gateway sends nothing further, closes the connection with code `4409` and reason `audit_divergence`, logs it locally and tries again on the next connection. It never rewrites, renumbers or skips local entries.
  - **Ack.** The server sends `{"ack": <seq>}` only after the transaction storing that entry has committed. The gateway treats an entry (for example a queued receipt) as delivered only when its `seq` is acknowledged, by `ack` or by a later `resume`. An ack means the entry is stored on ai.market, not that it was applied: whether a receipt advanced an order is ai.market's decision, shown to the seller and buyer through the order's delivery state (§2), and a receipt that was not applied leaves the order undelivered and its payout held. An ack never makes the gateway prune, rewrite or stop keeping a local entry; replay is always driven by `resume` against the full local log.
  - **Refusal and withholding.** A frame that is not a well-formed six-field entry under §6.4, or whose signature, `seq` or `prev_hash` fails, is refused: nothing is stored, no ack is sent and the connection closes with `4403`. An entry that passes those checks is always stored and acknowledged, so the chain never stalls. If the server then refuses its content, it applies nothing and stores the entry with its real `seq`, `prev_hash`, signature and entry hash but with the body replaced by `{"withheld": "<reason>"}`; the original body is never kept. The reason is one of:
    - `invalid_message`: an unknown `message_type`, a body that fails the §7.3 schema, or a phase-1 or buyer-identifier violation;
    - `description_not_confirmed`: a `description` with no confirmed request;
    - `gateway_revoked`: a type other than `receipt`, `revocation_ack`, `prepare_ack` and `error` from a revoked gateway;
    - `not_applicable`: a well-formed body that names an order, file, permission or instruction the server cannot match (for example a mismatched receipt). A valid receipt for an order already delivered is applied as a no-op, not withheld, and never reopens or holds a payout.
    No valid body of any type is exactly `{"withheld": …}`, so a withheld row cannot be mistaken for a sent body. A withheld row is verified by chain continuity (the next entry's `prev_hash` equals its stored entry hash), not by recomputing its hash from the stored body; the gateway's local log keeps the original.
  - **Alerts.** The backend logs every `4403` refusal, every `4409` close and every withheld entry with the gateway id and the detailed reason, and alerts support at most once per gateway per hour for each of the three.
  - **Why unsigned is safe.** A forged or wrong `resume` or `ack` cannot forge, alter or duplicate a stored entry: entries stay self-authenticating through their signatures and the hash chain, and the server refuses anything out of order. At worst it withholds delivery, by making the gateway believe an entry arrived when it did not, or by making it resend or wait. The next connection's `resume` is authoritative and repairs a wrong ack.
- **Server → gateway messages.** Every one is a signed instruction (§3.2), except the three unsigned control frames in §7.3 (`nonce`, `resume`, `ack`), which carry no instruction.
...
- **Control frames (server → gateway, unsigned, not audit entries; Amendment B):** `{"nonce": <64 lowercase hex characters>}`, `{"resume": {"seq": <integer ≥ 0>, "entry_hash": <64 lowercase hex, or "" when seq is 0>}}` and `{"ack": <integer ≥ 1>}`. The gateway ignores a control frame of any other shape. Close codes: `4403` (refused by the server) and `4409` (`audit_divergence`, sent by the gateway).
...
- **Golden vectors.** `aim-data-gateway/contract/vectors/*.json`, one file per message type and token. Each holds a test-only key pair, the input, the exact bytes and the expected verdict. The backend holds a byte-identical copy under `tests/contract/gateway/`, and a required CI check in each repository compares the copy's SHA-256 with the other side's pinned value (the S1732 preview-contract pattern). Vectors include an audit entry with its signed bytes and entry hash, whose body has `&`, `<`, `>`, U+2028, other non-ASCII text, zero, a negative integer, 2⁵³+1 and 2⁶³−1; malformed entries that must be refused (`1e2`, `1.0`, `-0`, a leading zero, 2⁶³, a duplicate key); a `hello` with its nonce; and the `resume`, `ack` and withheld forms (Amendment B); the null-rate boundaries (249 and 250 nulls in 10,000 rows; 9,749 and 9,750; zero rows), the default display name, and interval lengths at byte 0, byte 1, the last byte, suffix and open-ended ranges.
- **Chunk A (backend):**
  ...
  - the flag-off 404s;
  - resume and ack (Amendment B): `resume` is the first frame after `hello`, with `{0, ""}` for a new gateway and the last stored `seq` and entry hash otherwise; `ack` is sent only after commit, and a forced failure before commit sends none; a repeated or skipped `seq` and a wrong `prev_hash` close with `4403` and no ack; each withheld reason stores `{"withheld": reason}` with the real entry hash, keeps no part of the original body (checked for an unconfirmed `description`), applies nothing, is acknowledged, and the next entry is accepted; a mismatched receipt is withheld as `not_applicable` and the order stays undelivered with payout held; a malformed number or a duplicate key closes with `4403`; the nonce is 64 lowercase hex; `4403`, `4409` and withheld entries each alert at most once per gateway per hour; the Python entry hash equals the vector;
  - deletions: a generation split across several messages deletes nothing by absence; `present: false` marks the file deleted and raises `file_missing`; an exact duplicate entry closes with `4403` and changes nothing; a later entry repeating `present: false` changes nothing;
...
- **Chunk B (gateway):**
  ...
  - resume and ack (Amendment B): the Go entry hash equals the vector; after `resume` the gateway replays from `seq + 1` before any new entry; an ack lost after the server committed is settled by the next `resume` with no duplicate; a crash before the gateway records an ack leads to a resend that `resume` makes unnecessary, and nothing is lost; when the server is ahead, or the hash at `seq` differs, the gateway closes with `4409`, sends nothing further and keeps its log intact; a forged `ack` for an unsent entry is corrected by the next `resume` and the entry is sent; an ack never removes a local entry; a receipt counts as delivered only after its ack;
  - a file deleted between rescans is sent once with `present: false` in the next generation, including when that generation spans several messages.
