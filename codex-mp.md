...
| F-19 | Every MP dispatch crashes with 401 `Incorrect API key provided: sk-svcac...` on `chatgpt.com/backend-api/codex`, while `codex login status` says logged in via ChatGPT and `~/.codex/auth.json` is fresh | Most likely a server-side ChatGPT-auth fault (cause not proven): local auth looked healthy, a Max re-login did not fix it, and it cleared by itself (2026-09-26 00:44 to about 01:45 CEST; jobs 3c2b42e11df7, 2faa334c2aa8, 912146f69413, 0cde37197db6; Events d6f63993, 098ef116). A smoke with the `.env` `OPENAI_API_KEY` works, but that bills the API project and the bridge deliberately never uses env keys | Bare smoke with no env key: `cd /Users/max/Projects/ai-market/ai-market-backend && echo "Reply with exactly: SMOKE_OK" \| env -u OPENAI_API_KEY codex exec --model gpt-6-sol -`; check status.openai.com | Wait and re-smoke every 15 to 30 minutes; do not switch the bridge to env keys. If it persists over 2 hours, Repair-12 (re-login). Max may allow direct code by the instances meanwhile (Event 91773f44) | CONFIRMED (S1738, S1752) |
