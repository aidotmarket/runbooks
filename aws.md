...
| Backend STS identity `ai-market-backend-sts` | SHIPPED; creds wired to `ai-market-backend` + `ai-market-celery-worker` (S740, verified) | IAM user (backend-owned). Creds in Infisical `ai-market-backend`/prod: `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY` / `AWS_REGION` / `AI_MARKET_AWS_ACCOUNT_ID`. Runtime delivery: Railway env (see §E-07) | Titan profile `aimarket` resolves identity, verified S740 | 2026-05-31 |
