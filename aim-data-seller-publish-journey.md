...
The directory route is **SHIPPED, flag-gated, default off** (UI wiring fixes merged 2026-09-18 as aim-data PR #74 `d0e7f496`, Gate 3 3/3 on `f88373e2`): ai.market backend `MULTI_FILE_DATASETS_ENABLED` and the AIM Data install's `AIM_DATA_MULTI_FILE_DATASETS_ENABLED` must both be enabled for an authorized rollout. This edition verifies source, not deployment or a completed browser purchase/delivery run. Source pins: AIM Data `d0e7f4960733da2962daa47ab9c61e3ea8afde29` (main after PR #74; the directory journey below was written against `119f643b` and re-verified against the PR #74 diff), ai-market-frontend `3f15c1566a8f9aee2c96af397ff1eaab30f34fbc`; the receiver's version refusal was checked at ai-market-backend `b959b514` (on main — the chunk E merge; main has since advanced) (`MIN_AGENT_VERSION_LOCAL = "1.24.0"`) and on the chunk D branch at `4ba3fe4f421c640d015947f4aa063ee9fa2f4cb5`, which raises it to `1.25.0` — the minimum that applies once chunk D ships.
...
**Known limitation (release decision, Max 2026-09-18):** `FileUploadModal.tsx` **Browse Folder** still submits files individually (`UploadContext.tsx`), so a folder dragged into the upload box becomes one dataset per file; the folder route is the server directory import above. A Gate 1 amendment for browser folder upload → one directory dataset is the first follow-up. **Fixed in PR #74 (were pinned gaps at `119f643b`):** the directory page now renders the shared listing preparation with `DirectoryPublishControl`; directory privacy (Step 1) is served from the stored `directory_profile.pii` via the scan routes (flag-gated; `completed` only proceeds; 409 `directory_pii_not_ready` shown as the Step 1 reason); the server-import screen handles the flag-on direct response and registers the browsed folder; `docker-compose.aim-data.yml` forwards `AIM_DATA_MULTI_FILE_DATASETS_ENABLED` (default false); the import screen names `HOST_IMPORT_DIR`; the initial browse no longer sends an empty `?path=` (which hid the Local Import section on every install). Member role/sample edits block publication until the parent dataset refresh succeeds; sample sharing is an explicit opt-in.
...
- id: E-05
  ...
  pre_conditions:
    ...
    - The install runs an image that includes aim-data PR #74 (directory publish path, import screen, compose flag) — see the Overview limitation note for the browser folder upload
  ...
  idempotency: IDEMPOTENT_WITH_KEY
  idempotency_key: "resolved root_path — re-registering the same folder reuses the dataset (directory_registration.py:252-263; tests/test_directory_registration.py:51-67); changed contents intentionally refresh members (changed sha256 updates, absent files marked removed, is_sample preserved)"
  expected_success:
...
| F-15 | `sample_store_unavailable` (HTTP 503 from the receiver's sample endpoint) | The listing-asset store refused or timed out while accepting a sample object. | Restore the storage dependency (`listing-asset-store.md`), then retry the same publication; the version stays pending, never a second listing. | E-06 | CODE-VERIFIED |
| F-16 | **Pending members — upload is not complete. Retry publishing to resume.** | Member or sample upload did not finish; the version is registered but not active. | Retry publishing on the same dataset to resume the upload; pending_members is not success. For **Retry disclosure**, retry only disclosure. | E-06 | CODE-VERIFIED |
