...
The directory route is **SHIPPED server-side, flag-gated, default off** (the UI wiring gaps listed below are fixed on `build/bq-multi-file-datasets-s1717-ui-wiring` at `5ba74395`, under Gate 3 — until that merges the seller click-through is not complete): ai.market backend `MULTI_FILE_DATASETS_ENABLED` and the AIM Data install's `AIM_DATA_MULTI_FILE_DATASETS_ENABLED` must both be enabled for an authorized rollout. This edition verifies source, not deployment or a completed browser purchase/delivery run. Source pins: AIM Data `119f643b5fd25dc8fd61557649371c9832ca33c5`, ai-market-frontend `3f15c1566a8f9aee2c96af397ff1eaab30f34fbc`; the receiver's version refusal was checked at ai-market-backend `b959b514` (on main — the chunk E merge; main has since advanced) (`MIN_AGENT_VERSION_LOCAL = "1.24.0"`) and on the chunk D branch at `4ba3fe4f421c640d015947f4aa063ee9fa2f4cb5`, which raises it to `1.25.0` — the minimum that applies once chunk D ships.
...
**Pinned UI limitations:** `FileUploadModal.tsx` offers **Browse Folder**, but `UploadContext.tsx` still submits individual files; this is not evidence of one-directory browser upload. The server directory registration endpoint above is the verified grouping path. Also, `DatasetDetail.tsx:1457-1460` returns `DirectoryMembers` before `ListingPreparation`, so the directory detail route at this pin does not expose the publish controls described in step 3. Those controls exist in `DirectoryPublishControl` and have component tests; do not report a complete seller click-through until the UI wiring is fixed and verified. The server-import UI still expects a job response although the flag-enabled endpoint returns a dataset directly. Two install-level gaps at the same pin: the stock `docker-compose.aim-data.yml` `environment:` list does not forward `AIM_DATA_MULTI_FILE_DATASETS_ENABLED`, so a customer install cannot enable the flag from `.env` until the fix ships; and the server-directory screen names `AIM_DATA_IMPORT_DIR`, which the install does not read — the mount is `${HOST_IMPORT_DIR:-./import}:/data/import:ro` (compose `:22-23`), so the folder goes into `./import` next to the compose file or the directory set as `HOST_IMPORT_DIR`. All four are fixed on the wiring branch above (Event Ledger 3d12406f); the browser folder upload stays one-dataset-per-file this release by Max's 2026-09-18 decision, with a browser-folder-upload amendment queued as the follow-up.
