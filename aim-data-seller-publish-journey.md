...
The directory route is **SHIPPED server-side, flag-gated, default off** (the UI wiring gaps listed below are fixed on `build/bq-multi-file-datasets-s1717-ui-wiring` at `5ba74395`, under Gate 3 — until that merges the seller click-through is not complete): ai.market backend `MULTI_FILE_DATASETS_ENABLED` and the AIM Data install's `AIM_DATA_MULTI_FILE_DATASETS_ENABLED` must both be enabled for an authorized rollout. This edition verifies source, not deployment or a completed browser purchase/delivery run. Source pins: AIM Data `119f643b5fd25dc8fd61557649371c9832ca33c5`, ai-market-frontend `3f15c1566a8f9aee2c96af397ff1eaab30f34fbc`; the receiver's version refusal was checked at ai-market-backend main `b959b514` (`MIN_AGENT_VERSION_LOCAL = "1.24.0"`) and on the chunk D branch at `4ba3fe4f421c640d015947f4aa063ee9fa2f4cb5`, which raises it to `1.25.0` — the minimum that applies once chunk D ships.
...
- id: E-05
  ...
  idempotency: IDEMPOTENT_WITH_KEY
  idempotency_key: "resolved root_path — re-registering the same folder reuses the dataset (directory_registration.py:248-259; tests/test_directory_registration.py:47-64); changed contents intentionally refresh members (changed sha256 updates, absent files marked removed, is_sample preserved)"
  expected_success:
