...
- **GitHub token state**: Issues:read granted+verified S1630 (see infisical-secrets.md). Checks:read NOT grantable in the PAT console (no `checks` field in the edit form) while the API 403 demands `checks=read`. RESOLVED S1631: GitHub product limitation, not org policy — fine-grained PATs cannot hold checks:read at all (only GitHub Apps; github.com/orgs/community/discussions/129512). Nothing for Max to do in GitHub. SETTLED S1631: Max approved dropping check-runs from the adapter's expected resources; shipped as koskadeux-mcp `d13b609742` (adapter fetch block, `checks:read` config line, tests; 263/263 issue-channel tests), watcher redeployed on that SHA, live board flipped to `channel ok` verified via the public state API. CI-failure signal is fully carried by actions:read (workflow runs).
...
| Board says `channel unavailable (github)` | Should not recur since d13b609742 (check-runs dropped). If seen, a remaining GitHub sub-resource is 403ing: check watcher snapshot sources.github.detail_code. |
...
## R.13 RESIDUAL R2 CLOSED + R3 TOKEN + AC-6 GAP (S1633, 2026-08-28)

- **R2 real Option A enforcement receipt SHIPPED**: `enforcement_receipt_id` is now `R2-bf67c687ea-optionA-d5d7c3b6` (koskadeux-mcp main `6a7620df1d`, PR #186; CC + GLM APPROVE_WITH_NITS on candidate `b6dbd6ce71`). Receipt, probe and MANIFEST live in-repo under `evidence/bq-ci-health-visible-at-session-open-s1511/`. The receipt drives the REAL `OptionAClaudeCodeWorker` → `run_issue_channel_triage` → `run_claude_code` → `_build_cmd` path with ONLY `claude_code_client._run_sync_process_group` substituted; `subprocess.Popen` is instrumented independently to prove zero real launches. Watcher redeployed SUCCESS on `6a7620df1d`, github source `ok`, record_only. The six-field policy digest `d5d7c3b6…` is unchanged: `enforcement_receipt_id` is a CONTROL_FIELD, outside the digest (policy.py CONTROL_FIELDS vs POLICY_FIELDS).
- **Receipt-probe pattern for future receipts**: substitute only the process-exec boundary, instrument `Popen` to measure (not assert) zero real launches, bind the probe by sha256 + exact invocation, regenerate at the deployed tip, persist a sanitized full argv (task payload → sha256 prefix). GLM will REVISE/REQUEST_CHANGES a receipt that lacks a SHA-anchored candidate commit, a runnable diff, or numbered feared-failure questions — put the receipt on a branch first.
- **R3 Cloudflare token**: dedicated read-only token minted (see infisical-secrets.md `ISSUE_CHANNEL_CLOUDFLARE_TOKEN`; no expiry per Max). NOT wired yet — `adapters/cloudflare.py:18` still reads `CLOUDFLARE_API_TOKEN`, whose vaulted value is dead (`Invalid API Token`). Rewire at rollout step 9 (Cloudflare source), and repoint the backend remediator's Cloudflare read adapter (`remediator_adapters.py:436`, needs Workers Scripts:Read — NOT on the new token) separately.
- **AC-6 GAP found by MP (task 765d1afe313d)**: the clause "open breaker renders in the channel block as a channel issue" had no production implementation (storage.py channel renderer ~:756 and ground_truth_open_items.py ~:377 render from source health + open_count only). Build dispatched on `build/bq-s1511-breaker-channel-render` (snapshot `breaker` object from `breaker_state`, channel line `channel issue: dispatch breaker open (manual reset required)`, precedence stale > breaker > unavailable > ok). Pre-flip requirement; Gate 3 review before merge. AC-6 storm tests (100 correlated → 1 intent) on `build/bq-s1511-r4-ac6-storm`.

| Symptom | Cause / fix |
| --- | --- |
| Receipt probe hangs on `cc_profile: waiting more than 5s for profile lock` | A live CC review/build holds the shared CC profile lock; the probe waits up to `KD_CC_PROFILE_LOCK_WAIT_S` (default 900). Run it in the background or wait for the CC process to finish; do not kill the CC run. |
| MP stops with "AGENTS.md references @RTK.md, but no RTK.md is present" | Titan-1 `~/.codex/AGENTS.md` used the legacy `@RTK.md` include (rtk-token-optimization.md). Inlined S1633 (backup `~/.codex/AGENTS.md.bak-s1633`); re-inline if a Codex update restores the include. |
| Cloudflare dashboard `POST /api/v4/user/tokens` returns a 403 HTML "you have been blocked" | Cloudflare WAF blocks scripted calls through the dashboard proxy even with a valid session. Drive the Create Custom Token FORM (downshift group picker: type the group name then ArrowDown+Enter; react-select level: click the control then the option). |

## R.13 RESIDUALS R2 + R3 CLOSED (S1633, 2026-08-28)

- **R2 real Option A enforcement receipt — DONE, live.** `R2-bf67c687ea-optionA-d5d7c3b6` replaces the V-5a fake-worker placeholder `V3-3a0ee923-v5a` in `config/issue_channel/policy.yaml` (koskadeux-mcp PR #186, main `6a7620df1d`). The receipt drives the REAL `OptionAClaudeCodeWorker` → `run_issue_channel_triage` → `run_claude_code` → `_build_cmd` path with the shipped policy; the only substitution is `claude_code_client._run_sync_process_group`. Proves caps 3.0/8/600.0 reach the CLI argv/timeout, fail-closed on timeout and missing cost, refusal before exec on malformed bounds and post-construction policy swap; `subprocess.Popen` instrumented independently → 0 real launches, 0 cost. Receipt + probe + MANIFEST under `evidence/bq-ci-health-visible-at-session-open-s1511/` (receipt sha256 `cd0a6bba…`). Cross-review: round 1 CC APPROVE_WITH_NITS / GLM REVISE (provenance binding, measured zero-calls, full argv, tighter predicates — all folded, regenerated at the deployed tip); round 2 CC + GLM APPROVE_WITH_NITS (responses 20260828-111545-134120 / 20260828-111540-529215). The six-field policy digest `d5d7c3b6` is unchanged (receipt id is a CONTROL field). Watcher auto-deployed SUCCESS on `6a7620df1d`; live container `policy.yaml` verified via `railway ssh`; snapshot fresh, cost 0, external_actions 0, dispatch still disabled. Receipt regeneration: `cd evidence/bq-ci-health-visible-at-session-open-s1511 && ../../venv/bin/python probe/r2_option_a_enforcement_probe.py` — it takes the CC profile lock; if a Council CC/GLM review is live it waits (set `KD_CC_PROFILE_LOCK_WAIT_S`), never kill the reviewer.
- **R3 Cloudflare least-privilege token — MINTED, wiring pending merge.** `ISSUE_CHANNEL_CLOUDFLARE_TOKEN` (see infisical-secrets.md row): Account Settings:Read + Notifications:Read + Zone:Read, no expiry (Max directive S1633). The old shared `CLOUDFLARE_API_TOKEN` is DEAD (`Invalid API Token`, code 1000). Adapter repoint built by MP on `build/bq-s1511-cloudflare-token-repoint` (head `3a086bbbe8`, base `bf67c687ea`, 263/263 issue-channel tests) — awaiting cross-review + merge; the source stays disabled until then and until step 9. Also repoint `ai-market-backend/app/services/remediator_adapters.py` (still names the dead token) in the same increment.
- Lesson recorded in Council requests: give the FULL base SHA and 3–6 numbered feared-failure questions; both reviewers flagged the omission twice.

| Symptom | Cause / fix |
| --- | --- |
| Watcher refuses at boot: `enforcement receipt is absent` / receipt id mismatch | `enforcement_receipt_id` in policy.yaml must be a non-empty accepted id; current accepted id is `R2-bf67c687ea-optionA-d5d7c3b6`. |
| Probe hangs on `waiting more than 5s for profile lock` | A Council CC/GLM review holds `cc-profile.lock`; wait for it (never kill the reviewer). |

- **Cloudflare token repoint — DONE (S1634, 2026-08-28).** Issue-channel adapter now reads `ISSUE_CHANNEL_CLOUDFLARE_TOKEN` (koskadeux-mcp PR #187, main `435021f65e`; MP branch `3a086bbbe8`; 263/263 tests on the pinned SHA; CC `response-20260828-124151-479676` + GLM `response-20260828-124155-365482` APPROVE_WITH_NITS). Watcher auto-deployed SUCCESS on `435021f65e` (still `--source github`, so the new var is not load-bearing yet). Railway service var `ISSUE_CHANNEL_CLOUDFLARE_TOKEN` set as a literal, verified active against `/user/tokens/verify`; the running container picks it up on the next deploy (set with `--skip-deploys`). Carried nits: comment on the split `"CLOUDFLARE" + "_API_TOKEN"` literal in the test; advisory: `runner.py:161` builds adapters unguarded, so an all-source deploy with a missing credential crash-loops the whole watcher — decide as its own change before step 9 flips Cloudflare on.

## R.14 R4 AC-6 LANDED — storm acceptance + loud breaker (S1634, 2026-08-28)
- **AC-6 correlated-storm acceptance tests** (koskadeux-mcp PR #188, main `9483b846e0`, tests only). Round 1 hand-built the collapsed representative and self-journaled the escalation (GLM REVISE / CC nits); round 2 drives `RecordOnlyRunner.run_once()` with a 100-envelope correlated observation and real httpx/subprocess boundary guards. Pinned production behaviour: 100 issues → 1 correlation key, 1 episode key; dry-run runner `would_dispatch_count` 100 (one per fingerprint); offline replay collapses to 1; Postgres admission of one episode 100× → 1 admitted / 99 `idempotency_duplicate`. Reviewers: nothing here blocks the flip. Watch item for the live flip: per-fingerprint dry-run evaluation of a single episode (100 journal rows for one storm) — DB idempotency is the real ≤1-intent guarantee.
- **Open dispatch breaker renders LOUD** (producer koskadeux-mcp PR #189 main `a473796750`; consumer ops-ai-market PR #29 main `786cb22`, deployed FIRST per reviewer ordering, Railway `ops-dashboard` SUCCESS). Snapshot carries `breaker {kind,status,manual_reset_required,reasons}`; board channel emits `breaker_state` closed|open|unknown + status `channel issue: dispatch breaker <status>[ (manual reset required)], N open`; precedence stale > breaker > unavailable > ok; unreadable journal → `unknown` without manual-reset, inconsistent journal → `unknown` with manual-reset; invalid status → `channel unavailable` in both renderers. Ops panel is loud for prefix `channel issue` or breaker_state open|unknown. Verified live via `api.ai.market/api/v1/allai/state/infra:open-items-board`: `channel ok, 3 open`, `breaker_state: closed` from a post-deploy watcher snapshot. Reviews: R1 CC APPROVE_WITH_NITS / GLM REVISE (board contract gap — an open breaker would have rendered muted); R2 pair GLM + CC APPROVE_WITH_NITS (responses 20260828-142705-288460 / 20260828-144200-225528).
- Carried nits: test double duplicates `render_snapshot_status` (memory_store.py); consumer tests cover two of four breaker suffix combos; breaker precedence hides a concurrent source outage in the headline (source_health still present on the board object).
- Pre-flip requirements (R2 receipt, R3 tokens, R4 storm, AC-6 breaker render) are now ALL landed. Next: step 9 Railway record-only, then Cloudflare.

## R.15 Step 9a — Railway record-only LIVE (S1634, 2026-08-28)
- **V-1 Railway row accepted** after four review rounds (package `review-packages/S1511-GATE2-RECEIPTS/`: `v1-provider-capability-matrix.md` sha `53178b01…`, `.json` `c7e9bc2e…`, probe `v1_railway_probe.py`, raw runs `v1-railway-probe-raw{,-run3}.json`, dashboard observation `v1-railway-console-observation.json`, `event-ledger-375d78ab-railway-token-acceptance.json`, `v3-credential-inventory.json` railway row ACCEPTED-WITH-RESIDUAL). Final verdicts: GLM APPROVE_WITH_NITS `response-20260828-192533-894780`, CC ACCEPT_WITH_NITS `response-20260828-200239-604414`. Earlier rounds (R1 CC/GLM REVISE, R2 CC accept / GLM REVISE, R3 CC accept / GLM REVISE) are the audit trail for why the adapter changed.
- **Credential decision:** Railway has no read-only token grade. Max accepted the write-capable environment-scoped project token residual — Event Ledger `375d78ab-3b40-4f8b-9182-b1b65181b899`, verbatim "If that is our only option on Railway than I authorize it". `RAILWAY_API_TOKEN` set as a LITERAL on the `issue-channel-watcher` service (R.11) before merge.
- **Code:** koskadeux-mcp PR #191 → main `5e1ec0cdc7`. Repeatable `--source`; watcher CMD `--source github --source railway` (Cloudflare stays off). Railway adapter: early stop at the 2-day lookback only with in-page AND cross-page newest-first ordering; untrusted/unparseable stream keeps envelopes but reports the window incomplete → `partial` (`railway_observation_incomplete`), never `ok`; native id `project_id/environment_ID/service_id/deployment_id`; default `max_pages` 5 → worst case 2 + 17×5 = 87 calls/poll = 1,044/h sustained; typical 24/poll.
- **Deployed + verified:** watcher SUCCESS on `5e1ec0cdc7`; first post-deploy snapshot 18:41Z shows `sources: github ok, railway ok`, `open_count 4` including one Railway deployment issue (`railway:ai-market/production/ai-market-backend`, deployment `0bcff6b0`, 2026-08-27 FAILED); board republished v371 `channel ok, 4 open`, `breaker_state closed`; confirmed via `api.ai.market` state API.
- **Calibration watch item (step 10):** the 2026-08-28 12:32Z FAILED deployment (`9a6caaed`, PR #304, later superseded by SUCCESS #305) is not rendered while the older `0bcff6b0` is — check the normalize/resolution rule for superseded Railway deployments before any live rule.
- **CC profile contention:** two CC dispatches died on the 900 s profile-lock wait while Vulcan's reviews ran 20+ min; coordinate via bus before CC dispatch when both instances are reviewing.

## R.16 Step 9b — Cloudflare record-only LIVE, zone-status class only (S1634, 2026-08-28)
- **V-1 Cloudflare rows accepted** after two rounds: zone-status class API; notification-history class DEFERRED (adapter `since=now−2d` window is amnesic after a >2-day outage; account has exactly ONE notification policy — the auto-created billing alert — so the class carries no signal anyway). Package files `v1_cloudflare_probe.py`, `v1-cloudflare-probe-raw.json` (retains policies read, 30-day history read, `/user/tokens/verify` id+status, write-refusal proof HTTP 403), `v1-cloudflare-console-observation.json` (dashboard: 2 active zones, 1 billing policy), `v3-credential-inventory.json` cloudflare row COMPLETE-WITH-NOTE (grants set at mint S1633, not re-read by API). Verdicts: R1 CC ACCEPT_WITH_NITS `response-20260828-210442-453045` / GLM REVISE `response-20260828-210225-463232`; R2 CC ACCEPT_WITH_NITS `response-20260828-215733-425837`, GLM APPROVE_WITH_NITS `response-20260828-215737-949656`.
- **Code:** koskadeux-mcp PR #192 → main `a3d681f08d`. CMD `--source github --source railway --source cloudflare`; `config/issue_channel/sources.yaml` `cloudflare.notification_history: false` (DEFERRED); adapter never calls `/alerting/v3/history` when false, does not expect the resource, cannot create/resolve issues (tests); 294 passed. Carried nit: adapter constructor accepts truthy non-bool (config layer validates strictly).
- **Deploy preconditions:** `CLOUDFLARE_ACCOUNT_ID` (d5346d3e…) set as a literal on `issue-channel-watcher` and added to Infisical prod; `ISSUE_CHANNEL_CLOUDFLARE_TOKEN` literal already set (R.14).
- **Deployed + verified:** watcher SUCCESS on `a3d681f08d`; snapshot 20:08Z `sources: cloudflare ok, github ok, railway ok`, cloudflare expected == observed (account, zones, zone:ai.market, zone:vectoraiz.com), open_count 4; board republished v376 `channel ok, 4 open`, breaker closed, via `api.ai.market`.
- **Rollout step 9 COMPLETE (both sources).** Open configuration item for Max before the Cloudflare email exception can be retired: create notification policies (Tunnel health for the load-bearing cloudflared tunnel, SSL/TLS expiry, Cloudflare Status incidents, DoS, Workers) — account-owner write; then implement history continuity and re-verify the history class.
- **Flip condition for `notification_history: true`:** continuity from the last successful observation (capped by the 30-day retention), never `ok` when continuity is unproven; then re-verify with a fresh V-1 row.
