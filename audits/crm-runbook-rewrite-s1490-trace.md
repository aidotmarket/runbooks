...
| Steward skill list, "28 `@skill`-decorated" | `crm-target-state.md` §3 | `crm.md` §D | **CORRECTED.** 28 registered in `CRM_SKILLS`: 9 read (health is one of the reads) and 19 write. An earlier draft of this row said 29 by counting `health` twice; corrected S1496 against the CRM_SKILLS registry. |
...
| Dead model-class inventory and the 37 legacy select sites | §C.1 | The landmine map. Distinguishes unreachable dead code from the three reachable pipeline sites. |
...
3. 37 legacy select sites remain in six live files. Dead but present.
