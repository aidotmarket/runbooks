...
3. `peer_msg_send` is not exposed as a first-class MCP tool (only `peer_msg_ack` is). Until it is (BQ-PEER-BUS-SEND-TOOL-GAP-S816): instances WITH shell access send/claim/drain via the library directly — `venv/bin/python -c "from tools.peer_messages import peer_msg_send, consume_unread_peer_messages"` (signature `peer_msg_send(from_instance, to, kind, priority, ref_entity, body)`; `ref_entity` REQUIRED — returns 500 when omitted; drain inbox via `consume_unread_peer_messages(instance)`). Chat instances without shell fall back to `decision`-event logging + relaying urgent signals via Max. Verified working S833.
