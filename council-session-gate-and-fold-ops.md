...
- Gate 1 only: dispatch `mode=author` with `caller_instance=vulcan|mars` and NO dispatch_id / dispatch_token / target_gate — the gateway auto-signs with that peer's own key (passing ANY of the three disables auto-sign and demands a real token; target_gate then defaults to 1).
...
- Gate ≥2: mint explicitly on Titan-1 — `tools.author_dispatch.signer.auto_sign_for_caller(caller_id="<vulcan|mars>", target_gate=N)`. Key configuration is caller-bound: `AUTHOR_DISPATCH_{VULCAN|MARS}_KID` plus `AUTHOR_DISPATCH_{VULCAN|MARS}_PRIVATE_KEY_PATH` (inline `_PRIVATE_KEY` is supported for tests/controlled launchers). The private key chosen by `caller_id` must match the same `(caller_id, kid)` row in `author_dispatch_public_keys`; never borrow the other peer's identity. Token TTL is 300s — dispatch immediately. Pass dispatch_id + dispatch_token + target_gate. The legacy `auto_sign_for_vulcan()` wrapper remains only as a compatibility alias.
...
- MP: hard cap is env-tunable (MP_HARD_UPPER_BOUND_S; observed 600s live in S1221/S1222 despite the 1200s default) — size build prompts to fit. STRUCTURAL builds enforce a SINGLE-COMMIT invariant: do NOT mandate per-item commits (S1222 post_build_multiple_commits failure); one commit at the end, pushed. On a hard timeout with work left in the tree, checkpoint-commit the WIP before any resume (the structural wrapper requires a clean tree pre-dispatch).
