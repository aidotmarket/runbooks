---
title: Queue-Overlay Archival Cutover (Reform WS11)
owner: unassigned
...
> path (How to operate.4). The live-traffic machinery in How to operate.2 remains documented for any future
...
### How to operate.1 Feature-flag surfaces (`config:queue-overlay-feature-flags.body`)
...
### How to operate.2 Phase-D execution — PRECONDITIONS (NOT yet met as of S992)
...
### How to operate.3 Post-Phase-D → WS9 ACL enforce tie-in
...
### How to operate.4 EXECUTED — zero-traffic archive-and-close (method A, S1117)
...
gates in How to operate.2 (B-prime writer flip ≥3d, SLO volume floor, 14d Phase-E archive wait,
...
`enforce`→`warn` FIRST (inverse of the How to operate.4 guarded UPDATE), or every restore write
...
final chunk 5 at 7a023a80) → **Phase-D cutover (PENDING, gated How to operate.2)** → Gate-4 prod verification →
WS9 ACL enforce-flip. Tracker: `config:reform-completion-tracker` (v13, S992).

## Operator guide (quick procedures)

- **"Is WS11 done?"** YES — executed + Gate-4 verified S1117 (How to operate.4). See `config:reform-completion-tracker`.
- **"Can I run Phase D now?"** Only if all four How to operate.2 preconditions hold AND you are primary. Otherwise it no-ops with `reason='ws3_dashboard_not_production_stable'`.
- **"A sub-surface looks wrong post-flip"** → Repair: check parity events, fallback to `legacy` or `initiate_rollback`; never touch the other seven.
- **"How do I re-enable after rollback?"** → Repair re-cutover: forward-replay → SLO eligible → set `recutover_approved_<ss>=True` → `execute_recutover`.
