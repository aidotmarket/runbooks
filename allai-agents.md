...
Operator surface (S1175, T-2026-000220): the ops console has an APPROVALS panel (`ops.ai.market/approvals`, `HitlApprovalsPanel.tsx`) over these endpoints, with a nav badge showing the pending count, and `kd_session_open` surfaces a fail-open `pending_approvals` field at boot so pending rows are impossible to miss. To make the console reachable, the three HITL endpoints (list/approve/deny) moved from the JWT-only superuser dep to the established `get_admin_or_internal_key` dual-auth admin dep (backend `6a9c35f7`, UNANIMOUS Council: MP+AG+DeepSeek, nits folded) — all other `ops_agents` endpoints keep the superuser dep. Approve/deny accept optional `{resolver_email}`; it is honored ONLY for the synthetic internal-key identity (the console passes the Google-verified ops-login email so `resolved_by` points at the human who clicked); a real admin JWT is always self-attributed, and an unmatched email logs a warning and falls back. Live-verified deny leg: probe row 4b0d6cb4 denied via the console call path, `resolved_by` = Max's user id.
