> **DUPLICATE-COPY WARNING (Vulcan S1342, unresolved).** There are two divergent files named `agent-dispatch.md` in this repository: this one at the repo root (634 lines, carries §V and §W) and a different one at `runbooks/agent-dispatch.md` (525 lines, stops at §K, no §V/§W). `CATALOG.json` and `TOPIC-ROUTER.md` both resolve the NESTED copy, so the runbook gate reads a document that does not contain the dispatch mechanics a human reading this file would find. Two dispatches were rejected in S1341 for a section that exists only here. Do not add content to either copy without checking the other. Consolidation belongs to BQ-RUNBOOK-CATALOG-VALIDATOR-S1229.
>
