...
## §W — Kimi Council-review deadline (T-2026-000377)

- First-class Kimi reviews default to **900 seconds**. This is intentionally separate from the legacy/non-voting shadow-sample default of 120 seconds.
- Measured basis: an identical 28,606-character scoped review timed out at 120.297s, then completed at 265.472s with `timeout_s=900`; a following review completed at 222.303s.
- A Kimi transport deadline remains a typed `error_type=timeout` / `error_code=timeout`, not a refusal or verdict. It is a failed-closed nonvote and must never be recorded as `REJECT`.
- An explicit positive `timeout_s` still overrides the 900-second Council default. Do not add an automatic retry: a retry can duplicate provider cost and work. Re-dispatch deliberately after checking task state and preserving the same pinned artifact.
