...
| Pilot allowlist | `STRIPE_PAYIN_ONBOARDING_PILOT_PARTY_IDS` | retired from code (GA amendment, backend GA merge `229689c8`); variable absent. Any seller with seller capability, an `auth_user` party and TOTP on can add a card. |
| Quote endpoint | `POST https://api.ai.market/api/v1/data-verification/quote` with `{}` | `422` field errors (enabled). A `404 "data verification is disabled"` means the flag is off. |
| Card-setup readiness | `GET https://api.ai.market/api/v1/data-verification/payment-method/readiness` signed out | `401` (enabled). Signed in and ineligible callers get an indistinguishable `404 PAYIN_ONBOARDING_DISABLED`. |
...
2. **Free probe and quote.** The seller picks the scan depth options, optionally includes the schema preview and row counts, and selects **Run free probe and request quote**. AIM Data reaches the source locally (no data leaves), then asks ai.market for a quote (`POST /data-verification/quote`, install-signed). The panel shows the source reachability, objects discovered, size class and the **maximum card hold** (2x the token cost, $1–25). Nothing is charged; a row appears in `verification_quotes`.
3. **Two acknowledgements.** Both separate checkboxes are required (understanding of the point-in-time terms and of the charge boundary). Publication is never pre-consented here.
...
5. **Card setup on ai.market.** The seller signs in (step-up re-authentication is expected), selects **Add payment method**, and completes Stripe's hosted setup. ai.market never sees card details; it stores only the Stripe customer and payment-method identifiers (`data_verification_payin_setup_attempt`). Returning lands on `/dashboard/data-verification/payment-method/return`, which reconciles readiness.
...
Who could run it today (production, 2026-09-15): active `vz_installs` rows exist for `max@kisa.cat` (install registered 2026-09-08 from the `aim-data-fresh-s1665` container on Koskadeux, `http://127.0.0.1:8080`, v1.23.2, signed in, TOTP on, no dataset yet) and for Sergey (`eolymp.com`, install 2026-06-17, one published listing, TOTP on). The daily E2E seller (`e2e-test.ai.market`, `is_test = true`, TOTP off) can reach the quote but never the card surface.
