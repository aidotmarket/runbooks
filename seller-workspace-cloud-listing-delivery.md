...
> Current production status (2026-09-11, S1712): AWS S3 and Cloudflare R2 connection, selection, allAI-assisted approval/publication and real paid browser downloads have been exercised in production. AWS profiling has now passed through the normal production application; its admission is paused after test cleanup. Test cleanup is now fully verified; joint AIM Data compatibility remains open. The W1/W2 implementation-status statements below are historical, not current availability. Use the [operator guide](runbooks/seller-workspace-operator-guide.md) for diagnosis, support, upgrades and safe continuation, and [live release operations](runbooks/seller-workspace-live-release.md) for dated evidence, limits and remaining checks. The frozen architecture and security invariants below still apply unless superseded by a later approved implementation.
...
The CC-and-GLM-approved follow-up was merged as backend `50fdb680a5b89265a7e668223bfa1b8728c6c4e9` and deployed successfully as Railway deployment `496610fe-5cb6-4887-bf38-d7c47125e7e7` (image `sha256:95a394173a528011f8bb82ae5e667c4b5b948ac3fd95ddd0b0c49eb368c6f692`). It became the sole active backend deployment and reached lifespan READY. Live `/health` then reported `healthy`, `model_schema_drift=false`, `schema_missing_table_count=0`, and Alembic current/head `s1648_issue_channel_repair`. Owner-qualified database inspection confirmed the exact six `issue_channel` base tables, `provider_num_turns`, three distinct non-superuser roles, and the intended ACL boundary: the application role has no schema usage; the queue role has SELECT/INSERT/UPDATE but not DELETE. All Seller Workspace master/provider flags, including every R2 flag, remained false. The four production W2 tables remained empty; the audit table retained both append-only triggers and contained no rows. The live delivery-authority constraints still name and bind `legacy_serial`/`legacy_serial_id`, while the 152-test Seller Workspace and legacy delivery/serial selection passed.
...
### S1648 controlled seller and provider proof

The immutable redacted receipt for the live checks below is [`receipts/s1648-seller-workspace-gate4-redacted.json`](receipts/s1648-seller-workspace-gate4-redacted.json), SHA-256 `aace889514888a85f12287ce57e8302a1c0c03bea1b4cfaa36e72fe9d3344ddc`. It contains the exact non-secret test command and result, redacted SQL predicates and row IDs/timestamps, AWS policy hashes and negative object checks, Railway identities and cleanup projections, health and flag outputs, Git ancestry commands, and timestamped redacted Chrome capture metadata. It contains no ExternalId value, ADC JSON, access key, secret key, token, customer credential, or customer data.

The earlier Google OAuth HTTP 400 was specific to the controlled environment: the `verify-s1648` callback URI was absent from the Google OAuth client. After that exact non-public callback was added, Google sign-in succeeded. This evidence makes no public OAuth callback health claim.

The normal authorized operator Chrome identity completed Google sign-in as the controlled active seller `max@ai.market` against the non-public `verify-s1648` frontend. The browser then completed the visible AWS lifecycle for connection `329ab944-d400-4e01-b65e-ce0a44139f1e`: create and owner-only authorization display, initial verification to `Verified`, reconnect/rotation start, successful rotation completion back to `Verified`, and confirmed disconnect to `Disconnected`. No in-app browser, `kdbrowser`, command-line request, API output, or alternate identity substituted for this DOM proof.

The ceremony used only the synthetic role `arn:aws:iam::948749907373:role/aim-data-gate4-test-seller`, bucket `aim-data-gate4-test-948749907373`, region `us-east-1`, and bounded prefix `gate4/test`. A 37-byte AES256 synthetic object was copied temporarily into that prefix. The role received only the temporary prefix-scoped `s3:ListBucket` permission needed for verification; its existing object-read permission was unchanged. After the lifecycle proof, the saved original trust and inline policies were restored byte-for-byte by canonical JSON hash. An exact-prefix list returned zero objects, `HeadObject` returned HTTP 404, and the unversioned bucket reported no versions or delete markers for that key. The original root `test.csv` remained unchanged at 37 bytes with AES256.

Database evidence after rotation showed connection version 7, `rotation_substate=complete`, and `last_verification_status=rotation_verified`; the superseded credential envelope had every ciphertext field cleared and was marked destroyed. After Chrome disconnect, version 8 was `revoked`, both active and pending credential pointers were null, and both credential envelopes had every ciphertext field cleared and a destruction timestamp. Eleven append-only audit rows recorded create, initial verify, rotation start, three redacted denied attempts, three authorization retrievals, successful activation, and disconnect. A full-text audit scan found zero raw `aim_sw_` ExternalIds. The denied attempts were caused by a stale operating-system clipboard during controlled trust-policy transfer; browser-visible authorization, encrypted pending credential, and corrected trust value were compared without printing or persisting the value before the successful completion.

Idempotent retry remains a separate automated contract proof rather than a browser substitute. The passing 152-test selection includes `test_unknown_initial_provider_outcome_preserves_state_key_and_success_replay` and `test_unknown_rotation_outcome_preserves_pending_state_key_and_success_replay`, which prove same-key success replay after an unknown provider outcome without a second committed transition. The browser proof establishes the customer-visible lifecycle; the tests establish exact-key replay semantics.

The controlled backend was finally redeployed as Railway deployment `f88e1364-86a4-41d0-b4f9-6b9869a263ff` at exact backend merge `50fdb680a5b89265a7e668223bfa1b8728c6c4e9`, and the controlled frontend remained successful at `c9c18e78-b2cf-400c-bb91-2c6877af32eb` / `750f26ad4160782b84b8aad3917d51ef4b6c6569`. The controlled backend returned `healthy`, Alembic current/head `s1648_issue_channel_repair`, and no schema or model drift. The temporary ADC file and every temporary ADC/KMS environment variable were removed, and the configured AWS principal was restored to `arn:aws:iam::948749907373:user/ai-market-backend-sts` before that final redeploy.

The final production reread found backend deployment `ba7f01a8-ed90-4e29-b910-0b7c4aaefa57` successful at `50fdb680a5b89265a7e668223bfa1b8728c6c4e9`, and frontend deployment `83e55f53-8558-472a-94f8-e522967eb087` successful at `750f26ad4160782b84b8aad3917d51ef4b6c6569`. The exact Gate 3 candidates remain ancestors of both deployed commits. Production `/health` remained healthy at Alembic current/head `s1648_issue_channel_repair` with no Alembic, schema, or model drift; the public frontend returned HTTP 200. All nine Seller Workspace master/AWS/R2 flags remained false, and the 152-test Seller Workspace plus legacy delivery/serial selection passed again. No public flag, capability claim, customer credential, customer data, W3, R2, AIM Data runtime, or Mars-owned data-verification/corpus path was enabled or changed.
