...
| `workspace_connection_provider_outcome_unknown` | AWS transport, timeout, TLS, DNS, or endpoint handling did not produce a definitive verification result. This is not an access denial. | Keep the connection and rotation state unchanged and retry with the same browser idempotency key. Never record the outcome as a definitive deny or expose raw SDK content. |
