...
|---|---|---|
| Cloud connection | `pending_authorization`, `expired_pending`, `verified`, `disabled`, `revoked`, `error` | Only a verified connection can select or read objects. An initial pending connection whose authorization deadline passes enters the single terminal `expired_pending` state: it cannot verify, its authorization material is unavailable, and reconnect creates a new pending connection. Disconnect enters `revoked`, blocks new use immediately, and truthfully reports any residual provider-token lifetime. A revoked record is never revived. |
...
2. While the connection is pending, an authenticated, owner-only, `Cache-Control: no-store` ceremony response supplies the configured exact ai.market AWS principal, exact trust-policy JSON shape, and that connection's generated ExternalId. The frontend renders them as copy-only, memory-only text and clears them on navigation, completion, expiry, or disconnect. They are prohibited from browser persistence, service workers, URLs, analytics, logs, traces, task payloads/results, prompts, listings, samples, support screens, and buyer grants. If the ceremony cannot be recovered safely, the seller starts a new pending connection.
3. The seller configures the role, then submits role ARN, bucket, prefix, and region to that pending connection exactly once. The backend derives the AWS account from the role ARN, does not accept a principal or ExternalId from the seller, and assumes only with the principal and generated ExternalId already bound to that connection.
...
6. If initial authorization expires first, cleanup enters terminal `expired_pending`, destroys its authorization material, and makes both authorization retrieval and verification unavailable. The seller must create a new pending connection. Disconnect similarly revokes and destroys only the selected connection's active and pending authorization material; it never rotates, revokes, or broadens another connection.
...
| `POST /connections/{id}/verify` | Owner-only submission of role ARN, bucket, prefix, and region. Derive and pin the account, verify AssumeRole with the connection's generated ExternalId and configured exact principal, then atomically mark verified and close browser retrieval. For a pending rotation, verify only the existing immutable binding and atomically activate its replacement value. |
| `POST /connections/{id}/rotate` | Start an owner-only, connection-local replacement ExternalId ceremony. Verification can activate it only against the existing immutable binding; the active value remains authoritative until then. |
...
| `workspace_connection_authorization_unavailable` | The owner-only ceremony was consumed, the connection is not pending, or it reached terminal `expired_pending`; a foreign ID is intentionally indistinguishable from absent. | Read owner-visible status and redacted audit only. An expired pending connection cannot verify; create a new pending connection. |
