...
These evidence readers are observations, not gates: they add no approval, compliance step, lock, queue, credential, deployment requirement, or human confirmation, and they do not stop builds or deployments. The stage names, DONE definition, manual branch-deletion checks, sole-writer contract, and fail-open rendering remain unchanged. Evidence still fails closed for completion: an unreadable migration baseline reveals merged refs instead of hiding them; unavailable deployment, release, or documentation evidence cannot produce `safe_to_delete_branch=true`; and source-specific gaps are reported rather than duplicated by a generic no-source warning. The script never consults Living State for membership or stage.
...
*Created: S363 (2026-04-01). Updated: S1605 (2026-08-26) — documented exact branch-identity grouping for unrelated projects without changing board authority, lifecycle rules or operator gates.*
