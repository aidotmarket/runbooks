...
| F-08 | `qdrant_index_integrity` critical with `orphan_count` above 0 while `missing_count=0` and `version_mismatch_count=0` | entities marked `deleted=true` or `semantically_indexable=false` after they were indexed keep `qdrant_point_id`; the cursor reconciler scanned only live indexable rows, so nothing enqueued them and the monitor stayed critical indefinitely (S1721: ten `infra:*` rows such as handoffs, open-items-board, session status) | `SELECT key, qdrant_point_id FROM state_entities WHERE qdrant_point_id IS NOT NULL AND (deleted = true OR semantically_indexable = false)` | Self-heals from ai-market-backend PR #426: reconciler Phase 0 (`app/tasks/reconciler.py:_orphan_entities`) enqueues each orphan once per daily 04:00 UTC run; the worker sees a non-indexable canonical row, deletes the point and clears the pointer (`_delete_owned_points`). Verify with the predicate returning zero rows after the next run. If rows remain after the first 04:00 UTC run, check `qdrant_sync_outbox` for those `target_id`s: `dead_letter` or `failed` means the worker delete failed (see F-03 / Repair G-02); no row at all means the reconciler did not run or hit its 25-orphan sub-budget, so wait one more run before acting. All ten S1721 rows stored the derived point id `uuid5(NAMESPACE_DNS, "state:"+key)`, which is the id the worker deletes. Board ticket, not a page. | CONFIRMED |
