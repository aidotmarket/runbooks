...
Malformed transfer, order or listing ids are refused with `AUTHORIZATION_MISMATCH` before any database read, with a rollback, and cancel nothing.
...
| Seller's frames refused with `AUTHORIZATION_MISMATCH` | Frame names a transfer/order/listing the device's seller does not own, or an id is not a UUID | Compare the frame ids with `transfer_sessions`/`orders` rows for that seller |
...
Tests that pin this behaviour: `tests/test_ack_monitor_authorization_s1734.py` (25 tests). `tests/test_fulfillment_listener.py` (8) and `tests/test_s1681_s3_delivery.py` (83) have local-harness failures that are identical on base `d4582fe9`; CI Gold Path is the gate.
