...
> - **Synthesis:** the orchestrating instance synthesizes, flags disagreements, presents to Max. An unresolved split fails the gate; Max may direct rework or explicitly supersede the Council under CORE §5.
> - **Hard limit:** 3 rounds.

> **Decision rules:** unanimous (3/3) for every item, and only after all three voters return valid verdicts (3/3 valid participation); missing, failed, malformed, or model-mismatched voters fail the gate closed — no builder substitution, no reduced quorum, no fallback voter, and no non-Council second opinion standing in for a voter; an unusable vote is rerun once and the gate fails if the rerun is unusable; any voter CRITICAL veto halts and escalates to Max; mandatory dissent record.
...
| GLM, DeepSeek, Gemini | Review and vote independently | `council_request mode=review` | Read-only | COMPLETE |
| CC | Explicit-name non-Council second opinion | `council_request agent=cc mode=review` | Read-only, non-voting | COMPLETE |
...
- id: E-03
  ...
  tool_or_endpoint: council_request(agent=<glm|deepseek|gemini>, mode=review, task=<audit_prompt>, cwd=<repo>, dispatch_sha=<sha>) once for each active voter, then state_request(action=bq_complete, bq_code=<code>, summary=<summary>, gate=4, evidence_links=<links>, session_id=<session>, verification=<production_evidence>); agent=cc is optional and non-Council
