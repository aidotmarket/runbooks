...
- **C2-C legacy-path deletion LANDED.** Recorded S1637, 2026-08-29, mars. `aidotmarket/ai-market-backend` PR #296 "refactor(payments): remove legacy Stripe Express path" merged 2026-08-27 as `10b56cb0a37d6408e84ec0a47d652522d66987ec` (candidate `b03e3790` on base `800f2798`); production `682ff2946d` contains it (`git merge-base --is-ancestor` verified). Removed: the obsolete Stripe Express router, wrappers, schemas and retired tests; preserved: Standard Connect and the central Stripe webhook at `/api/v1/webhooks/stripe`. Reviews: CC APPROVE_WITH_NITS + GLM APPROVE_WITH_NITS (R2), Kimi waived by Max (Event Ledger `23836f5c-7e13-47ad-a084-d7ad327c73f7`); live Stripe read-only registration proof showed both enabled endpoints on the central webhook and none on the retired prefix. The P1-P7 precondition pack (Amendment A1 R4, `1ab86d07`) was measured fresh in S1625 before dispatch. Previous text of this bullet (evidence gate, recorded before the merge) is superseded.

- **C2-D duplicate webhook and legacy fallback deletion LANDED.** Recorded S1659, 2026-09-07, mars. `aidotmarket/ai-market-backend` PR #328 merged 2026-09-07 as `164c6986bc1fc29ccec4a3ef04cfadf8e3d1b64a` (candidate `836663afab14a06c9a6fd5a86bd39c5e152122ed` on base `4eadf8edcc395a63258ec42b7f38f3499b57141e`). Removed `/api/v1/connect/webhook`, the central seller-profile account-update leg, and both legacy account-to-user lookups; `/api/v1/webhooks/stripe` remains the sole Stripe event route. Synthetic-account and payout-failure resolution now resolve through `get_stripe_connect_user_sync`, which joins canonical `stripe_connect` and `auth_user` identities to `users` and raises on an ambiguous party. Pre-dispatch live/test Stripe registration re-read 2026-09-06 23:14 UTC: no receiver registered on the retired route. Gate 3 unanimous on `836663af`: CC APPROVE_WITH_NITS, GLM APPROVE_WITH_NITS, DeepSeek APPROVE_WITH_NITS at R2 after production read-only evidence (0 legacy-column accounts without a bridge, 0 parties with duplicate auth identities). Deauthorization contract after C2-D: `account.application.deauthorized` disables payouts through `_handle_account_update(deauthorized=True)` and does NOT remove the Connect party identity (`remove_stripe_connect_identity` is no longer wired; retained for scripts). Known LOW follow-up: `_handle_account_update` still keys its `users` write on `users.stripe_account_id`; affected population in production is the bootstrap system user only.
...
| Updated | S1659, 2026-09-07, mars — recorded C2-D merge (PR #328) and the post-C2-D deauthorization contract |
