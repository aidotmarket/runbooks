...
S1721 supersedes the S1651 roster snapshot. For gate voting, `REQUIRED_MEMBERS` is exactly `{glm, deepseek, gemini}`. CC is available only by explicit name for a non-Council second opinion, Kimi is removed, and MP is the builder rather than a voter. Those gate facts constrain gate review; they do not independently define a Hall's participant list.

Historical roster snapshots only: S528 described DeepSeek as a graduated full voter, and S726 described an MP/AG/DeepSeek/CC four-voter default. Both snapshots are superseded by S1721 and must not be used to select live Hall participants.
...
| CC | Explicit-name non-Council second opinion | Read-only CC path | None for current gates | COMPLETE |
...
| Phase 1 dispatch | `council_request(agent=<glm\|deepseek\|gemini>, mode=open_response, task=<frozen prompt>)`, one call per governed voter participant, none shown another's answer. CC may be selected explicitly only for a separate non-Council second opinion. E-02 unchanged. |
