...
- Manual bounce when the reloader defers on live sessions (log: `deploy pending (...) but N live/unverifiable session(s); deferring`) and a peer needs the new code now: the reloader has already fast-forwarded `/Users/max/koskadeux-mcp`; confirm `git rev-parse HEAD` equals the target and that no bridge build is running (fresh job specs in `koskadeux-state/ts-sockets/jobs/`, `ts` per socket). Announce BOUNCING on the peer bus, then run `launchctl kickstart -k gui/$(id -u)/com.koskadeux.mcp` via `shell_request action=background`. The kickstart kills the shell that issued it, so nothing chained after it runs: in a fresh call afterwards, verify the new PID with `launchctl list | grep com.koskadeux.mcp`, write the new SHA to `koskadeux-state/deployed_sha`, append a `RELOADED: manual bounce` line to `/tmp/koskadeux_mcp_reload.log`, and post DONE. Pass `instance=` on tool calls after the bounce (G-03 caveat). Verified S1738, 2026-09-23, onto `821f5aa1`.
