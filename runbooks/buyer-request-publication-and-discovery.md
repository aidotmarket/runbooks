...
The market uses one decision everywhere. An open, unexpired request from a verified non-synthetic buyer becomes public only when the buyer has consented to the exact current public text and the automated safety result is clean. Public website, API, sitemap, text, agent, homepage, search-submission, and seller-matching surfaces consume that persisted decision; none may invent its own eligibility rule.
