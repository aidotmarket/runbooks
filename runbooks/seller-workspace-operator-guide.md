...
As of September12,13:23UTC, the complete revised design is candidate `793e79153d9eac5c2d57536982d3f793b04d2c19` in [draft PR169](https://github.com/aidotmarket/runbooks/pull/169), spec SHA256 `9885f42481b947e901c991c5827346f4df77fd7461b0fe4253bca6ede547a16e`. The immutable [full implementation contract](https://github.com/aidotmarket/runbooks/blob/793e79153d9eac5c2d57536982d3f793b04d2c19/specs/SELLER-WORKSPACE-ACTIVATION-S1712-GATE2.md) defines the planned schema, commands, custody, failure states, tests and upgrade obligations. These commands and tables are proposed, not installed or authorized to execute by this paragraph.

The [prior complete panel](evidence/workspace-gate2-s1712/SELLER-WORKSPACE-GATE2-PANEL-S1712.json) did not accept e138f16: DS required revision and GLM requested changes even though CC approved with nits. Do not reuse the old vote or interpret a majority as money/authority acceptance. The [new dispatch receipt](evidence/workspace-gate2-s1712/SELLER-WORKSPACE-GATE2-R2-DISPATCH-S1712.json) binds all three actual request/response paths to the same full packet, with the prior full responses preserved. Each response must be fully read, its exact file hashed and its findings reconciled before a final decision. A submitted/running wrapper is not an approval; an observation timeout does not authorize a duplicate dispatch.

The revised design first records the owner's window and a complete backend-observed order baseline, then registers one real paid W order from that delta. The signed refund exception uses that exact immutable registration. It keeps ordinary finance processing as the only financial writer and preserves P/N and paid history. It explicitly accounts for concurrent order creation and backend surgical reset; a host lock alone cannot protect those database races.
...
The normal seed clears seller02 TOTP/Connect/payouts state, while public Connect routes refuse synthetic users. GET Connect status can persist provider state for admitted users. A reviewed seed-only activation/deactivation amendment is required; do not patch payout flags, copy seller01 secrets or relax public synthetic guards. Workspace Gate1 is unanimously accepted at `1ff494acc79245e8481ece2e985eda902fbf5207`. The concrete Gate2 contract is draft PR169, candidate `f3f8e7df534eaea06393926071cd81cbcad18ea9`; it still requires final integration binding and review before MP implementation. Mars's conditional non-interference feedback requires exact shared locking, captured before-state, provider disposition and default seed/reset refusal while an amendment is active or inconsistent.
