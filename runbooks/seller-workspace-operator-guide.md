...
## Historical project authority — 12 September 2026

Max explicitly authorized continuation without CC on 12 September. Owner event e4c7e041-4eee-4217-8be5-6591157274ca records that then-current exception for the S1714 ai.market production objective including Workspace. Max further clarified: “If GLM and Deepseek agree that is approval.” This paragraph is retained as historical authority and does not override the later S1721 CORE §5 roster: every current gate requires unanimous valid verdicts from GLM, DeepSeek, and Gemini. CC is available only by explicit name for a non-Council second opinion, and Kimi is removed. Existing CC findings, complete raw responses and terminal quota failure remain historical evidence. No prior vote carries. Full testing and source freeze, environment integration, combined review, genuine phases, cutover and enabled AWS S3/Cloudflare R2 proof remain required within the cumulative USD100 authorization.

## September 12 continuation status

S1712 remains the sole Vulcan owner. The full release is incomplete. MP244 is independently verified at `c3a62af8ccf60929a8e7cec3d62c9d30e0adefd2`: all42 targets,321 migration/two protected hashes and1,024 executions across1,010 unique cases reconcile. The six exact pre-existing baseline failures remain; all142 F7,20 F6 and19 posting positives pass. The exact product candidate now has unanimous CC/GLM/DeepSeek APPROVE_WITH_NITS acceptance; The corrected environment candidate606267a has unanimous full14-file approval with nits and merged as43d67264 with an identical reviewed tree. Its348 contract tests plus2 static checks pass; this is implementation acceptance, with real holds and live verification still ahead. See the [merchant and evidence-file diagnosis](seller-refund-access-diagnosis.md#environment-merchant-identity-and-immutable-evidence-correction). Workspace Gate2 candidateb444332a now has unanimous fresh CC/GLM/DeepSeek APPROVE_WITH_NITS acceptance, with no blocking findings; the exact decision and implementation dispositions are recorded below. The final source/spec recording is runbooks PR188 merge0ec6bac7; future runtime pins remain uninstalled. See the current design/recovery section below; no fixture implementation or owner window is certified by design acceptance. Existing cumulative $100 approval covers current work. Mars’s separate request to extend the original September15 handoff boundary remains unanswered; it does not pause today’s authorized work.

Read [refund and delivery diagnosis](seller-refund-access-diagnosis.md) for the admission/streaming/queue/retry lifecycle, support and upgrade procedures, and exact evidence. MP244 corrects the actual PostgreSQL startup race with authenticated named-database TCP readiness, total/per-probe timeouts and real delayed-start/expiry/cleanup proof. Hosted34688843645 passed on attempt1 at the corrected commit. All three local validation containers were independently confirmed absent. The old MP243 retry-only success remains historical. Canonical temporary-table logs have an explicit coverage limit; full new-schema refund proof comes from migrated PostgreSQL17 core. Product review is accepted with the explicit support limits in the diagnosis guide; environment implementation review is accepted; deployment and live release remain open.
...
R1, R2 and R3 were not implementation accepted. The latest correction protects the four packaged identities in both actual backend cleanup services and explicitly binds TEST synthetic-checkout arming. A prior approval, majority, submitted wrapper or observation timeout is not acceptance or authority to duplicate work. Backend MP is authorized first from acceptedc3 under the existing Mars umbrella with Vulcan the sole Workspace component owner; environment follows from43d67264 against the actual new interface. Full implementation review and genuine runtime proof remain separate.

The revised design first records the owner's window and a complete backend-observed order baseline, then registers one real paid W order from that delta. The signed refund exception uses that exact immutable registration. It keeps ordinary finance processing as the only financial writer and preserves P/N and paid history. It explicitly accounts for concurrent order creation and both backend account cleanup services; a host lock alone cannot protect those database races.

Credential cleanup is designed to retain the fixed seller02 TOTP generation rather than issue a name-addressed DELETE. Exact UUID/version/digest/marker lineage must match for reuse. `inactive_with_retained_custody` means readiness has been restored and the custody item remains; it does not mean the secret is absent. Drift or uncertain ownership preserves the item and records incomplete recovery. The design gives seed/reset/up and the fixed-party backend reset path explicit history-preservation rules. Do not manually erase an audit, baseline or authority row to make a retry succeed.

For diagnosis, start with the immutable contract and the current `outputs/SELLER-CHECKPOINT-S1712.json`, Living State `infra:seller-workspace-release-s1712`, actual full reviewer files and exact candidate/CI receipts. Distinguish owner review, Council design approval, MP implementation, isolated tests, Gate3 and live acceptance. Backend implementation is staged from acceptedc3, environment implementation from43d67264 against the resulting interface, with one Workspace owner and the existing Mars umbrella build record; do not reopen the completed historical Workspace build row. The accepted owner decision authorizes the ordered component build; retain exact build handles and source/test evidence before claiming implementation acceptance.

Preserve protected run25 `20260911T233041Z`, old environment03d3f30/backendd766/specs519b1d5. Its online hold deadline `2026-09-13T23:41:11.491739Z` is not a window release. Original online/offline lifecycle, corrected P/N genuine48-hour phases, real scheduled provenance, stable AIM Data release and the owner-issued transition precede W. The owner interface has no existing read-only command that proves all these gates: `--pins` checks source only, while normal settlement and `verify --from-clean-seed` mutate lifecycle. Do not run them as casual status checks. Production trigger freeze, old-writer quiescence, reconciliation, actual adopted images and fresh AWS/R2 public-path evidence remain separate requirements.

### Cleanup refusal, runtime configuration and monitor diagnosis

The planned shared fixed-UUID guard permanently refuses surgical reset and teardown of seller01, buyer01, buyer02 and seller02, including missing/damaged Workspace lineage and dry-run/apply. Existing auth/review failures keep their current meaning. `/api/v1/e2e/reset/account` returns400 with detail `workspace_protected_fixture`. `/api/v1/e2e/teardown/account` retains202 background-request acceptance; follow its returned run_id through `/api/v1/e2e/teardown/status/{run_id}` to `outcome=refusal`, `reason=workspace_protected_fixture` and empty executed_counts. A404 status result is not a terminal outcome. If no audit arrives, inspect the exact run/nonce and sanitized task/audit failures, then preserved DB evidence; never infer deletion or success from202 alone. Do not change allowlists, switch cleanup endpoints, mint substitute tokens or erase history to bypass the refusal.

Final reviewed TEST Compose sets `E2E_TEST_ROUTES_ENABLED=true` and both destructive allowlists to literal empty arrays, and binds them in immutable runtime_settings. Host checks the actual nonsecret container entries; the application independently compares settings. Both service guards must also be tested under deliberately populated isolated allowlists. Production and protected run25 are not armed by this design. Final versions.env must name the actual built Workspace backend in the later owner-reviewed environment Gate3/source recording.

The existing armed-window monitor, if invoked with these TEST settings and no E2E_ARMED_AT, reports P1 `e2e_routes_over_armed`. This is an explicitly expected, retained TEST-operation result, not healthy status and not evidence that the monitor has actually been scheduled or paged. Do not suppress it, broaden its window, add production exceptions or use broad sysadmin health as Workspace/release proof. Actual production occurrences remain actionable. Preserve registered refunds before any owner-supported disarm/retirement; changing immutable settings mid-window is not an alert-cleanup technique.

Upgrade qualification must inventory all flag effects (checkout, router, authenticated harness-bypass helper and sysadmin monitor) and any new account-deletion entrypoints, verify both services use the same pure fixed-UUID guard, and run actual signed route/task/status, PostgreSQL destructive-effect and configuration tests. The W refund disjunct is appended after both unchanged S1681 predicates. Keep cheap non-W rejection before optional Workspace lookups and preserve registered refund convergence during recovery.

Pre-registration provider refund and extra-order incidents differ: an unregistered signed refund is terminally acknowledged/suppressed with no authority or convergence path, and remains `recovery_required_unregistered_refund` until separately reviewed owner reconciliation. An extra order after registration preserves `recovery_required_extra_order` while only the original registered refund converges normally. Neither permits retroactive registration, deletion, baseline rewrite or automatic closure. Retain complete provider/event/order/custody/protected evidence and escalate to the existing owner.
...
4. Dispatch exact-candidate review using the current task-visible Council contract. GLM, DeepSeek, and Gemini are the required voters, unanimous 3/3. CC is available only by explicit name for a non-Council second opinion; Kimi is removed. Preserve candidate SHA, submitted artifact hashes, returned verdicts and any accepted nits. Do not dispatch using guessed signed inputs or stale schema.
...
The normal seed clears seller02 TOTP/Connect/payouts state, while public Connect routes refuse synthetic users. GET Connect status can persist provider state for admitted users. A reviewed seed-only activation/deactivation amendment is required; do not patch payout flags, copy seller01 secrets or relax public synthetic guards. Workspace Gate1 is unanimously accepted at `1ff494acc79245e8481ece2e985eda902fbf5207`. The concrete Gate2 contract is accepted candidate `b444332af4ec86c4258ae62d7c0c1e12f3f2645d` with the exact owner decision and nonblocking implementation dispositions above; implementation and live activation remain separate gates. Mars's conditional non-interference feedback requires exact shared locking, captured before-state, provider disposition and default seed/reset refusal while an amendment is active or inconsistent.
...
The protected old acceptance proof revoked and confirmed the same order. Accepted F1 at `9ab2e7cfef6272b6c708093d28c135d61f7b93c9`, precision fold `f34cb198d2415acf82c637b807618245025b6a5e`, replaces that future sequence: P is buyer01, N is buyer02 purchasing the same seller01 listing through a later normal checkout. N confirms before revocation, waits its own genuine 48 hours, proves bare-revoked refusal, receives a normal full refund and proves refund-specific refusal; P remains unrevoked for its payout. No second listing or two unpaid tabs. The four packaged fixtures must retain exact UUID/email/role/is_test and reciprocal order/transaction bindings. Vulcan seed correction `1ac556c13238a8b74f1bced9c8af870babcb2221` was integrated by Mars with 70 isolated tests passing; exact browser/host binding correction `c2fb627937822606c481ec7d586f6a80f82f19dd` rejects 18 independently tested corruptions in each validator. These are partial implementation checks, not a new live proof. Preserve run25 as original-contract evidence; never relabel its pending v1 receipt as failed-run refund authority. Do not reuse S1681 `--abandon-refunded` for Workspace orders or invoke internal handlers manually.
...
## Journal posting diagnosis and upgrade dependency

A payment method returning successfully or an ORM object showing `status=posted` is insufficient. Read the stored journal row again in a fresh query and verify `status`, `posted_at`, exact lines and account balances. The S1714 partial build exposed eleven real posting failures because the installed `public.finance_prevent_posted_entry_mutation()` function returns OLD on allowed draft UPDATE. In a BEFORE row trigger, that discards the proposed changes. The existing `_post_entry` callers can therefore leave a draft despite setting posted state in memory. The separate manual approval path updates `posted_at` without calling `_post_entry`; test its actual persistence and report residual status inconsistencies separately.

The [production catalogue receipt](evidence/seller-journal-s1712/SELLER-JOURNAL-TRIGGER-PRODUCTION-PROVENANCE-S1712.json) at September12 03:50:26UTC explicitly identifies Railway project `e81dd66f-808c-412e-b32c-f6d910f0ac5d`, production environment `23e322c3-b195-45d8-9151-c4c27a998c33`, Postgres service `b86ef2a3-97e1-42fe-922a-73fe18b1d518`, and database `railway`. The old body SHA256 is `1f2b86b5ac66d98691c8c4d8b1c6b98d48d16fb551aed59de795475e8260e724`. The enabled trigger is BEFORE DELETE OR UPDATE, FOR EACH ROW, on `public.journal_entries`, bound to that function without a WHEN clause. This was production evidence, separate from the disposable probe.

The [aggregate journal-state receipt](evidence/seller-journal-s1712/SELLER-JOURNAL-PRODUCTION-STATE-COUNTS-S1712.json) at 03:51:37UTC returned no groups over all journal entries: zero persisted rows at that observation. This does not establish whether expected historical journals are missing, or whether other financial tables are correct. Do not infer an affected-row count from a draft count, and do not repair historical data automatically. Refresh the observation before cutover.

To reproduce diagnosis, first use the established [Infisical](../infisical-secrets.md), [SecOps](../local-secops.md) and [production cutover](seller-production-payment-cutover.md) access procedures. Resolve the exact Railway project/environment/Postgres service; retrieve connection variables by the confirmed environment and service UUIDs and hold the DSN only in process memory. Require connection read-only mode and a bounded statement timeout, then run the SELECT-only `query` retained in the receipt inside its BEGIN READ ONLY / ROLLBACK transaction. Record current database, server identity, target IDs, timestamp, `transaction_read_only`, exact function metadata/body and trigger definitions. Suppress credential values and connection/error bodies. Do not execute the disposable probe SQL against production. A missing/unknown definition, alternate function, disabled trigger, changed attachment, WHEN clause or different event/timing/row scope is drift to reconcile, not permission to replace blindly.

F2 design is accepted in Mars decision `8b4a315ddfc5325959190ca6ae1b5e07fc915b35`, section15 of the [payment specification PR166](https://github.com/aidotmarket/runbooks/pull/166). MP must implement the change in the named new migration/readiness/report scope: retain rejection of any old posted or posted_at-nonnull row; return OLD only for allowed DELETE and NEW for allowed UPDATE. Preserve all balance, period, line immutability and entity guards. Require exact supported function result/language/security/body and sole intended trigger attachment, with all relevant guards enabled O. The unapplied body-only patch is incomplete evidence and must not be pasted into production. Reinstalling the defective old body is not an authorized rollback.

Before release, prove full-ancestry and repeat migration convergence on disposable PostgreSQL, refusal of missing/altered/disabled/misbound definitions, draft edit/delete behavior, persisted posting with exact balances, posted entry AND line UPDATE/DELETE refusal, all eleven failing positive money paths and all six shared finance posting callers. Exercise post-then-mutate behavior and the separate manual path. Those runtime results are distinct from design approval and the temporary trigger-semantic probe. Final exact product/environment candidates need both GLM and DeepSeek approval under the current project waiver. Freeze auto-deployment before product merge, quiesce old writers, reconcile unknown provider outcomes, migrate with dispatch disabled and verify readiness before enabling, following the cutover runbook. No harness bypass or historical financial backfill is part of this correction.
...
Historical MP248 results and the canonical payment binding diagnosis are preserved in evidence/workspace-canonical-pi-s1712. For the current candidate, qualification and required next action, follow [Authoritative continuation status](#authoritative-continuation-status).

## Authoritative continuation status

This is the single operator-facing continuation status. Parent runbooks9645601d50c10a5fe42f38e7972b8c159dc69b04 remains unaccepted. DeepSeek response220544-760714 and GLM response222137-175754 completed the full prior specification/source and all33 historical bodies; both retain the same MEDIUM webhook-audit expected_binding F1. Earlier DeepSeek HIGH and original CC/GLM nonblocking votes remain historical; no old vote carries. CC is USER-WAIVED under the current project authority above. The coherent31-target correction explicitly fixes node09/287 as proven legacy expected_binding=None and node20/661 as complete canonical12-field binding, preserves neighboring business assertions, separates legacy fixtures from durable attempts, qualifies native uniqueness and actual cleanup atomicity, and inventories all metadata/spend writers. This proposed candidate requires fresh GLM plus DeepSeek approval before sole Mars MP implementation. Full enabled release remains incomplete.

MP248 backend817f7331a07d07929a67198b28a82ed9b98baa56 remains partial and unqualified. Its capture failure and44 template setup errors, source-vector drift and original1990-node reconciliation remain mandatory qualification work. Freeze the coherent final specification with the complete response history and owner disposition, obtain fresh required review, then bind the accepted source/tree/spec and31-file scope into the sole owner-dispatched backend build. Do not call proposed methods until their implementation and source identity are verified. Complete backend qualification must precede environment integration, combined review, genuine protected phases, cutover and full enabled AWS S3/R2 proof.
