...
4. Dispatch exact-candidate review using the current task-visible Council contract. GLM and DeepSeek are the required voters for this current objective under the explicit September12 CC waiver; CC is not counted as approved. Kimi is comparison-only. Preserve candidate SHA, submitted artifact hashes, returned verdicts and any accepted nits. Do not dispatch using guessed signed inputs or stale schema.
