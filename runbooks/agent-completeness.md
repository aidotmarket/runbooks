...
| Provider intake | GitHub uses the existing signed `workflow_run` webhook. Railway reuses the existing five-minute SysAdmin read probe and forwards only latest `FAILED` or `CRASHED` deployment facts; do not add Railway's undocumented-auth project webhook or a second scheduler. The normalized provider-event fingerprint is checked across all historical incidents, so the same polled deployment cannot reopen after a terminal outcome. Cloudflare v1 is adapter-only/manual intake: do not add a webhook, upgrade a zone plan, or widen the read token merely to automate ingestion. |
