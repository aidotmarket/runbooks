...
| Verification retry cap and aging (S1657) | After 8 consecutive verification failures with the same `safe_reason`, the incident receives a `remediator_demotion` action with reason `verification_stalled`, leaves Needs-Max for the operator Attention list, and is not rechecked. Seven-day demotion applies to every escalated incident in the Remediator recovery pass, measured from its latest escalation, regardless of playbook attempts: action `remediator_demotion`, reason `aged_out`, Event Ledger event `incident_aged_out`. Demoted incidents are excluded from subsequent recovery checks. |
...
Use this residual path only for non-GitHub incidents and GitHub incidents where the default-branch workflow is genuinely still red. GitHub recovery observed by the backend auto-resolves as `superseded` without manual closeout. This procedure does not replace the claim-first worker or make provider adapters writable.
...
3. Check whether the backend already observes one of the three GitHub recovery conditions above; those incidents leave automatically. A genuinely red default-branch workflow still requires investigation and evidence of resolution before manual closeout.
...
| F-05 | Needs-Max counts do not match the audit, a current Remediator escalation within the S1657 Needs-Max audience is absent WITHOUT a demotion or supersession record, an escalation is duplicated, or viewing the report starts an agent. | The read model stopped using canonical `IncidentAction` events, attention selection drifted from current `escalated`-minus-demoted state within the S1657 Needs-Max audience, or reporting gained a write/agent path. (An escalation that left the feed WITH a demotion or supersession action is intended behaviour; GitHub failures outside the S1657 audience are also intentionally absent from Needs-Max.) | Compare `remediator_claimed` and terminal actions for the seven-day window with the endpoint; compare current Remediator-owned escalated incidents within the S1657 Needs-Max audience without demotion actions against the flagged needs-Max rows; confirm repeated GET requests add no audit rows and start no Codex process. | G-05 | CONFIRMED |
