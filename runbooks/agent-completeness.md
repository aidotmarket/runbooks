...
**S1585 consolidation (pending merge of build/bq-for-max-queue-signal-s1585-*):** the standalone /for-max page is retired; needs-Max items render as flagged rows at the top of https://ops.ai.market/build-queue, the red count badge lives on the BUILD QUEUE nav item (favicon + title count unchanged), /for-max redirects there, ForMaxPanel.tsx is deleted (row subcomponent NeedsMaxRow.tsx), the feed excludes owner-assigned tickets (first-class assignee columns) and demoted/superseded incidents, the seven-day Remediator report is no longer rendered on the queue page, and demoted incidents plus unknown-owner tickets appear on the OPS panel Attention list via GET /api/v1/ops/operator-attention.
