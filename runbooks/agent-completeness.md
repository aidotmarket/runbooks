...
- id: E-04
  ...
  argument_sourcing: {runner_config: /Users/max/Projects/ai-market/allai-remediator-runner/.codex/config.toml, required_live_mcp_tools: "[allai_remediator_request]", required_external_integrations: "[allai_remediator_request]", allowed_inert_nested_host_primitives: "[apply_patch, update_plan]", forbidden_inventory: "exec_command or any command runner, browser, app, provider, repository, network tool, or other external integration"}
  idempotency: IDEMPOTENT
  expected_success: {shape: layered host-scaffolding and singleton external-integration evidence, verification: inspect a fresh actual runner context; require live scoped MCP tools/list to equal exactly allai_remediator_request and require it to be the exactly one external operational/service integration; prove nested host inventory contains no exec_command, browser, app, provider, repository, network tool, or other external integration; record that the remaining root host controls are platform scaffolding and may not be invoked except functions.exec solely to call allai_remediator_request; record that pre-install nested ALL_TOOLS contained only inert apply_patch and update_plan, neither may be used by the automation, and apply_patch was write-denied for both runner workspace and /tmp with no file created; cross-check code_mode_host remains enabled only for MCP orchestration while every forbidden feature and non-remediator plugin is disabled, approval_policy=never, web_search=disabled, the workspace is read-only, and command network is disabled}
...
| F-03 | Live scoped MCP `tools/list` is not exactly `{allai_remediator_request}`; the runner has another external operational/service integration; nested host inventory exposes `exec_command`, another executable, or a browser/app/provider/repository/network tool; host scaffolding is invoked outside the sole `functions.exec` call to `allai_remediator_request`; or layered inventory proof is incomplete. | A forbidden feature is enabled, inherited `node_repl` or a non-remediator plugin remains enabled, plugin/server/tool scope drifted, the run did not load the project-local config, or inert built-in host scaffolding was mistaken for an authorized automation capability. Root `functions.exec`/`wait`/`request_user_input` and collaboration control names can remain system-visible even under strict config; visibility alone is not an extra external integration. | Keep ingress false and the recurring task paused. Separately capture root host controls, nested `functions.exec` `ALL_TOOLS`, and live scoped MCP `tools/list`. Require the live MCP singleton and exactly one external operational/service integration; require no nested command runner or browser/app/provider/repository/network/external tool; and verify that all host scaffolding remains unused except `functions.exec` solely for the MCP call. Confirm `code_mode_host=true`, and retain the empirical proof that pre-install nested inventory was only inert `{apply_patch, update_plan}` and that workspace and `/tmp` patch attempts were write-denied and created no file. | G-03 | CONFIRMED |
...
- id: G-03
  ...
  change_pattern: Keep ingress false and the task paused; disable shell_tool, unified_exec, view_image, multi_agent, apps, in_app_browser, all browser_use variants, computer_use, image_generation, inherited node_repl, and every installed non-remediator plugin; set collaboration_modes=false; retain approval_policy=never, web_search=disabled, code_mode_host=true solely to orchestrate the plugin MCP call, the allai-remediator permission profile, command network disabled, and only ai-market@personal server allai-remediator tool allai_remediator_request; prohibit invoking all built-in host scaffolding except functions.exec solely to call allai_remediator_request, including prohibiting automation use of inert apply_patch and update_plan; then start a fresh runner context and rerun E-04.
  ...
  integrity_check: Live scoped tools/list is exactly allai_remediator_request; it is the exactly one external operational/service integration; nested host inventory contains no exec_command, browser, app, provider, repository, network tool, other executable, or other external integration; remaining built-in host scaffolding is unused except functions.exec solely for the MCP call; and the apply_patch write-denial evidence remains defense in depth rather than a total-tool claim.
- id: G-04
  symptom_ref: F-04
  component_ref: Scheduled Operator Worker
...
scenario_set:
  ...
  - {id: I-15, type: repair, refs: [G-03], scenario: "A fresh nested host inventory exposes exec_command or another external integration, or the automation invokes inert apply_patch or update_plan.", expected_answers: [{kind: human_action, verb: keep paused and remove, object: unexpected executable or external integration and prohibit host-primitive use before fresh layered proof, target: project-local Codex config and automation instructions}], weight: 0.0625}
