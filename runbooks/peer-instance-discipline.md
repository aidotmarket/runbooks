...
> Phase 2 Chunk A forward-path draft. The root source remains in place. M5/M6
> containment from `work-checkout.md` and `vulcan-configuration.md` is not
> claimed by this path materialization. While this header says DRAFT, this file
> is not catalog authority and generated catalog surfaces must remain unchanged.
