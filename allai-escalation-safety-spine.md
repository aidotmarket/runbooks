...
| SysAdmin deferred monitor recheck | ai-market-backend app/allai/agents/sysadmin/agent.py:SysAdminAgent._recheck_contract_after_warmup | Instance lock/done flag plus fresh capability bindings | SysAdmin scheduler and bound monitor capabilities | At grace expiry, concurrent deferred contracts share exactly one fresh full re-bind on the scheduler-owning instance before their contracts run. A re-bind exception is logged but never swallows a recheck; a capability still disabled by the fresh probe follows the existing retry-then-`monitor_unavailable` page path. |
...
- A guaranteed post-warmup contract recheck evaluates fresh capability bindings: the scheduler-owning SysAdmin instance performs one lock-guarded full re-bind at grace expiry, then every deferred contract proceeds normally. A fresh probe that remains broken still retries/pages as `monitor_unavailable`, and a re-bind exception is logged without suppressing the contract run.
- The warmup breadcrumb covers restart loops only while the same Railway container filesystem survives. A replacement container starts breadcrumb-less, so cross-container crash loops fall back to Railway healthcheck deploy-failure alerting plus the external allAI fail-open watchdog; the breadcrumb continues to cover within-container process restart loops.
