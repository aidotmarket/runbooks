...
| Headed Chrome dies with `TargetClosedError` seconds after launch · browser window appears on Max's desktop during an agent run | [browser-runner.md §F/§G](browser-runner.md) — verify the `kdbrowser` GUI session and route headed launches through the isolated runner |
...
**Browser runner — isolated headed Chrome on Titan-1** — the `kdbrowser` macOS GUI session, localhost `GET /health` and `POST /run` job surface, `/tmp/kd-browser-jobs` path convention, LaunchAgent lifecycle, isolation guarantee, and repair paths for `TargetClosedError` or a browser window appearing on Max's desktop: [browser-runner.md](browser-runner.md). Authored by Mars in S1211; either instance (Vulcan/Mars) operates it.
