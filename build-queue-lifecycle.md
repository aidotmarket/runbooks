...
| bq_status DB-direct dashboard (S1172) | `tools/state.py _handle_state_bq_status` (koskadeux-mcp) | Living State DB (psycopg2) | dashboards, session standup | Totals via `GROUP BY body->>'status'` aggregate; open items via full non-terminal select — replaces the truncated 500-entity most-recently-updated API window that hid open work (T-2026-000216; DB had 861 build entities). httpx path retained only as degraded fallback with a visible truncation warning. Merged main d31d9d01; live after gateway restart |
| assignment_query freshness filter REMOVED (S1172) | `tools/assignment_query/query.py` | Living State DB | pickup/assignment | The hard `git_state_last_reconciled_at > NOW()-300s` row filter is gone from CANONICAL_SELECT + COUNT_SQL (it returned zero rows whenever the reconciler was quiet >5 min, T-2026-000216). Staleness now surfaces via envelope metadata (`reconciliation_freshness_min_seconds`), never by hiding eligible rows. Merged main d31d9d01; live after gateway restart |
