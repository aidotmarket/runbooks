#!/usr/bin/env bash
# Daily Cloudflare DNS+settings export -> WORM S3 (aimarket-backups-prod/railway-config/).
...
LOG=/Users/max/Library/Logs/aimarket_cloudflare_export.log
LOCK="/tmp/aimarket_cloudflare_export_$(date -u +%Y%m%d).lock"
...
echo "[$(ts)] === starting cloudflare export ===" >> "$LOG"
PYBIN=""
...
"$INF" run --token "$TOKEN" --projectId "$PROJECT" --env prod --path / --domain "$INF_DOMAIN" -- \
  "$PYBIN" "$SCRIPT_DIR/cloudflare_export.py" >> "$LOG" 2>&1
rc=$?
unset TOKEN
if [ "$rc" -eq 0 ]; then touch "$LOCK"; echo "[$(ts)] cloudflare export OK (lock set)" >> "$LOG"; else echo "[$(ts)] cloudflare export FAILED rc=$rc" >> "$LOG"; fi
exit $rc
