...
| Password registration | `app/api/v1/endpoints/auth.py:432` (`POST /auth/register`) | Email + password. Creates the user `status=active` immediately. **A verification email is sent at register and login is blocked until verified for accounts created after 2026-07-07** (S1139, `EMAIL_VERIFICATION_ENFORCE_AFTER` grandfathers older accounts) — see the Email Verification section. |
...
| Email + password | `POST /auth/register` → `POST /auth/login` | **Yes, since S1139 (2026-07-07).** `/register` creates the account `status=active` with `email_verified=false` AND sends the verification email inline (`_send_email_verification`, auth.py:499-500). `/auth/login` raises 403 `{email_verification_required:true}` for unverified accounts created after the `EMAIL_VERIFICATION_ENFORCE_AFTER` cutoff (`_enforce_email_verification`); pre-cutoff accounts are grandfathered. Kill switch: `REQUIRE_EMAIL_VERIFICATION`. |

Historical note (pre-S1139): a password user could register and use the platform without ever confirming their address; snapshot 2026-07-07 showed 31 of 39 active users with `email_verified=false`. Since S1139 shipped that day, new password accounts must verify before login; grandfathered pre-cutoff accounts may still show `email_verified=false` while remaining working accounts (relevant to attempt detection in BQ-SIGNUP-LIFECYCLE-EMAILS-S1548).

The verification primitives already exist and work: `_create_email_verify_token` / `_verify_email_token` (HMAC-SHA256, `EMAIL_VERIFY_TTL`), `POST /auth/verify-email?token=...` (flips `email_verified=true`), and `POST /auth/send-verification-email` (since S1139, actually sends). The email transport is live and proven by magic-link: `app/services/email_service.py` `get_email_service()` (provider Resend via `RESEND_API_KEY`, gmail fallback), `send_magic_link_email` / `send_password_reset_email`.
