---
title: Account Teardown & User-Data Erasure
owner: vulcan
last_verified: '2026-07-11'
aliases: []
error_signatures:
- interactive-login garbage from the infisical CLI
- empty secret value (rc=0, len 0)
...
|---|---|---|---|
| ai.market prod Postgres | The `users` row and its FK closure; all weak-link tables | `AUTHOR_DISPATCH_DATABASE_URL` — Infisical `ai-market-backend`/prod (project `bd272d48…`); Railway-native `DATABASE_URL` vars are STALE, do not use (`backup-and-recovery.md` When it breaks-02 caveat) | ai.market backend |
...
| Legal-record tables | (frozen — no teardown entry point) | `terms_acceptance`, `disclosure_snapshots`, `seller_license_acceptances`, `license_acceptances`, `license_acceptance_events`, and custom `license_documents` | terms enforcement, disclosure, listing licences and signed order records | The S1735 licence migration adds RESTRICT legal-record references and append-only acceptance/event guards. These records and custom documents are under the H.1 legal hold: disposition on erasure is a per-request Max decision, never routine deletion. See `runbooks/listing-licenses.md` and backend `alembic/versions/20260922_001_s1735_listing_licenses.py`. The S1165 table counts above are a dated production snapshot and must be re-derived before teardown. |
...
- id: E-01
  ...
  expected_success:
    ...
    verification: record the run date; diff against Architecture & interactions
  expected_failures:
  ...
  next_step_success: if counts drifted from Architecture & interactions, refresh Architecture & interactions and Maintenance in the same session (Repair-03)
...
- id: E-02
  ...
  tool_or_endpoint: Repair-01 ordered manual procedure (operator SQL, single transaction, dry-run first)
  argument_sourcing:
  ...
  expected_success:
    ...
    verification: Repair-01 integrity check
  expected_failures:
    - signature: foreign_key_violation on NO ACTION/RESTRICT tables
      cause: ordering error (When it breaks-01)
    - signature: residual weak-link rows
      cause: no-FK tables never cascade (When it breaks-02)
  next_step_success: log a decision event in Living State recording the teardown (id, date, session)
  next_step_failure: ROLLBACK (single transaction), diagnose per When it breaks, fix ordering, retry once; 2-strike rule applies
- id: E-03
  ...
  expected_success:
    ...
    verification: then Repair-01 for the DB, plus Stripe deletion/anonymization, CRM removal (CRM Steward), Google token revocation, and a recorded note that WORM backups purge by retention expiry only
  expected_failures: []
...
|---|---|---|---|---|---|
| F-01 | `DELETE FROM users` fails with foreign_key_violation | 43 NO ACTION + 2 RESTRICT direct-FK tables (by design); children not deleted first or delete out of order | read the constraint name in the error; `SELECT conrelid::regclass, confdeltype FROM pg_constraint WHERE conname='<name>'` | Repair-01 | CONFIRMED |
| F-02 | User PII still findable after a "successful" teardown | weak-link tables have no FK so nothing cascades: CRM (`party_person`), tokens, support tables, `accounts`, beta/partner intake, `api_usage` family | run the How to operate-01 step-3 weak-link scan filtered by the user's id AND email | Repair-02 | CONFIRMED |
| F-03 | PII persists outside the database after full DB teardown | Stripe customer/Connect objects; Google-side OAuth grants; S3 WORM backups | Stripe dashboard search by email; token tables + Google account page; backup objects persisting is EXPECTED (retention) | Repair-02 | CONFIRMED |
| F-04 | Footprint numbers in Architecture & interactions disagree with a fresh How to operate-01 run | schema drift — every migration can grow/shrink the closure (grew 116→121 between S1163 and S1165); the S1163 P3 drop will shrink it | re-run How to operate-01; diff table lists against Architecture & interactions | Repair-03 | CONFIRMED |
| F-05 | Teardown transaction hangs | lock contention with live writers on closure tables | `pg_stat_activity` / `pg_locks` for the blocked pid; confirm `lock_timeout` was set per Repair-01 | Repair-01 | HYPOTHESIZED |
...
- id: G-02
  ...
  repair_entry_point: How to operate-01 step-3 scan parameterized by the user's id AND email; CRM Steward tools; Stripe API/dashboard; Google token revocation before row deletion
  change_pattern: for each PII-bearing weak-link table in Architecture & interactions, delete or anonymize rows matching the user id/email (test account — delete). For a real erasure additionally — Stripe customer delete/anonymize per Max ruling; CRM party_person + interactions via CRM Steward; revoke Google grants BEFORE deleting token rows; record that WORM backups expire by retention, not deletion
...
- id: G-03
  ...
  repair_entry_point: this file — Architecture & interactions numbers/lists + Maintenance refresh fields
  change_pattern: re-run How to operate-01, update Architecture & interactions counts and table lists, note the drift and its causing migration, refresh Maintenance in the same commit; after the S1163 P3 drop lands this refresh is MANDATORY (a Maintenance refresh trigger)
  rollback_procedure: git revert of the runbook commit
  integrity_check: scripts/check.py passes and Architecture & interactions matches a same-day How to operate-01 run
```

### G-05. Rehearsal gotchas (S1168 go-live T-2026-000214; S1171 completion T-2026-000215/218)

| Symptom | Cause | Fix |
|---|---|---|
| Entire backend API unresponsive (health curl 000) while a teardown run is in flight; pg_stat_activity shows engine connection idle-in-transaction on the catalog query and scheduler connections frozen at the same instant; container may be silently replaced within ~30s | Blocked asyncio event loop, pure CPU, zero awaits. TRUE root cause (S1171 faulthandler proof, corrects the S1168 diagnosis): _catalog_snapshot's max-depth propagation loops forever because the prod FK graph has a users<->referral_codes 2-cycle (users_referred_by_code_fkey, CRM referrals migration) and the root never enters the depths map. The predicate PATH-enumeration blowup named by T-2026-000214 was a second, co-located defect of the same class | BOTH FIXED S1171: cycle-safe depth propagation (self-edge skip, root skip, depth cap, iteration hard-cap -> incident refusal, cooperative yields) @ 6b9f6af1 (T-2026-000215); DAG-DP predicate builder + E2E_TEARDOWN_PREDICATE_BUILD_BUDGET_S (default 60s) + per-table yields @ 5dec512d (T-2026-000214). IMMEDIATE recovery if it ever recurs: Railway deploymentRestart, then disarm (flag false + FULL redeploy) |
| Second teardown run refused "concurrent teardown run refused by advisory lock" while nothing is running; pg_locks shows an idle pooled backend holding the advisory lock | Session-level pg_advisory_lock outlived the wrapper's connection handle: after the engine's COMMIT the async session returned the DBAPI connection to the pool, so the finally-unlock ran on a different connection (no-op) and the lock leaked on every completed run | FIXED S1171 @ df25b268 (T-2026-000218): transaction-scoped pg_try_advisory_xact_lock as the first statement of the run's transaction; auto-releases on every commit/rollback branch; manual unlock deleted. If seen on an old build: full redeploy clears the pool |
...
- **Legal-record gate:** `terms_acceptance`, `disclosure_snapshots`, `seller_license_acceptances`, `license_acceptances`, `license_acceptance_events`, and custom `license_documents` are consent, signature or contract records under legal hold. The licence tables and their referenced rows use RESTRICT FKs; seller acceptances and lifecycle events are append-only, while buyer acceptances permit only `active → terminated`. Never delete these rows without an explicit per-request Max ruling, and never weaken their RESTRICT/immutability guards. Refresh the live FK closure before any teardown; S1165 counts predate S1735.
...
- Weakening a legal-record RESTRICT delete rule (`terms_acceptance`, `disclosure_snapshots`, or the S1735 licence tables/references) to CASCADE or SET NULL, or weakening the licence acceptance/event immutability guards.
...
- Adding any new PII-bearing table or identifier column (extends the erasure footprint; the Architecture & interactions inventory must be updated in the same change).
...
The future teardown feature's API endpoint(s) and any MCP tool signature it registers; today there is no public contract — the manual Repair-01 procedure is operator-internal.
...
scenario_set:
  - id: I-01
    ...
    expected_answers:
      - kind: human_action
        action: run the How to operate-01 footprint queries via psql against AUTHOR_DISPATCH_DATABASE_URL (closure + delete-rule inventory + weak-link scan filtered by the user's id/email)
    weight: 0.08333333
  - id: I-02
    type: operate
    refs: [E-02]
  ...
  - id: I-04
    ...
    expected_answers:
      - kind: human_action
        action: resolve the constraint name to its table and delete rule via pg_constraint, then follow the Repair-01 ordering
    weight: 0.08333333
  - id: I-05
    type: isolate
    refs: [F-02]
    ...
    expected_answers:
      - kind: human_action
        action: run the weak-link scan (How to operate-01 step 3) by the user's id AND email — no-FK tables never cascade
    weight: 0.08333333
  - id: I-06
    type: isolate
    refs: [F-03]
    ...
    expected_answers:
      - kind: human_action
        action: check the Stripe customer object by email per Repair-02 — external surfaces are untouched by SQL
    weight: 0.08333333
  - id: I-07
    type: isolate
    refs: [F-04]
    scenario: A fresh footprint run says 124 closure tables but Architecture & interactions says 121. First action?
    expected_answers:
      - kind: human_action
        action: treat as schema drift — identify the migration that added the tables, then refresh Architecture & interactions and Maintenance per Repair-03
    weight: 0.08333333
  - id: I-08
    type: repair
    refs: [G-01]
    ...
    expected_answers:
      - kind: human_action
        action: within the Repair-01 single transaction, delete the account's terms_acceptance and disclosure_snapshots rows (test account — allowed) before the users delete
    weight: 0.08333333
  - id: I-09
    type: repair
    refs: [G-02]
    ...
    expected_answers:
      - kind: human_action
        action: revoke the Google grant first, then delete the token row (Repair-02 ordering)
    weight: 0.08333333
  - id: I-10
    type: evolve
  ...
  - id: I-11
    ...
    expected_answers:
      - kind: classification
        action: REVIEW — it extends the erasure footprint; the Architecture & interactions weak-link/PII inventory must be updated in the same change
    weight: 0.08333333
  - id: I-12
    type: ambiguous
    refs: [E-01, G-01, G-02]
    ...
    expected_answers:
      - kind: human_action
        action: run the How to operate-01 footprint filtered by the user's id/email
      - kind: human_action
        action: run Repair-01's dry-run SELECT phase for the id
      - kind: human_action
        action: run the weak-link scan by id AND email
    weight: 0.08333333
```

Pass threshold: weighted score ≥ 0.80. Equal weights (1/12); no Acceptance criteria.1 justification needed.
...
Refresh log:
- S1168 (2026-07-10): Capabilities/Agent capabilities PLANNED→SHIPPED for the automated teardown feature (BQ-ACCOUNT-TEARDOWN-S1165 t1–t3 merged: backend 096ac580/e9c0495c/38c9453d, harness 408ac96; Gate 3 UNANIMOUS each chunk, builder excluded; Gate 4 prod-verified t1/t2). Feature is DORMANT: routes flag-gated off, token secret unset on Railway. Architecture & interactions inventory NOT re-derived — the S1163 P3-drop trigger has not fired yet; re-derivation stays mandatory when it lands. Go-live = spec GATE2-T3 deliverable 4 (Max: flip E2E_TEST_ROUTES_ENABLED, set E2E_TEARDOWN_TOKEN_SECRET on Railway backend from Infisical, then ordered create→reset→teardown→footprint-clean verification). last_refresh_commit references the pre-refresh main head (80708fd1); this entry lands in its child commit.
