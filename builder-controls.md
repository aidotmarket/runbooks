...
lifecycle_ref: §J
authoritative_scope: |
  The control surface between "dispatch a build" and "get a diff back": dispatch-time gates, in-flight bounds, and post-build/pre-push gates on the MP (Codex) builder path. Anchors cite koskadeux-mcp main @ e80d30600c (2026-08-08). Every file:line anchor below was re-measured against that tree in S1485; the S1455 originals in the 5800-7500 range of tools/agents.py were wrong by roughly 2,300 lines (they named the dispatch wrapper region, not the gate composition) and are corrected here. Fire counts come from the full stored dispatch corpus in /var/tmp/koskadeux/cc_tasks (3,024 task records; 254 MP dispatches). Dispatch mechanics, model pinning and CLI quirks are codex-mp.md; Council review mechanics are agent-dispatch.md and council-gate-process.md; this runbook is the controls inventory.
...
| Pre-push gate composition (12 terminal sites) | SHIPPED (scheduled for replacement) | `tools/agents.py:8063 _run_pre_push_gate_composition`, gate bodies ~8100-9600 | Exercised on every MP dispatch | 2026-08-06 |
...
The build path is: `dispatch_mp_build` / `council_request(mode=build)` handler in `tools/agents.py` → dispatch-time gates → FIFO slot queue → `codex_cli_bridge.py` runs the Codex CLI → output-envelope parsing/repair → post-build pre-push gate composition in `tools/agents.py` (`_run_pre_push_gate_composition` :8063; twelve terminal return sites spread across ~8100-9600; wrapper call sites :6256, :7766, :7864) → push → result persistence to `/var/tmp/koskadeux/cc_tasks`. Total control surface at e80d30600c: 21,247 lines (`tools/agents.py` 11,743; `codex_cli_bridge.py` 3,318; `koskadeux_mcp/structural_gate.py` 2,222; `koskadeux_mcp/structural_gate_runtime.py` 1,675; `claude_code_client.py` 1,528; `tools/agent_usage_log.py` 381; `tools/owned_child_exec.py` 380).
...
Operators (Vulcan/Mars): dispatch builds, read every control's verdict, salvage preserved work, author the routing fork and any live-file edit (never MP). MP (Codex): the only builder; builds new files for the bridge programme; never edits `codex_cli_bridge.py` or `tools/agents.py` (bootstrap hazard, decision event a03efd33). Council (CC/Kimi/GLM): owns correctness adjudication at Gate 3 - the controls below were never a substitute for review and the rebuild makes that explicit.
...
- **C-02 Peer-claim coordination.** `tools/agents.py:1609` `_dispatch_coordination_instance`; requires `caller_instance` and honors BQ claims. Prevents the two operators double-building one item. STATUS: KEEP (lives above the bridge).
...
- **C-06 Git preconditions.** `tools/agents.py`: `pre_build_workspace_dirty` :8587, `pre_build_branch_ahead` :8680, `dispatch_git_evidence_unavailable` :8821, `dispatch_base_remote_mismatch` :8849. Protect the peer's uncommitted work and guarantee an exact recorded base. Fires: 10, mostly the T-490 tooling defect; dirty/ahead essentially never falsely. STATUS: KEEP; per-dispatch worktrees make the shared-clone cases moot.
...
- **C-12 Output-envelope schema + repair.** `council_dispatch_middleware/schema_repair.py:75`, `exceptions.py:20` (`RepairExhaustedError`). Parses/repairs the builder's structured output envelope; exists to honor CORE S7 structured AI-to-AI output. Fires: 25, an all-false-negative hit list - repeatedly reported FAILURE on builds whose commits were correct, pushed and independently verified, including an 88-line one-file spec (disproving size as trigger). STATUS: REMOVE from the build path. Git is the output; S7 continues to govern reviewer/Council interchange where it belongs.

### E.3 Post-build / pre-push gates (`tools/agents.py` composition, twelve terminal sites ~8063-9600)

- **C-13 Exactly-one-commit invariant.** Emits `post_build_no_commit` (:8901, second site :8985) and `post_build_multiple_commits` (:8992); restores via `_restore_after_post_build_failure`. Exists for review hygiene. Fires: 11 multi-commit + 10 no-commit; the multi-commit class discarded completed correct folds whose only sin was arriving as two commits (s1315 defect 2). STATUS: REMOVE the discard; squash at merge is trivial for an operator.
- **C-14 SHA claim check.** `post_build_sha_mismatch` (:9019). Verifies the builder's claimed head matches git. STATUS: superseded - the bridge reports git directly instead of checking claims about it.
- **C-15 CI gate.** `_gate_failure_payload` :9324, discard via `_discard_ci_failed_build` :9075; `ci_regression` classes at 9484/9502/9511/9524/9548/9568/9590. Exists for CORE S4 tests-before-completion. Fires: 1 - THE ONLY genuine correctness catch in the corpus. STATUS: RELOCATED, not dropped: tests run and are reported by the bridge (never gate), and CI re-runs at Gate 3 review, which is a hard prerequisite for any merge (spec §7, AC10).
- **C-16 Builder-output manifest verification.** `koskadeux_mcp/builder_output_verification.py:65` `verify_builder_output`, `:127` `builder_output_claim_mismatch`; restored via `_restore_after_post_build_failure` (:9117). Verifies the builder's line-anchor claims about its own diff. Fires: 10; the false-negative-by-regex specimen (`FAILED_FALSE_NEGATIVE_OUTPUT_MANIFEST_REGEX`) is durably recorded on Living State entity `build:bq-concurrent-build-capacity-s1214`, field `s1401_c5_canary_r10`. A build was failed by a regex. STATUS: REMOVE; Gate 3 reviews the actual diff, not claims about it.
- **C-17 Git-ref producer completion gate.** completion record written at :8316, refusal message at :8334; discard via `_discard_to_recorded_base` (:9043, a `git reset --hard`). Bookkeeping persistence of the completed range, failing which the build is destroyed. Fires: 11 - each destroyed work that had ALREADY passed commit-count, SHA, CI and manifest gates (s1398 FINDING A). STATUS: REMOVE the discard; bookkeeping failure must never destroy work.
- **C-18 Push gate + shared-branch CAS.** `push_failed` :9405/:9461/:9574, `shared_branch_cas_rejected` :9423/:9461; local commit preserved, recovery guidance attached. Protects shared branches from clobbering. Fires: 13 "push failed after all pre-push gates passed" - work preserved, cycle lost. STATUS: KEEP the preserve behavior; bridge adds fast-forward-only pushes, unique retry branches, 3x backoff, and a `pushed`/`preserved` split.
- **C-19 work_preserved / commits_created telemetry.** `tools/agents.py:9235`: `"work_preserved": commits_created > 0`. Defects, both live-confirmed: reports work lost while it sits uncommitted in the tree (s1315 defect 1; 8741f4ba on 2026-08-06), and `commits_created` reports WHOLE-HISTORY DEPTH, never a per-build figure (2726 specimen, execution-confirmed via `git rev-list --count`). Treat both fields as labels, not evidence. STATUS: replaced by the bridge's `preserved`/`pushed`/`push_status` report.
...
- **C-23 guard_direction_evidence completion gate.** `bq_complete` path: `guard_direction_evidence.py:792` `requires_directional_evidence` (call sites `tools/state.py:2711`, `build_completion_gate.py:331`) returns True on any `entity_state` other than `present`, so a failed entity FETCH fails the gate shut; the only unlock is declaring `guard_class=trust`, which non-guard builds must never do. STATUS: defect, in the S1455 removal scope.
...
Created S1455 (2026-08-06) by Max directive. Anchors re-measured and corrected S1485 (2026-08-08) at koskadeux-mcp e80d30600c: every symbol named in this runbook was found in the deployed tree, but ten of the tools/agents.py line anchors pinned at S1455 were wrong at their own anchor commit and have been replaced with measured values. Refresh trigger: any merge touching `tools/agents.py` gate composition, `codex_cli_bridge.py`, `builder_output_verification.py`, `structural_gate*.py`, or the minimal-bridge module. Owner re-verifies anchors at each refresh; stale anchors are a §K conformance failure.

## §K. Conformance

This runbook conforms to the §A-§K standard and is registered in CATALOG.json and TOPIC-ROUTER.md in the same change that created it. Fire counts and anchors are evidence-cited, not asserted; anchors were last measured against the deployed tree in S1485 by direct grep, and the measurement is repeatable by grepping each named symbol in koskadeux-mcp. The inventory's burden-of-proof rule mirrors the programme it documents: a control claimed here as justified must show its real catch.
