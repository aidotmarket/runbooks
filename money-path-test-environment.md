...
| 403 `synthetic_purchase_requires_e2e_guard` on `POST /api/v1/checkout/create` for buyer-02 / seller-01 | Synthetic pair outside an armed E2E window (backend `transaction_service.py:367`) | Arm one window as in G.2 (private override, Event Ledger record), buy within 15 minutes, then recreate the backend without the override | G.2; `docker exec ai-market-money-path-s1656-backend-1 sh -c 'echo $E2E_ARMED_AT'` |
| Frontend log `Failed to proxy http://localhost:18000/api/v1/licenses/... ECONNREFUSED`; full-licence link returns 500 | Next rewrites proxied inside the frontend container to host localhost (environment below PR #52) | Run at an environment head containing PR #52 and recreate the frontend | G.2 curl check |
