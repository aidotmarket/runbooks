...
For the accepted future lifecycle, `versions.env` at environment `43d67264ab7a3a7e47d6cc6911737273ce1c94cc` pins backend `c3a62af8ccf60929a8e7cec3d62c9d30e0adefd2`; frontend, AIM Data source/image and browser identities in the table remain unchanged. This is a source acceptance record, not a production deployment or permission to replace run 25. The table records the currently protected runtime.

| Product/runtime | Protected run 25 identity |
| --- | --- |
...
- **Recreate only the backend with the Workspace overlay** (for example after an overlay change). `host-seed` recreates the backend without the overlay, so do it by hand from the environment checkout, under the Infisical test-env, with Stripe production names unset:
  `set -a; . /Users/max/koskadeux-state/s1656/nightly-pins.env; set +a; export S1681_ENVIRONMENT_SHA=$(git rev-parse HEAD) S1738_WORKSPACE_OVERLAY=true S1712_WORKSPACE_AWS_PRINCIPAL_ARN=arn:aws:iam::157263244532:user/svc-s1681-broker INFISICAL_API_URL=https://secrets.ai.market INFISICAL_TOKEN=$(tr -d '\r\n' < ~/.config/infisical/sysadmin-token); infisical run --projectId=bd272d48-c5a1-4b52-9d24-12066ae4403c --env=test-env --silent -- sh -c 'unset STRIPE_SECRET_KEY STRIPE_PUBLISHABLE_KEY STRIPE_WEBHOOK_SECRET STRIPE_WEBHOOK_SECRET_PREVIOUS INFISICAL_TOKEN; docker compose --env-file versions.env -f compose.yaml -f compose.aim-data.override.yaml -f compose.workspace-interop.override.yaml up -d --no-deps --force-recreate backend'`
  Then **always** `docker restart ai-market-money-path-s1656-webhook-proxy-1`: nginx keeps the old backend IP. Use the same pattern with `frontend` to pick up a frontend compose change.
