...
- **Backend-only or frontend-only recreation has no supported, lock-wrapped command.** No supported lock-wrapped backend-only or frontend-only recreate exists at `e99803d` (`seed/host-seed.py` is not one; its only forced recreate targets `aim-data`). Environment PR #55 (open) adds a guarded `bin/recreate`. In S1738 the backend was recreated three times and the frontend once by a raw `infisical run ... docker compose --env-file versions.env -f compose.yaml -f compose.aim-data.override.yaml -f compose.workspace-interop.override.yaml up -d --no-deps --force-recreate <service>` outside the environment lock (no active run existed; recorded here as an unguarded operation, not a procedure). After any backend recreation the webhook proxy keeps the old backend IP and needed `docker restart ai-market-money-path-s1656-webhook-proxy-1`. Do not repeat this as a routine step: a lock-wrapped single-service recreate in the environment repo is an open follow-up; until it exists, use the guarded `bin/up` lifecycle and accept the overlay defect, or escalate.
...
- **Playwright** writes failure context (including request headers with bearer tokens) under `--output`; always pass `--output` to a private path outside Git and delete it afterwards.
...
| 403 `synthetic_purchase_requires_e2e_guard` on `POST /api/v1/checkout/create` for buyer-02 / seller-01 | Synthetic pair outside an armed E2E window (backend `transaction_service.py:367`) | Arm one window as in G.2 (private override, Event Ledger record), buy within 15 minutes, then disarm through the guarded `bin/up` lifecycle (or `bin/recreate backend` once environment PR #55 is merged), or escalate; never a raw compose recreate | G.2; `docker exec ai-market-money-path-s1656-backend-1 sh -c 'echo $E2E_ARMED_AT'` |
| Frontend log `Failed to proxy http://localhost:18000/api/v1/licenses/... ECONNREFUSED`; full-licence link returns 500 | Next rewrites proxied inside the frontend container to host localhost (environment below PR #52) | Bring the environment to a head containing PR #52 through the guarded `bin/up` lifecycle (or `bin/recreate frontend` once PR #55 is merged), or escalate; never a raw compose recreate | G.2 curl check |
