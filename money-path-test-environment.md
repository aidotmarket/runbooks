...
| Browser fails `return from hosted setup did not reach readiness ready` on the second (return-first) card setup | Frontend return page mounted without the re-authentication dialog (intermittent; `s1590-money-path.spec.ts:601–611`); the attempt stays `open` | Rerun (serial budget permitting); if it repeats twice in a row, root-cause the return page's session rehydrate in `DataVerificationPaymentMethod.tsx` under review | `select id,state,last_reconciled_at from data_verification_payin_setup_attempt order by created_at desc limit 2` in the backend database |
