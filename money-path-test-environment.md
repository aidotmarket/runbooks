...
- **Door probe** (`workspace/license-doors.py`): the in-container inventory import takes about 30 s and the app logs to stdout; the tool uses a 180 s timeout and a marker line. For a Workspace web order only the Workspace download/session/file routes and the deliveries token/download-url routes can succeed; the others are declared as pinned design refusals (PR #53). Put design-refusal cases first and active ones last so a mismatch aborts before any download is consumed; an order allows three downloads and three delivery-token attempts.
