...
1. Generate a new key on Titan-1:
   ...
   PY
   ```
2. Write the private key to Infisical prod at `/listing-enrichment-signing` (`POST /api/v3/secrets/raw/LISTING_ENRICHMENT_SIGNING_PRIVATE_KEY_PEM`, `secretPath` `/listing-enrichment-signing`, `type` `shared`). For rotation, first add the NEW public key to `LISTING_ENRICHMENT_SIGNING_PUBLIC_KEYS` (keep the old one), let the sync land and the service redeploy, then switch `LISTING_ENRICHMENT_SIGNING_KEY_ID` and replace the private secret. Remove the old public key only after every checkpoint signed with it has been superseded (see the T contract, signer_keys rotation rules).
3. Write or update the four `/`-path settings (`POST /api/v3/secrets/raw/<NAME>` with `secretPath` `/`). The Infisical→Railway sync (`railway-backend-prod`, auto-sync ON) pushes them; Railway redeploys the backend automatically on variable change (observed ~2 minutes).
4. Verify:
   ```bash
