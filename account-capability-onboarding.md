...
| Seller publish and Seller Workspace gate is `provisioning`, not `active` (S1739): a seller can connect storage, build and publish before Stripe payouts are live | SHIPPED | `app/routers/vz_publish.py:publish_listing`, `confirm_version`, `rotate_key`, `revoke_install`, `authenticate_local_action` and `app/api/v1/endpoints/seller_workspace.py` `require_active_seller` all require `provisioning` (which also admits `active`; not_requested and suspended are refused). Credential-bearing Seller Workspace routes (create AWS/R2 connection, replace R2 credentials, verify, rotate, disconnect, GET connection authorization, set profile source keys, authorize profile runtime) additionally require completed 2FA (`require_seller_workspace_2fa`). First publish is serialised by `pg_advisory_xact_lock('vz_publish:{seller}:{raw_id}')` taken before the local/non-local dispatch | `tests/test_vz_publish.py`, `tests/test_vz_publish_local_route.py::test_first_publish_same_raw_id*`, `tests/test_s1647_seller_workspace_b2b.py`; Gate 3 unanimous R3 (GLM, DeepSeek, Gemini); backend `33130819`, deployed 2026-09-22 | S1739 |
| Payout readiness is the purchase gate | SHIPPED | `app/services/seller_setup_service.py:assert_seller_payout_ready` (422 seller-setup hold) is called by browser checkout, `app/services/agent_service.py:execute_purchase` (agent and wallet purchases) and `app/services/x402_service.py` (`create_payment_offer` before any offer or transaction row, `verify_payment` before on-chain verification). A published listing whose seller has no live payouts cannot be paid for on any rail | `tests/test_x402.py::TestPayoutReadiness`, agent purchase tests; backend `33130819` | S1739 |
