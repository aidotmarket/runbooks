...
- id: E-05
  ...
  pre_conditions:
    ...
    - use a seller-purpose synthetic fixture whose exact account id is explicitly authorized by production preflight; buyer-01 is the read-only buyer fixture for BROWSER charters and must not be driven through the dashboard as a seller. This is scoped to the browser: it does NOT govern an AIM Data seller run, where `buyer-01` is the sanctioned account (Max ruling 2026-09-19, S1720; see run-it-as-a-seller.md) and already publishes nightly through the AIM Data API.
  ...
  argument_sourcing:
    account_id: for a browser charter, an explicitly authorized seller-purpose is_test fixture, never buyer-01 and never a real customer; an AIM Data seller run is out of this entry's scope and uses buyer-01
...
| F-12 | A BROWSER seller-publication charter uses buyer-01, finds no create control or seller readiness, and files a product ticket | The charter asks the buyer-only fixture to perform the unshipped Phase 3 seller journey through the dashboard. Note this is about the browser surface only: buyer-01 publishing through the AIM Data API is sanctioned and works (run-it-as-a-seller.md). The dashboard Listings page is a management surface for AIM Data/vectorAIz publications, not direct creation. | Confirm Phase 3 is still PLANNED in Capabilities; confirm the charter account is buyer-01; inspect the deployed Listings contract for the AIM Data/vectorAIz management-only empty state | Repair-12 | CONFIRMED |
