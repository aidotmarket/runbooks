...
> **S1253 ROTATION:** the previous token died (9109, T-2026-000273). Max minted `titan-1-koskadeux-cf-2026-07` (account-scoped, NO expiry) on 2026-07-17, stored in the same Infisical slot. Scopes: Workers Scripts Write, Workers KV Storage Write, Cloudflare Tunnel Write, Connectivity Directory Admin (account) + Zone Read, DNS Write, **Workers Routes Write** (all zones). Drift item 6 is CLOSED — routes now attach via `wrangler deploy` (proven: aim-channel* route, worker version d9a442e9). The scope caveats in the paragraph below describe the OLD token and are historical.
